	...
	BaseApplicableConfigPanel_SpecificConfig( parent )
{
	...
	m_check_SynchronousGS = new pxCheckBox( left, _("使用同步 MTGS"),
		_t("For troubleshooting potential bugs in the MTGS only, as it is potentially very slow.")
	);

	m_check_DisableOutput = new pxCheckBox( left, _("禁用所有 GS 输出"),
	...
	m_span->AddFrame(_("帧数步进"));

	m_fpan = new FramelimiterPanel( left );
	m_fpan->AddFrame(_("帧数限制"));
