  ...
  switch (file_status(fname)) {
   ...
   /*     if (!have_lockfile()) {
          if (try_locking(get_options(), 0)<0) {
            ...
            log(severity,LD_FS,get_lang_str(LANG_LOG_ROUTER_FILE_LOCKED),fname);
...
router_orport_found_reachable(void)
{
  if (!can_reach_or_port) {
    ...
    if (!me) { /* should never happen */
      log_warn(LD_BUG,get_lang_str(LANG_LOG_ROUTER_ORPORT_IS_REACHABLE_2));
      return;
    }
...
router_dirport_found_reachable(void)
{
  if (!can_reach_dir_port) {
    ...
    if (!me || decide_to_advertise_dirport(get_options(), me->dir_port))
      mark_my_descriptor_dirty();
    if (!me) { /* should never happen */
      log_warn(LD_BUG,get_lang_str(LANG_LOG_ROUTER_DIRPORT_IS_REACHABLE_2));
      return;
    }
                         ...
                         crypto_pk_env_t *ident_key)
{
...
#ifdef DEBUG_ROUTER_DUMP_ROUTER_TO_STRING
  {
    ...
    if (!ei_tmp) {
      log_err(LD_BUG,get_lang_str(LANG_LOG_ROUTER_ERROR_PARSING_EXTRAINFO),s);
      tor_free(s_dup);
      return -1;
    }
    tor_free(s_dup);
