...
inform_testing_reachability(void)
{
  ...
  log(LOG_NOTICE,LD_OR,get_lang_str(LANG_LOG_CIRCUITBUILD_TEST_REACHABILITY),me->address, me->or_port,me->dir_port ? dirbuf : "",me->dir_port ? "are" : "is",TIMEOUT_UNTIL_UNREACHABILITY_COMPLAINT/60);
  return 1;
}
...
circuit_send_next_onion_skin(origin_circuit_t *circ)
{
  ...
  if (circ->cpath->state == CPATH_STATE_CLOSED) {
    ...
    log_info(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITBUILD_FIRST_HOP_CELL_SENT),fast ? "CREATE_FAST" : "CREATE",router ? router->nickname : "<unnamed>");
  } else {
    ...
    if (!hop) {
      ...
      if (!has_completed_circuit && !circ->build_state->onehop_tunnel) {
        ...
        log(LOG_NOTICE, LD_GENERAL,get_lang_str(LANG_LOG_CIRCUITBUILD_CIRCUIT_OPEN_SUCCESS));
...
{
  if(get_options()->DirFlags&16)
  {	//adjust_time(seconds_elapsed);
  	return;
  }
  int severity = server_mode(get_options()) ? LOG_WARN : LOG_NOTICE;
  log(severity, LD_GENERAL,get_lang_str(LANG_LOG_CIRCUITBUILD_YOUR_SYSTEM_CLOCK),seconds_elapsed >=0 ? seconds_elapsed : -seconds_elapsed,seconds_elapsed >=0 ? "forward" : "backward");
...
//  log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITBUILD_CHOOSING_EXIT_NODE),
//         n_pending_connections);
  ...
  for (i = 0; i < smartlist_len(dir->routers); ++i) {/* iterate over routers */
    ...
    if (router_is_me(router)) {
      n_supported[i] = -1;
//      log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITBUILD_SKIPPING_SELF), router->nickname);
    ...
    if (!(router->is_valid || options->_AllowInvalid & ALLOW_INVALID_EXIT)) {
      ...
      n_supported[i] = -1;
//      log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITBUILD_SKIPPING_INVALID),
//             router->nickname, i);
    ...
    if (router_exit_policy_rejects_all(router)) {
      n_supported[i] = -1;
//      log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITBUILD_SKIPPING_REJECT_ALL),
//             router->nickname, i);
    ...
    SMARTLIST_FOREACH(connections, connection_t *, conn,
    {
      ...
      if (connection_ap_can_use_exit(TO_EDGE_CONN(conn), router)) {
        ++n_supported[i];
//        log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITBUILD_N_SUPPORTED),
//               router->nickname, i, n_supported[i]);
      } else {
//        log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITBUILD_WOULD_REJECT_THIS),
//               router->nickname, i);
      }
...
//      log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITBUILD_NEW_BEST),
//             router->nickname);
  ...
  if (best_support > 0) {
    ...
    else if((unsigned int)crypto_rand_int(100)<options->FavoriteExitNodesPriority)
    {	routersets_get_disjunction(use, supporting, options->ExitNodes,options->_ExcludeExitNodesUnion, 1);
...
//          log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITBUILD_TRY_NEW_POSSIBILITY),
//                 try, router->nickname);
  ...
  if (router) {
    ...
    log_info(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITBUILD_CHOSEN_EXIT_SERVER),router->nickname);
    if(options->IdentityFlags&IDENTITY_FLAG_LIST_SELECTION && !get_router_id_sel())	set_router_id_sel(router->router_id,1);
    return router;
  }
  if(options->StrictExitNodes)
  {	if(!(notify_no_exit&8))
...
//    log_debug(LD_CIRC,
//              get_lang_str(LANG_LOG_CIRCUITBUILD_CONTEMPLATING),
//              i, r->nickname);
    if (r->is_running == 0) {
//      log_debug(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITBUILD_NOPE_DIR_SAYS_NOT_RUNNING),i);
      goto next_i_loop;
    }
    if (r->is_valid == 0) {
//      log_debug(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITBUILD_NOPE_DIR_SAYS_INVALID),i);
      goto next_i_loop;
...
entry_guards_compute_status(void)
{
  ...
  if (changed) {
    SMARTLIST_FOREACH_BEGIN(entry_guards, entry_guard_t *, entry) {
      ...
      log_info(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITBUILD_ENTRY_GUARD_SUMMARY),entry->nickname,entry->unreachable_since ? "unreachable" : "reachable",entry->bad_since ? "unusable" : "usable",reason ? ", ": "",reason ? reason : "",entry_is_live(entry, 0, 1, 0) ? "live" : "not live");
    } SMARTLIST_FOREACH_END(entry);
...
bridges_retry_helper(int act)
{
  ...
  log_debug(LD_DIR,get_lang_str(LANG_LOG_CIRCUITBUILD_BRIDGE_STATS),any_known, any_running);
