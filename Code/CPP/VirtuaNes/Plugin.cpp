...
BOOL	CPlugin::FindPlugin( LPCSTR lpModulePath )
{
	...
	while( bFind ) {
		DWORD	dwHandle;
		DWORD	dwSize = ::GetFileVersionInfoSizeA( find.cFileName, &dwHandle );

		if( dwSize && (hDLL = ::LoadLibraryA( find.cFileName )) ) {
			...
			if( !(!GetVlpVersion || !GetVlpLanguage || !GetVlpLocaleID) ) {
				...
				if( ver >= VIRTUANES_PLUGIN_VERSION ) {
					...
					::strcpy( (LPSTR)m_Plugin[m_PluginNum].Path, Path.c_str() );
					GetVlpLanguage( (LPSTR)m_Plugin[m_PluginNum].Language );
					m_PluginNum++;
				}
			}
		...
		bFind = ::FindNextFileA( hFind, &find );
...
BOOL	CPlugin::FindPlugin( LPWSTR lpModulePath )
{
	m_PluginNum = 0;

	wstring	Path = CPathlib::MakePath( lpModulePath, L"*.vlp" );
	...
	while( bFind ) {
		...
		if( dwSize && (hDLL = ::LoadLibrary( find.cFileName )) ) {
			...
			if( !(!GetVlpVersion || !GetVlpLanguage || !GetVlpLocaleID) ) {
				...
				if( ver >= VIRTUANES_PLUGIN_VERSION ) {
					m_Plugin[m_PluginNum].LocaleID = GetVlpLocaleID();
					wstring	Path = CPathlib::MakePath( lpModulePath, find.cFileName );
					::wcscpy( m_Plugin[m_PluginNum].Path, Path.c_str() );
					GetVlpLanguage( (LPSTR)m_Plugin[m_PluginNum].Language );
					m_PluginNum++;
				}
			}
