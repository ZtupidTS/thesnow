	...
	: BaseConfigurationDialog( parent, AddAppName(_("模拟设置 - %s")), 580 )
{
	...
	AddPage<VideoPanel>				( pxL("视频图像"),		cfgid.Cpu );
	AddPage<GSWindowSettingsPanel>	( pxL("视频窗口"),		cfgid.Video );
	AddPage<SpeedHacksPanel>		( pxL("游戏加速"),		cfgid.Speedhacks );
	AddPage<GameFixesPanel>			( pxL("游戏修正"),		cfgid.Gamefixes );
	...
	: BaseConfigurationDialog( parent, AddAppName(_("组件选择 - %s")),  600 )
{
	...
	AddPage<PluginSelectorPanel>	( pxL("插件"),		cfgid.Plugins );
