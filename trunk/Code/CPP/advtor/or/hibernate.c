...
accounting_parse_options(or_options_t *options, int validate_only)
{
  ...
  case UNIT_WEEK:
    ...
    if (!ok) {
      log_warn(LD_CONFIG,get_lang_str(LANG_LOG_HIBERNATE_ACCOUNTING_DAY_ERROR));
      goto err;
    }
    break;
  case UNIT_MONTH:
    ...
    if (!ok) {
      log_warn(LD_CONFIG,get_lang_str(LANG_LOG_HIBERNATE_ACCOUNTING_MONTH_ERROR));
      goto err;
    }
    break;
  case UNIT_DAY:
  ...
  if (!ok) {
    log_warn(LD_CONFIG,get_lang_str(LANG_LOG_HIBERNATE_ACCOUNTING_HOUR_ERROR));
    goto err;
  }
  if (!cp || *cp!=':') {
    log_warn(LD_CONFIG,get_lang_str(LANG_LOG_HIBERNATE_ACCOUNTING_TIME_ERROR));
    goto err;
  }
  m = tor_parse_long(cp+1, 10, 0, 59, &ok, &cp);
  if (!ok) {
    log_warn(LD_CONFIG,get_lang_str(LANG_LOG_HIBERNATE_ACCOUNTING_MINUTE_ERROR));
    goto err;
  }
  if (!cp || *cp!='\0') {
    log_warn(LD_CONFIG,get_lang_str(LANG_LOG_HIBERNATE_ACCOUNTING_TIME_ERROR));
