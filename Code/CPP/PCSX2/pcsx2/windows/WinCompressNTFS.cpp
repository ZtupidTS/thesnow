...
void StreamException_ThrowLastError( const wxString& streamname, HANDLE result )
{
	...
	switch( error )
	{
		...
		default:
		{
			throw Exception::Stream( streamname ).SetDiagMsg(wxsFormat( L"常规 Win32 文件/流 错误 [GetLastError: %d]", error ));
		}
	}
}
...
void NTFS_CompressFile( const wxString& file, bool compressStatus )
{
	...
	if( !StreamException_LogLastError( file, L"NTFS 压缩提示", bloated_crap ) )
	{
		...
		if( !result )
			StreamException_LogLastError( file, L"NTFS 压缩提示" );
