...
connection_cpu_process_inbuf(connection_t *conn)
{
  ...
  if (conn->state == CPUWORKER_STATE_BUSY_ONION) {
    ...
    log_debug(LD_OR,get_lang_str(LANG_LOG_WORKER_ONIONSKIN_ANSWER_OK));
  } else {
...
//      log_fn(LOG_ERR,"read type failed. Exiting.");
      if (r == 0) {
        log_info(LD_OR,get_lang_str(LANG_LOG_WORKER_CONN_CLOSED));
      } else {
        log_info(LD_OR,get_lang_str(LANG_LOG_WORKER_CONN_ERROR),fd,tor_socket_strerror(tor_socket_errno(fd)));
      }
      goto end;
    }
