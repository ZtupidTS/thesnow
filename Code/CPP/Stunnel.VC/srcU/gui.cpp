...
static void message_box(const LPWSTR, const UINT);
...
////////////////动态调用API声明/////////////////
#ifdef UNICODE
DWORD (WINAPI *pGetModuleFileNameW)(HMODULE,LPWSTR,DWORD);
DWORD (WINAPI *pGetModuleFileNameExW)(HANDLE,HMODULE hModule,LPWSTR,DWORD);
BOOL (WINAPI *pGetSaveFileNameW)(LPOPENFILENAMEW);
HINSTANCE (WINAPI *pShellExecuteW)(HWND,LPCWSTR,LPCWSTR,LPCWSTR,LPCWSTR,INT);
BOOL (WINAPI *pShell_NotifyIconW)(DWORD,PNOTIFYICONDATAW);
#define pGetModuleFileName pGetModuleFileNameW
#define pGetModuleFileNameEx pGetModuleFileNameExW
#define pGetSaveFileName pGetSaveFileNameW
#define pShellExecute pShellExecuteW
#define pShell_NotifyIcon pShell_NotifyIconW
#else
DWORD (WINAPI *pGetModuleFileNameA)(HMODULE,LPSTR,DWORD);
DWORD (WINAPI *pGetModuleFileNameExA)(HANDLE,HMODULE hModule,LPSTR,DWORD);
BOOL (WINAPI *pGetSaveFileNameA)(LPOPENFILENAMEA);
HINSTANCE (WINAPI *pShellExecuteA)(HWND,LPCSTR,LPCSTR,LPCSTR,LPCSTR,INT);
BOOL (WINAPI *pShell_NotifyIconA)(DWORD,PNOTIFYICONDATAA);
#define pGetModuleFileName pGetModuleFileNameA
#define pGetModuleFileNameEx pGetModuleFileNameExA
#define pGetSaveFileName pGetSaveFileNameA
#define pShellExecute pShellExecuteA
#define pShell_NotifyIcon pShell_NotifyIconA
#endif
////////////////动态调用API声明/////////////////


int WINAPI _tWinMain(HINSTANCE this_instance, HINSTANCE prev_instance,
#ifdef UNICODE
        LPWSTR lpCmdLine,
#else
        LPSTR lpCmdLine,
...
#ifdef UNICODE
	(FARPROC&)pGetModuleFileNameW=GetProcAddress(hKernel32,"GetModuleFileNameW");
	(FARPROC&)pGetModuleFileNameExW=GetProcAddress(hPSAPI,"GetModuleFileNameExW");
	(FARPROC&)pGetSaveFileNameW=GetProcAddress(hComdlg32,"GetSaveFileNameW");
	(FARPROC&)pShellExecuteW=GetProcAddress(hShell32,"ShellExecuteW");
	(FARPROC&)pShell_NotifyIconW=GetProcAddress(hShell32,"Shell_NotifyIconW");
#else
	(FARPROC&)pGetModuleFileNameA=GetProcAddress(hKernel32,"GetModuleFileNameA");
	(FARPROC&)pGetModuleFileNameExA=GetProcAddress(hPSAPI,"GetModuleFileNameExA");
	(FARPROC&)pGetSaveFileNameA=GetProcAddress(hComdlg32,"GetSaveFileNameA");
	(FARPROC&)pShellExecuteA=GetProcAddress(hShell32,"ShellExecuteA");
	(FARPROC&)pShell_NotifyIconA=GetProcAddress(hShell32,"Shell_NotifyIconA");
#endif
//
    LPSTR command_line;
#ifndef _WIN32_WCE
...
#ifndef _WIN32_WCE
   pGetModuleFileName(0, stunnel_exe_path, MAX_PATH);
...
//    c=strrchr(stunnel_exe_path, '\\'); /* last backslash */
 ...
 //       errmsg=str_printf("不能设置当前目录到 %s", stunnel_exe_path);
...
static BOOL CALLBACK enum_windows(HWND other_window_handle, LPARAM lParam) {
    ...
    HANDLE hProcess;
    TCHAR *stunnel_exe_path=(TCHAR *)lParam;
    ...
    if(!pGetModuleFileNameEx(hProcess, hInstance, window_exe_path, MAX_PATH)) {
...
static int initialize_winsock() {
    ...
    if(WSAStartup(MAKEWORD( 2, 2 ), &wsa_state)) {
        message_box("初始化 winsock 失败...", MB_ICONERROR);
        return 1; /* error */
    }
#ifndef _WIN32_WCE
    handle=LoadLibrary(TEXT("ws2_32.dll")); /* IPv6 in Windows XP or higher */
    if(handle) {
    ...
    handle=LoadLibrary(TEXT("wship6.dll")); /* experimental IPv6 for Windows 2000 */
    if(handle) {
        s_getaddrinfo=(GETADDRINFO)GetProcAddress(handle, "getaddrinfo");
        s_freeaddrinfo=(FREEADDRINFO)GetProcAddress(handle, "freeaddrinfo");
        s_getnameinfo=(GETNAMEINFO)GetProcAddress(handle, "getnameinfo");
...
static int gui_loop() {
    ...
    RegisterClass(&wc);
#else
    wc.hIconSm=(HICON)small_icon; /* 16x16 icon */
    RegisterClassEx(&wc);
#endif
...
static LRESULT CALLBACK window_proc(HWND main_window_handle,
    ...
    case WM_CREATE:
        ...
        SendMessage(edit_handle, WM_SETFONT,
            (WPARAM)CreateFont(-12, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE,
                ...
                PROOF_QUALITY, DEFAULT_PITCH, TEXT("Microsoft YaHei")),
    ...
    case WM_DESTROY:
        ...
        pShell_NotifyIcon(NIM_DELETE, &nid); /* this removes the icon */
        PostQuitMessage(0);
...
//                    "stunnel.conf", NULL);
			pShellExecute(NULL,TEXT("open"),TEXT("notepad.exe"),TEXT("stunnel.conf"),NULL,SW_SHOW);
#endif
            break;
...
#ifndef _WIN32_WCE
            if(!cmdline.service) /* security */
                pShellExecute(main_window_handle, TEXT("open"),
                    TEXT("http://www.stunnel.org/static/stunnel.html"), NULL, NULL, SW_SHOWNORMAL);
#endif
            break;
        case IDM_HOMEPAGE:
#ifndef _WIN32_WCE
...
static LRESULT CALLBACK pass_proc(HWND dialog_handle, UINT message,
    ...
    case WM_INITDIALOG:
        ...
        titlebar=str_printf("私钥: %s", ui_data->section->key);
...
int pin_cb(UI *ui, UI_STRING *uis) {
    ui_data=(UI_DATA *)UI_get0_user_data(ui); /* was: ui_data=UI_get_app_data(ui); */
    if(!ui_data) {
...
static void save_log() {
    ...
    ofn.hwndOwner=hwnd;
    ofn.lpstrFilter=TEXT("日志文件 (*.log)\0*.log\0所有文件 (*.*)\0*.*\0\0");
    ofn.lpstrFile=file_name;
    ofn.nMaxFile=MAX_PATH;
    ofn.lpstrDefExt=TEXT("LOG");
    ofn.lpstrInitialDir=TEXT(".");

    ofn.lpstrTitle=TEXT("保存日志");
...
static void win_log(LPSTR line) {
    ...
    curr=(LIST *)str_alloc(sizeof(struct LIST)+len*sizeof(TCHAR));
    curr->len=len;
...
static LPTSTR log_txt(void) {
    ...
    buff=(TCHAR*)str_alloc((len+1)*sizeof(TCHAR)); /* +1 for trailing '\0' */
    if(!buff)
        return NULL;
    for(curr=head; curr; curr=curr->next) {
...
static void invalid_config() {
    /* update the main window title */
        win32_name=TEXT("Stunnel ") TEXT(STUNNEL_VERSION) TEXT(" on ")
            TEXT(STUNNEL_PLATFORM) TEXT(" (无效 stunnel.conf)");
...
static void update_tray_icon(void) {
    NOTIFYICONDATA nid;
	TCHAR *win32_names=new TCHAR[255];
    ...
    if(error_mode)
        _stprintf(nid.szTip, TEXT("服务已关闭什么的"));
    else
        _stprintf(nid.szTip, TEXT("目前活动会话数: %d"), num_clients);
    nid.uFlags=NIF_TIP;
	_stprintf(win32_names,TEXT("%s[活动会话 %d]"),win32_name,num_clients);
	if(hwnd) SetWindowText(hwnd, win32_names);
    /* only nid.szTip and nid.uID are valid, change tip */
    if(pShell_NotifyIcon(NIM_MODIFY, &nid)) /* modify tooltip */
    ...
    nid.hIcon=(HICON)small_icon; /* 16x16 icon */
    pShell_NotifyIcon(NIM_ADD, &nid); /* this adds the icon */
}
...
static void message_box(const LPWSTR text, const UINT type) {
	if(cmdline.quiet)
		return;
#ifdef UNICODE
	MessageBox(hwnd, text, win32_name, type);
#else
	MessageBoxW(hwnd, text,L"SSL 加密隧道", type);
#endif
...
//    service_path=str_printf("\"%s\" -service %s", stunnel_exe_path, command_line);
    _stprintf(service_path,TEXT("\"%s\" -service %s"), stunnel_exe_path, commandline);
...
//    str_free(service_path);
    ...
    message_box("服务已成功安装", MB_ICONINFORMATION);
...
static int service_uninstall(void) {
    ...
    if(serviceStatus.dwCurrentState!=SERVICE_STOPPED) {
        message_box("服务目前仍然在运行,表示你需要先停止服务什么的.", MB_ICONERROR);
        CloseServiceHandle(service);
        CloseServiceHandle(scm);
        return 1;
    }
    ...
    message_box("表示服务已经被干掉了.", MB_ICONINFORMATION);
    CloseServiceHandle(service);
    CloseServiceHandle(scm);
    return 0;
}
...
static int service_start(void) {
    ...
    if(serviceStatus.dwCurrentState!=SERVICE_RUNNING) {
        message_box("表示启动服务失败.", MB_ICONERROR);
        CloseServiceHandle(service);
        CloseServiceHandle(scm);
        return 1;
    }
    message_box("那啥服务已经成功启动了.", MB_ICONINFORMATION);
    CloseServiceHandle(service);
    CloseServiceHandle(scm);
    return 0;
}
...
static int service_stop(void) {
    ...
    if(serviceStatus.dwCurrentState==SERVICE_STOPPED) {
        message_box("表示服务本来就没有运行.", MB_ICONERROR);
        CloseServiceHandle(service);
        CloseServiceHandle(scm);
        return 1;
    }
    ...
    message_box("表示服务已经成功停止.", MB_ICONINFORMATION);
    CloseServiceHandle(service);
    CloseServiceHandle(scm);
    return 0;
}
