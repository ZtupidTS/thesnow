...
void CMemcardManager::CreateGUIControls()
{
	...
	for (int slot = SLOT_A; slot <= SLOT_B; slot++)
	{
		...
		m_PrevPage[slot] = new wxButton(this, ID_PREVPAGE_A + slot, _("上一页"));
		m_NextPage[slot] = new wxButton(this, ID_NEXTPAGE_A + slot, _("下一页"));
		...
		m_MemcardPath[slot] = new wxFilePickerCtrl(this, ID_MEMCARDPATH_A + slot,
			 wxString::From8BitData(File::GetUserPath(D_GCUSER_IDX).c_str()), _("选择内存卡:"),
		_("Gamecube 内存卡 (*.raw,*.gcp)") + wxString(wxT("|*.raw;*.gcp")), wxDefaultPosition, wxDefaultSize, wxFLP_USE_TEXTCTRL|wxFLP_OPEN);
		...
		sMemcard[slot] = new wxStaticBoxSizer(wxVERTICAL, this, _("内存卡") + wxString::Format(wxT(" %c"), 'A' + slot));
...
void CMemcardManager::CopyDeleteClick(wxCommandEvent& event)
{
	...
	case ID_SAVEIMPORT_B:
	{
		...
		{
			wxString temp2 = wxFileSelector(_("保存 GCI 为.."),
				wxEmptyString, wxEmptyString, wxT(".gci"),
				_("GCI 文件(*.gci)") + wxString(_T("|*.gci")),
...
void CMemcardManager::CMemcardListCtrl::OnRightClick(wxMouseEvent& event)
{
	...
	if (item != wxNOT_FOUND) 
	{
		...
		popupMenu->Append(ID_COPYFROM_A + slot, wxString::Format(_("复制到内存卡 %c"), 'B' - slot));
		popupMenu->Append(ID_DELETE_A + slot, _("删除存档"));
		popupMenu->Append(ID_SAVEIMPORT_A + slot, _("导入存档"));
		popupMenu->Append(ID_SAVEEXPORT_A + slot, _("导出存档"));
		popupMenu->Append(ID_EXPORTALL_A + slot, _("导出所有存档"));
				
		popupMenu->FindItem(ID_COPYFROM_A + slot)->Enable(__mcmSettings.twoCardsLoaded);

		popupMenu->AppendSeparator();

		popupMenu->Append(ID_FIXCHECKSUM_A + slot, _("修正校检和"));
		popupMenu->Append(ID_PREVPAGE_A + slot, _("上一页"));
		popupMenu->Append(ID_NEXTPAGE_A + slot, _("下一页"));
		popupMenu->Append(ID_MEMCARDPATH_A + slot, wxString::Format(_("设置为默认内存卡 %c"), 'A' + slot));
...
//		popupMenu->AppendCheckItem(COLUMN_BANNER, _("显示存档LOGO"));
		popupMenu->AppendCheckItem(COLUMN_TITLE, _("显示存档标题"));
		popupMenu->AppendCheckItem(COLUMN_COMMENT, _("Show save comment"));
		popupMenu->AppendCheckItem(COLUMN_ICON, _("显示存档图标"));
