...
static wxString ResultToString( int result, const MsgButtons& buttons )
{
	switch( result )
	{
		case wxID_OK:		return L"确定";
		case wxID_CANCEL:	return L"取消";
		case wxID_APPLY:	return L"应用";
		case wxID_YES:		return L"是";
		case wxID_NO:		return L"否";
		case wxID_YESTOALL:	return L"全是";
		case wxID_NOTOALL:	return L"全否";

		case wxID_ABORT:	return L"终止";
		case wxID_RETRY:	return L"重试";

		// [TODO] : maybe add in an Ignore All?
		case wxID_IGNORE:	return L"忽略";

		case wxID_RESET:	return L"重置";
		case wxID_CLOSE:	return L"关闭";
	}
...
wxWindowID pxIssueConfirmation( wxDialogWithHelpers& confirmDlg, const MsgButtons& buttons, const wxString& disablerKey )
{
	...
	if( cfg != NULL )
	{
		...
		DisablerCtrl = new pxCheckBox(&confirmDlg, _("不要再显示这个对话框."));
	...
	: wxPanelWithHelpers( parent, wxHORIZONTAL )
{
	...
	if( buttons.HasReset() )
		AddCustomButton( wxID_RESET, _("重置") );
	...
	if( buttons.HasIgnore() )
		AddCustomButton( wxID_IGNORE, _("忽略") );
