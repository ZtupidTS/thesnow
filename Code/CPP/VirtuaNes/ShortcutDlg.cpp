...
void	CShortcutDlg::OnInitialMember()
{
	...
	for( INT i = 0; CConfig::ShortcutKeyID[i*3+0]; i++ ) {
		...
		// 設定ｷｰ
		if( m_ControlBuf[i] == 0 ) {
			ListView_SetItemText( hWndCtrl, i, 1, L"----" );
		} else {
			wstring	str = Config.ShortcutToKeyName( m_ControlBuf[i] );
			ListView_SetItemText( hWndCtrl, i, 1, (LPWSTR)str.c_str() );
		}
		// 設定ｷｰ2
		if( m_ControlBuf2[i] == 0 ) {
			ListView_SetItemText( hWndCtrl, i, 2, L"----" );
		} else {
			wstring	str = Config.ShortcutToKeyName( m_ControlBuf2[i] );
			ListView_SetItemText( hWndCtrl, i, 2, (LPWSTR)str.c_str() );
		}
	}
...
//	DEBUGOUT( "CShortcutDlg::OnInitDialog\n" );
	...
	// 設定中メッセージの保存
	CApp::LoadString( IDS_CUT_TITLE, (LPTSTR)m_szMessage, sizeof(m_szMessage) );
	...
	if( wParam == (WPARAM)m_TimerID ) {
		...
		// Button push?
			...
			if( DirectInput.m_Sw[DIK_ESCAPE] && !(DirectInput.m_Sw[DIK_LSHIFT] || DirectInput.m_Sw[DIK_RSHIFT]) ) {
				if( m_SelectSubID == 2 ) {
					ListView_SetItemText( hWndCtrl, m_SelectID, 2, L"----" );
					m_ControlBuf2[m_SelectID] = 0;
				} else {
					ListView_SetItemText( hWndCtrl, m_SelectID, 1, L"----" );
					...
					if( DirectInput.m_Sw[i]&0x80 ) {
						if( DirectInput.SearchKeyName(i) ) {
							...
							if( m_SelectSubID == 2 ) {
								...
								ListView_SetItemText( hWndCtrl, m_SelectID, 2, (LPTSTR)str.c_str() );
							} else {
								m_ControlBuf [m_SelectID] = keyex+i;
								ListView_SetItemText( hWndCtrl, m_SelectID, 1, (LPTSTR)str.c_str() );
							}
