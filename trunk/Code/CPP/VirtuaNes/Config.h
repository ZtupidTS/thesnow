...
public:
	...
	WCHAR	szRomPath     [_MAX_PATH];
	WCHAR	szSavePath    [_MAX_PATH];
	WCHAR	szStatePath   [_MAX_PATH];
	WCHAR	szSnapshotPath[_MAX_PATH];
	WCHAR	szMoviePath   [_MAX_PATH];
	WCHAR	szWavePath    [_MAX_PATH];
	WCHAR	szCheatPath   [_MAX_PATH];
	WCHAR	szIpsPath     [_MAX_PATH];
	...
	void	Default() {
		...
		::lstrcpy( szRomPath,      L".\\roms\\" );
		::lstrcpy( szSavePath,     L".\\save\\" );
		::lstrcpy( szStatePath,    L".\\state\\" );
		::lstrcpy( szSnapshotPath, L".\\snapshot\\" );
		::lstrcpy( szMoviePath,    L".\\movie\\" );
		::lstrcpy( szWavePath,     L".\\wave\\" );
		::lstrcpy( szCheatPath,    L".\\cheatcode\\" );
		::lstrcpy( szIpsPath,      L".\\ips\\" );
	}
};
...
// Converter
	wstring	ShortcutToKeyName( INT nShortcut );
...
public:
	// for Cartridge
	void	Load( DWORD crc );
	void	Save( LPCWSTR name, DWORD crc );
	// for FDS
	void	Load( DWORD gid, DWORD mid );
	void	Save( LPCWSTR name, DWORD gid, DWORD mid );
