...
void wxCheatsWindow::Init_ChildControls()
{
	...
	m_Notebook_Main = new wxNotebook(this, ID_NOTEBOOK_MAIN, wxDefaultPosition, wxDefaultSize);
		...
		m_Label_Codename = new wxStaticText(m_Tab_Cheats, ID_LABEL_CODENAME, _T("名称: "), wxDefaultPosition, wxDefaultSize);
		m_GroupBox_Info = new wxStaticBox(m_Tab_Cheats, ID_GROUPBOX_INFO, _T("Code 信息"), wxDefaultPosition, wxDefaultSize);
		m_Button_ApplyCodes = new wxButton(m_Tab_Cheats, ID_BUTTON_APPLYCODES, _T("应用修改"), wxDefaultPosition, wxDefaultSize);
		m_Label_NumCodes = new wxStaticText(m_Tab_Cheats, ID_LABEL_NUMCODES, _T("Codes 数量: "),  wxDefaultPosition, wxDefaultSize);
		...
		m_Button_UpdateLog = new wxButton(m_Tab_Log, ID_BUTTON_UPDATELOG, wxT("更新"));
		m_CheckBox_LogAR = new wxCheckBox(m_Tab_Log, ID_CHECKBOX_LOGAR, wxT("启用 AR 记录"));
	...
	m_Notebook_Main->AddPage(m_Tab_Cheats, _T("Codes 列表"));
	m_Notebook_Main->AddPage(m_Tab_Log, _T("记录"));

	// Button Strip
	m_Button_Close = new wxButton(this, ID_BUTTON_CLOSE, _T("关闭"), wxDefaultPosition, wxDefaultSize);
...
void wxCheatsWindow::OnEvent_CheatsList_ItemSelected(wxCommandEvent& WXUNUSED (event))
{
	...
	for (size_t i = 0; i < indexList.size(); i++)
	{
		...
		{
			ARCode code = GetARCode(i);
			m_Label_Codename->SetLabel(wxT("名称: ") + wxString(code.name.c_str(), *wxConvCurrent));
			char text[CHAR_MAX];
			char* numcodes = text;
