  ...
  /* Add the actual router entries. */
  {
    ...
    while (1) {
      ...
      if (rs_out.has_exitsummary) {
        ...
        if (r<0) {
          log_warn(LD_BUG,get_lang_str(LANG_LOG_DIRVOTE_BUFFER_TOO_SMALL_2));
          *buf = '\0';
        }
        smartlist_add(chunks, tor_strdup(buf));
      };
...
dirvote_compute_consensus(void)
{
  ...
  consensus = networkstatus_parse_vote_from_string(consensus_body, NULL,
                                                   NS_TYPE_CONSENSUS);
  if (!consensus) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_DIRVOTE_CONSENSUS_ERROR));
    goto err;
  }
    ...
    SMARTLIST_FOREACH(pending_consensus_signature_list, char *, sig,
      {
        ...
        if (r >= 0)
          ...
          log_warn(LD_DIR,get_lang_str(LANG_LOG_DIRVOTE_ERROR_ADDING_SIGNATURE),msg);
        tor_free(sig);
      });
    if (n_sigs)
      log_notice(LD_DIR,get_lang_str(LANG_LOG_DIRVOTE_SIGNATURES_ADDED),n_sigs);
    smartlist_clear(pending_consensus_signature_list);
  }
                       ...
                       const char **msg_out)
{
  ...
  *msg_out = NULL;
  ...
  if (!(sigs = networkstatus_parse_detached_signatures(
    ...
    *msg_out = get_lang_str(LANG_LOG_DIRVOTE_SIGNATURE_PARSING_FAILED);
    goto err;
  }

  log_info(LD_DIR,get_lang_str(LANG_LOG_DIRVOTE_SIGNATURES_TO_ADD),smartlist_len(sigs->signatures));
  ...
  if (r >= 1) {
    ...
    if (!new_detached) {
      *msg_out = get_lang_str(LANG_LOG_DIRVOTE_NO_SIGNATURES);
      goto err;
    }
    new_consensus_len =
    ...
    *msg_out = get_lang_str(LANG_LOG_DIRVOTE_SIGNATURES_ADDED_3);
  } else if (r == 0) {
    *msg_out = get_lang_str(LANG_LOG_DIRVOTE_SIGNATURES_IGNORED);
  } else {
    goto err;
  }
 ...
 err:
  if (!*msg_out)
    *msg_out = get_lang_str(LANG_LOG_DIRVOTE_ERROR_ADDING_SIGNATURES);
 done:
  if (sigs)
    ns_detached_signatures_free(sigs);
  return r;
...
dirvote_publish_consensus(void)
{
  ...
  if (!pending_consensus ||
    ...
    log_warn(LD_DIR,get_lang_str(LANG_LOG_DIRVOTE_CONSENSUS_INSUFFICIENT_INFO));
    return -1;
  }

  if (networkstatus_set_current_consensus(pending_consensus_body, 0))
    log_warn(LD_DIR,get_lang_str(LANG_LOG_DIRVOTE_CONSENSUS_PUBLISH_FAILED));
  else
    log_notice(LD_DIR,get_lang_str(LANG_LOG_DIRVOTE_CONSENSUS_PUBLISH_OK));
