...
bool COptionsPageTransfer::Validate()
{
	...
	if (!pCtrl->GetValue().ToLong(&tmp) || tmp < 1 || tmp > 1000)
		return DisplayError(pCtrl, _("Please enter a number between 1 and 1000 for the number of concurrent transfers."));
