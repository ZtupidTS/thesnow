...
{	plugin_info_t *plugin_tmp;
	...
	SMARTLIST_FOREACH(conns, connection_t *, conn,{
		...
		{	edge_connection_t *conn2=TO_EDGE_CONN(conn);
			...
			{	buffer[i].socks_original_address = &conn2->socks_request->original_address[0];
				buffer[i].socks_final_address = &conn2->socks_request->address[0];
			}
			else
	...
	SMARTLIST_FOREACH(conns, connection_t *, conn,{
		...
		{	if(CONN_IS_EDGE(conn))
			{	edge_connection_t *conn1=TO_EDGE_CONN(conn);
				...
				{	if(original_address) return &conn1->socks_request->original_address[0];
					else return &conn1->socks_request->address[0];
				}
				else return NULL;
			}
			else	return NULL;
		...
		{	if(CONN_IS_EDGE(conn))
			{	edge_connection_t *conn1=TO_EDGE_CONN(conn);
				...
				{	conn1->socks_request->command=command;
					tor_snprintf(conn1->socks_request->address,MAX_SOCKS_ADDR_LEN,original_address);
					tor_snprintf(conn1->socks_request->original_address,MAX_SOCKS_ADDR_LEN,original_address);
...
{	plugin_info_t *plugin_tmp;
	for(plugin_tmp=plugins;plugin_tmp && (plugin_tmp->hDll!=plugin_instance);plugin_tmp=plugin_tmp->next_plugin)	;
	if(!plugin_tmp || !(plugin_tmp->rights&PLUGIN_RIGHT__CAN_TRANSLATE_ADDRESSES)) return 0;
	strlcpy(translated_address,original_address,MAX_SOCKS_ADDR_LEN);
	addressmap_rewrite(translated_address,MAX_SOCKS_ADDR_LEN,NULL);
	plugins_remap(NULL,original_address,original_address,0);
...
{	return GeoIP_getfullname(geoip_get_country_by_ip(ip));
}
...
{	plugin_info_t *plugin_tmp;
	...
	strlcpy(conn->socks_request->address,remote_address,MAX_SOCKS_ADDR_LEN);
	strlcpy(conn->socks_request->original_address,conn->socks_request->address,MAX_SOCKS_ADDR_LEN);
...
{	plugin_info_t *plugin_tmp;
	...
	{	e_conn->socks_request->command=SOCKS_COMMAND_CONNECT;
		strlcpy(e_conn->socks_request->address,remote_address,MAX_SOCKS_ADDR_LEN);
		strlcpy(e_conn->socks_request->original_address,e_conn->socks_request->address,MAX_SOCKS_ADDR_LEN);
...
{	if(plugin_tmp->GetConfigurationWindow && plugin_tmp->rights&PLUGIN_RIGHT__ADVTOR_PAGE)
	{	plugin_tmp->listItem = next_list_item;
		SendDlgItemMessage(hMainDialog,200,LB_SETITEMDATA,SendDlgItemMessage(hMainDialog,200,LB_ADDSTRING,0,(LPARAM)plugin_tmp->dll_name),plugin_tmp->listItem);
	}
	int i;
...
{	if(plugin_tmp->GetConfigurationWindow && plugin_tmp->listItem)
	{	int i=0,j=0;
		while(j!=LB_ERR)
		{	j=SendDlgItemMessage(hMainDialog,200,LB_GETITEMDATA,i,0);
			if(j==plugin_tmp->listItem)
			{	SendDlgItemMessage(hMainDialog,200,LB_DELETESTRING,i,0);
	...
	if(plugin_tmp->connection_param!=-1 && plugin_tmp->connection_param<MAX_PLUGIN_CONNECTION_PARAMS)
...
{	if(!(plugin_tmp->load_status&PLUGIN_LOADSTATUS_LOADED) && (plugin_tmp->rights & PLUGIN_RIGHT__CAN_BE_LOADED))
	{	plugin_tmp->description[0]=0;
		if(!plugin_tmp->hDll)
		{	HINSTANCE hDll;
			hDll=GetModuleHandle(plugin_tmp->dll_name);
			if(!hDll)
			{	char *fname=get_datadir_fname("plugins");
				char *plname=tor_malloc(1024);
				tor_snprintf(plname,1024,"%s\\%s",fname,plugin_tmp->dll_name);
				hDll=LoadLibrary(plname);
...
{	char *fname=get_datadir_fname("plugins");
	...
	if((hFind=FindFirstFile(plname,&wfdata))!=INVALID_HANDLE_VALUE)
	{	while(1)
		{	plugin_tmp=plugins;
			while(plugin_tmp)
			{	if(!strcasecmp(plugin_tmp->dll_name,wfdata.cFileName)) break;
				plugin_tmp=plugin_tmp->next_plugin;
			}
			if(plugin_tmp)	plugin_tmp->load_status |= PLUGIN_LOADSTATUS_FOUND;
			else
			{	tor_snprintf(plname,1024,"%s\\%s",fname,wfdata.cFileName);
				isLoaded=0;
				HINSTANCE hDll;
				hDll=GetModuleHandle(plname);
				if(hDll) isLoaded++;
				else hDll=LoadLibraryEx(plname,0,DONT_RESOLVE_DLL_REFERENCES);
				if(hDll)
				{	if(GetProcAddress(hDll,PLUGIN_FN_INIT))
					{	plugin_info_t *newPlugin=tor_malloc_zero(sizeof(plugin_info_t));
						...
						tor_snprintf(newPlugin->dll_name,MAX_PATH,"%s",wfdata.cFileName);
						if(plugins)
						{	plugin_tmp=plugins;
...
{	plugin_info_t *plugin_tmp;
	...
	{	plugin_info_t *newPlugin=tor_malloc_zero(sizeof(plugin_info_t));
		...
		newPlugin->rights=PLUGIN_RIGHT__ALL_RIGHTS;
		newPlugin->load_status=PLUGIN_LOADSTATUS_FOUND;
		for(i=0;i<MAX_PATH && option->value[i]>32;i++)	newPlugin->dll_name[i]=option->value[i];
...
{	plugin_info_t *plugin_tmp;
	...
	{	if(plugin_tmp->TranslateAddress && plugin_tmp->rights&PLUGIN_RIGHT__CAN_TRANSLATE_ADDRESSES)
		{	if(conn)	r=(plugin_tmp->TranslateAddress)(TO_CONN(conn)->global_identifier&0xffffffff,original_address,address,(plugin_tmp->connection_param==-1)?NULL:&TO_CONN(conn)->lParam[plugin_tmp->connection_param],is_error);
			else	r=(plugin_tmp->TranslateAddress)(0,original_address,address,NULL,is_error);
			if(!r)
