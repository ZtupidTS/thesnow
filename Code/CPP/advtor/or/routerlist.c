...
{
  if(get_options()->DirFlags&2) return;
...
authority_certs_fetch_missing(networkstatus_t *status, time_t now)
{
  ...
  if (status) {
    SMARTLIST_FOREACH(status->voters, networkstatus_voter_info_t *, voter,
      {
        ...
        if (cert) {
          if ((now < cert->expires)||(options->DirFlags&2))
	...
	{	log_notice(LD_DIR,get_lang_str(LANG_LOG_ROUTERLIST_MISSING_CERT),hex_str(voter->signing_key_digest,DIGEST_LEN));
  ...
  SMARTLIST_FOREACH(trusted_dir_servers, trusted_dir_server_t *, ds,
    {
      ...
      SMARTLIST_FOREACH(cl->certs, authority_cert_t *, cert,
        {
          if ((options->DirFlags&2)||!ftime_definitely_after(now, cert->expires)) {
      ...
      if (!found &&
          download_status_is_ready(&cl->dl_status, now,options->MaxDlFailures?options->MaxDlFailures:32767) &&
...
router_rebuild_store(int flags, desc_store_t *store)
{
  ...
  if (!(flags & RRS_DONT_REMOVE_OLD)&&!(get_options()->DirFlags&2))
  ...
  if (! store->mmap) {
    if (errno == ERANGE) {
      ...
      log_warn(LD_FS,get_lang_str(LANG_LOG_ROUTERLIST_MMAP_ERROR),fname);
    }
  }
...
router_pick_directory_server_impl(authority_type_t type, int flags)
{
  ...
  if (smartlist_len(tunnel)) {
    ...
    result = get_options()->DirFlags&1?smartlist_choose(trusted_tunnel):smartlist_get(trusted_tunnel,0);
    ...
    result = get_options()->DirFlags&1?smartlist_choose(trusted_direct):smartlist_get(trusted_direct,0);
  }
                                  ...
                                  int *n_busy_out)
{
  ...
  if (smartlist_len(tunnel)) {
    result = get_options()->DirFlags&1?smartlist_choose(tunnel):smartlist_get(tunnel,0);
  } else if (smartlist_len(overloaded_tunnel)) {
    result = get_options()->DirFlags&1?smartlist_choose(overloaded_tunnel):smartlist_get(overloaded_tunnel,0);
  } else if (smartlist_len(direct)) {
    result = get_options()->DirFlags&1?smartlist_choose(direct):smartlist_get(direct,0);
  } else {
    result = get_options()->DirFlags&1?smartlist_choose(overloaded_direct):smartlist_get(overloaded_direct,0);
  }
...
routerlist_add_family(smartlist_t *sl, routerinfo_t *router)
{
  ...
  if (options->EnforceDistinctSubnets==1)
    routerlist_add_network_family(sl, router);
  else if (options->EnforceDistinctSubnets==2)
...
routers_in_same_family(routerinfo_t *r1, routerinfo_t *r2)
{
  ...
  if ((options->EnforceDistinctSubnets == 1) && routers_in_same_network_family(r1,r2))
    return 1;
  if ((options->EnforceDistinctSubnets == 2) && routers_in_same_country(r1,r2))
    return 1;
                          ...
                          router_crn_flags_t flags)
{
  ...
  if (!choice && !strict) {
    ...
    if (!choice && (need_uptime || need_capacity || need_guard)) {
      ...
      log_info(LD_CIRC,get_lang_str(LANG_LOG_ROUTERLIST_NO_LIVE_ROUTERS),need_capacity?get_lang_str(LANG_LOG_ROUTERLIST__FAST):"",need_uptime?get_lang_str(LANG_LOG_ROUTERLIST__STABLE):"",need_guard?get_lang_str(LANG_LOG_ROUTERLIST__GUARD):"");
  ...
  if (!choice) {
    if (strict) {
      log_warn(LD_CIRC,get_lang_str(LANG_LOG_ROUTERLIST_NO_LIVE_ROUTERS_2));
    } else {
                         ...
                         int from_cache, int from_fetch)
{
  ...
  if (sdmap_get(routerlist->desc_digest_map,
    ...
    log_info(LD_DIR,get_lang_str(LANG_LOG_ROUTERLIST_DESC_ALREADY_HAVE),router->nickname);
...
launch_router_descriptor_downloads(smartlist_t *downloadable, time_t now)
{
  ...
  if ((! should_delay || options->DirFlags&2) && n_downloadable) {
    int i, n_per_request;
  ...
  SMARTLIST_FOREACH(consensus->routerstatus_list, routerstatus_t *, rs,
    {
      ...
      if (!options->DirFlags&2 && !directory_caches_dir_info(options) &&
...
update_router_descriptor_downloads(time_t now)
{
  ...
  if (!options->Address &&
      ...
      last_dummy_download + DUMMY_DOWNLOAD_INTERVAL < now && !options->DirFlags&2) {
...
count_loading_descriptors_progress(void)
{
  ...
  fraction = num_present / ((get_options()->DirFlags&4?3:1)*num_usable/4.);
  if (fraction > 1.0)
...
update_router_have_minimum_dir_info(void)
{
  ...
  if (num_present < (options->DirFlags&4?3:1)*num_usable/4) {
                                       ...
                                       const char **msg)
{
  ...
  if (ei->pending_sig) {
    ...
    if (crypto_pk_public_checksig(ri->identity_pkey, signed_digest,sizeof(signed_digest),
                   ...
                   country_t country)
{
  ...
  if (addr && compare_tor_addr_to_addr_policy(addr, orport, set->policies)
    ...
    return 3;
/*  char *cname=(char*)geoip_get_country_name(geoip_get_country_by_ip(geoip_reverse(tor_addr_to_ipv4h(addr))));
  ...
  if (set->countries) {
    if (country < 0 && addr)
      country = geoip_get_country_by_ip(geoip_reverse(tor_addr_to_ipv4h(addr)));
                          ...
                          int running_only)
{
  ...
  /*  	if(name[0]=='{')
	...
	{*/
		...
		if (router)
		{	if (!running_only || router->is_running)	smartlist_add(out, router);
		}
//	}
    });
  } else {
...
{
  ri->country = geoip_get_country_by_ip(geoip_reverse(ri->addr));
}
  ...
  do {
    ...
    if (r->is_hs_dir) {
      if (r->dir_port || use_begindir)
        ...
        log_info(LD_REND,get_lang_str(LANG_LOG_ROUTERLIST_UNREACHABLE_ROUTER),r->nickname);
...
hid_serv_acting_as_directory(void)
{
  ...
  if (!(c = networkstatus_get_latest_consensus())) {
    log_info(LD_REND,get_lang_str(LANG_LOG_ROUTERLIST_NO_CONSENSUS_2));
    return 0;
  }
  rs = networkstatus_vote_find_entry(c, me->cache_info.identity_digest);
  if (!rs) {
    log_info(LD_REND,get_lang_str(LANG_LOG_ROUTERLIST_NOT_HS_DIR_2));
    return 0;
  }
  if (!rs->is_hs_dir) {
    log_info(LD_REND,get_lang_str(LANG_LOG_ROUTERLIST_NOT_HS_DIR_3));
    return 0;
  }
  return 1;
}
...
char contry_all[]="*";
char addr_all[]="*:*";
char name_all[]="<< Random router >>";
LV_ITEM lvit;

void insert_router_1(HWND hDlg,uint32_t lParam1,char *country,char *address,char *rname,char *bw,char *is_exit)
{	int lvErr;
	lvit.lParam=lParam1;
	lvit.mask=LVIF_TEXT|LVIF_PARAM;
	lvit.iSubItem=0;
	lvit.pszText=country;
	lvit.cchTextMax=3;
...
{	or_options_t *options=get_options();
	...
	{	if((selType==SELECT_EXIT&&router->is_exit)||(selType==SELECT_ENTRY&&router->is_possible_guard)||(selType==SELECT_ANY))
		{	raddr=geoip_reverse(router->addr);
			if((last_country_sel==0x200)||(last_country_sel==geoip_get_country_by_ip(raddr)))
			{	tor_snprintf(raddress,100,"%d.%d.%d.%d:%d",raddr&0xff,(raddr>>8)&0xff,(raddr>>16)&0xff,(raddr>>24)&0xff,router->or_port);
				...
				else if(router->bandwidthrate>1024)
				{	bandwidthrate=(double)router->bandwidthrate/1024;
					if(bandwidthrate>1024)
					{	bandwidthrate/=1024;
						if(bandwidthrate>1024)
						{	bandwidthrate/=1024;
							tor_snprintf(rbw,100,"%.2f GB",bandwidthrate);
						}
						else tor_snprintf(rbw,100,"%.2f MB",bandwidthrate);
					}
					else tor_snprintf(rbw,100,"%.2f kB",bandwidthrate);
				}
				else tor_snprintf(rbw,100,"%d B",router->bandwidthrate);
				if(selType==SELECT_ANY)
				{	s1=&is_exit[0];
					if(router->is_possible_guard) *s1++='E';
					if(router->is_exit) *s1++='X';
					*s1=0;
					insert_router_1(hDlg,j?-(router->router_id):(router->router_id),(char *)geoip_get_country_name(geoip_get_country_by_ip(raddr)),raddress,router->nickname,rbw,is_exit);
				}
				else	insert_router_1(hDlg,j?-(router->router_id):(router->router_id),(char *)geoip_get_country_name(geoip_get_country_by_ip(raddr)),raddress,router->nickname,rbw,NULL);
...
{	if(!routerlist)	return NULL;
	if(i&0x80000000) i = -i;
	i -= 1024;
	if(i < smartlist_len(routerlist->routers))	return smartlist_get(routerlist->routers,i);
	return NULL;
//	SMARTLIST_FOREACH(routerlist->routers, routerinfo_t *, router,
...
{	or_options_t *options=get_options();
	...
	{	SMARTLIST_FOREACH(routerlist->routers, routerinfo_t *, router,
		{	if((routerCount<MAX_TRAY_IP_LIST)&&(router->is_exit)&&(!routerset_contains_router(options->_ExcludeExitNodesUnion,router))&&routerset_contains_router(options->ExitNodes,router))
			{	raddr=geoip_reverse(router->addr);
				if((country_sel==0x200)||(country_sel==geoip_get_country_by_ip(raddr)))
				{	tor_snprintf(raddress,100,"[%s] %d.%d.%d.%d (%s)",geoip_get_country_name(geoip_get_country_by_ip(raddr)),raddr&0xff,(raddr>>8)&0xff,(raddr>>16)&0xff,(raddr>>24)&0xff,router->nickname);
	...
	SMARTLIST_FOREACH(routerlist->routers, routerinfo_t *, router,
	{	if((routerCount<MAX_TRAY_IP_LIST)&&(router->is_exit)&&(!routerset_contains_router(options->_ExcludeExitNodesUnion,router))&&(!options->ExitNodes || !routerset_contains_router(options->ExitNodes,router)))
		{	raddr=geoip_reverse(router->addr);
			if((country_sel==0x200)||(country_sel==geoip_get_country_by_ip(raddr)))
			{	tor_snprintf(raddress,100,"[%s] %d.%d.%d.%d (%s)",geoip_get_country_name(geoip_get_country_by_ip(raddr)),raddr&0xff,(raddr>>8)&0xff,(raddr>>16)&0xff,(raddr>>24)&0xff,router->nickname);
				iplist[routerCount]=router->addr;
...
{	if(router_id_sel) return (router_id_sel==router_id)?((exclKey&0xfffffffe)==0):((exclKey&0xfffffffe)!=0);
	...
	else if(geoip_get_country_by_ip(geoip_reverse(addr))==country_sel)	return 1;
	return 0;
}
...
{	*reply++=5;
	...
	{	int rcountry=geoip_get_country(country);
		...
		{	if((router->is_exit)&&(router->bandwidthrate>=bandwidth))
			{	raddr=router->addr;
				if(((rcountry==-1)||(rcountry==geoip_get_country_by_ip(raddr)))&&(raddr>=ip_range_low)&&(raddr<=ip_range_high))
				{	num_routers++;
				}
			}
		});
					...
					{	if(num_routers) num_routers--;
						...
						{	raddr=geoip_reverse(router->addr);
							...
							*reply++=(char)get_options()->CircuitPathLength;
							ret1=(char*)geoip_get_country_name(geoip_get_country_by_ip(raddr));
							if(ret1)
