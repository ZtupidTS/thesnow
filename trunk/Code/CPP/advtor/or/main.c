...
static or_options_t *options=NULL;
...
connection_add(connection_t *conn)
{
  ...
  if (conn->s >= 0 || conn->linked || conn->hs_plugin) {
...
connection_stop_reading(connection_t *conn)
{
  ...
  if (conn->linked) {
    ...
    if (event_del(conn->read_event))
      log_warn(LD_NET,get_lang_str(LANG_LOG_MAIN_LIBEVENT_ERROR),conn->s,tor_socket_strerror(tor_socket_errno(conn->s)));
  }
}
...
connection_start_reading(connection_t *conn)
{
  ...
  if (conn->linked) {
    ...
    if (event_add(conn->read_event, NULL))
      log_warn(LD_NET,get_lang_str(LANG_LOG_MAIN_LIBEVENT_ERROR_2),conn->s,tor_socket_strerror(tor_socket_errno(conn->s)));
  }
}
...
connection_stop_writing(connection_t *conn)
{
  ...
  if (conn->linked) {
    ...
    if (event_del(conn->write_event))
      log_warn(LD_NET,get_lang_str(LANG_LOG_MAIN_LIBEVENT_ERROR_3),conn->s,tor_socket_strerror(tor_socket_errno(conn->s)));
  }
}
...
connection_start_writing(connection_t *conn)
{
  ...
  if (conn->linked) {
    ...
    if (event_add(conn->write_event, NULL))
      log_warn(LD_NET,get_lang_str(LANG_LOG_MAIN_LIBEVENT_ERROR_4),conn->s,tor_socket_strerror(tor_socket_errno(conn->s)));
  }
}
...
conn_read_callback(int fd, short event, void *_conn)
{
  ...
  log_debug(LD_NET,get_lang_str(LANG_LOG_MAIN_READ_EVENT),conn->s);
...
#ifndef MS_WINDOWS
      log_warn(LD_BUG,get_lang_str(LANG_LOG_MAIN_READ_ERROR),conn_type_to_string(conn->type), conn->s);
      tor_fragile_assert();
#endif
...
conn_write_callback(int fd, short events, void *_conn)
{
  ...
  LOG_FN_CONN(conn,(LOG_DEBUG,LD_NET,get_lang_str(LANG_LOG_MAIN_WRITE_EVENT),conn->s));
...
//  assert_connection_ok(conn, time(NULL));
  ...
  if (connection_handle_write(conn, 0) < 0) {
    if (!conn->marked_for_close) {
      ...
      log_fn(LOG_WARN,LD_BUG,get_lang_str(LANG_LOG_MAIN_WRITE_ERROR),conn_type_to_string(conn->type), conn->s);
      tor_fragile_assert();
      if (CONN_IS_EDGE(conn)) {
...
//  assert_all_pending_dns_resolves_ok();
  ...
  log_debug(LD_NET,get_lang_str(LANG_LOG_MAIN_CONN_CLEANUP),conn->s);
  if ((conn->s >= 0 || conn->linked_conn) && connection_wants_to_flush(conn)) {
    ...
    if (!conn->hold_open_until_flushed)
      log_info(LD_NET,get_lang_str(LANG_LOG_MAIN_CONN_FLUSH),escaped_safe_str(conn->address),conn->s,conn_type_to_string(conn->type),conn->state,(int)conn->outbuf_flushlen,conn->marked_for_close_file,conn->marked_for_close);
    if (conn->linked_conn) {
    ...
    if (retval >= 0 && /* Technically, we could survive things like
      ...
      if (retval > 0) {
        LOG_FN_CONN(conn,(LOG_INFO,LD_NET,get_lang_str(LANG_LOG_MAIN_CONN_FLUSH_3),conn->s));
    ...
    if (connection_wants_to_flush(conn)) {
       ...
       * is meaningless. */
      log_fn(severity,LD_NET,get_lang_str(LANG_LOG_MAIN_WE_STALLED_TOO_MUCH),(int)buf_datalen(conn->outbuf),escaped_safe_str(conn->address),conn->s,conn_type_to_string(conn->type),conn->state,conn->marked_for_close_file,conn->marked_for_close);
    }
  }
...
directory_all_unreachable(time_t now)
{
  ...
  while ((conn = connection_get_by_type_state(CONN_TYPE_AP,
    ...
    edge_connection_t *edge_conn = TO_EDGE_CONN(conn);
    log_notice(LD_NET,get_lang_str(LANG_LOG_MAIN_NETDOWN),safe_str(edge_conn->socks_request->address),edge_conn->socks_request->port);
...
run_connection_housekeeping(int i, time_t now)
{
  ...
  if (conn->type == CONN_TYPE_DIR &&
    ...
    log_info(LD_DIR,get_lang_str(LANG_LOG_MAIN_DIR_CONN_EXPIRE),conn->s,conn->purpose);
  ...
  if (or_conn->is_bad_for_new_circs && !or_conn->n_circuits) {
     ...
     * mark it now. */
    log_info(LD_OR,get_lang_str(LANG_LOG_MAIN_EXPIRE_OLD_CONN),conn->s,conn->address,conn->port);
    ...
    log_info(LD_OR,get_lang_str(LANG_LOG_MAIN_EXPIRE_NON_OPEN_CONN),conn->s,conn->address, conn->port);
    connection_mark_for_close(conn);
    }
  } else if (we_are_hibernating() && !or_conn->n_circuits &&
             !buf_datalen(conn->outbuf)) {
    /* We're hibernating, there's no circuits, and nothing to flush.*/
    log_info(LD_OR,get_lang_str(LANG_LOG_MAIN_EXPIRE_NON_USED_CONN),conn->s,conn->address,conn->port);
    connection_mark_for_close(conn);
    conn->hold_open_until_flushed = 1;
  } else if (!or_conn->n_circuits &&
             now >= or_conn->timestamp_last_added_nonpadding +
                                         IDLE_OR_CONN_TIMEOUT) {
    log_info(LD_OR,get_lang_str(LANG_LOG_MAIN_EXPIRE_NON_USED_CONN_2),conn->s,conn->address,conn->port,(int)(now - or_conn->timestamp_last_added_nonpadding));
    connection_mark_for_close(conn);
    conn->hold_open_until_flushed = 1;
  } else if (
      now >= or_conn->timestamp_lastempty + options->KeepalivePeriod*10 &&
      now >= conn->timestamp_lastwritten + options->KeepalivePeriod*10) {
    log_fn(LOG_PROTOCOL_WARN,LD_PROTOCOL,get_lang_str(LANG_LOG_MAIN_EXPIRE_STUCK_CONN),conn->s,conn->address,conn->port,(int)buf_datalen(conn->outbuf),(int)(now-conn->timestamp_lastwritten));
    connection_mark_for_close(conn);
...
void signewnym_impl(time_t now)
{
  ...
  if(get_options()->IdentityFlags&IDENTITY_FLAG_EXPIRE_CIRCUITS)	circuit_expire_all_circuits();
  if(get_options()->IdentityFlags&IDENTITY_FLAG_EXPIRE_TRACKED_HOSTS)	addressmap_clear_transient();
...
run_scheduled_events(time_t now)
{
  ...
  if (signewnym_is_pending &&
      time_of_last_signewnym <= now) {
    log(LOG_INFO, LD_CONTROL,get_lang_str(LANG_LOG_MAIN_NEWNYM));
    signewnym_impl(now);
  }
  ...
  if (time_to_try_getting_descriptors < now) {
    ...
    update_extrainfo_downloads(get_time(NULL));
...
#define CHECK_V3_CERTIFICATE_INTERVAL (5*60)
   ...
   */
  if ((!(options->DirFlags&2))&&(time_to_check_for_expired_networkstatus < now)) {
...
#define NUM_JUMPED_SECONDS_BEFORE_WARN 100
  ...
  if (evtimer_add(timeout_event, &one_second))
    log_err(LD_NET,get_lang_str(LANG_LOG_EVENTDNS_TIMER_ERROR_3));
}
...
control_signal_act(int the_signal)
{
  switch (the_signal)
    ...
    case SIGCLEARDNSCACHE:
      ...
      break;
    default:
      log_warn(LD_BUG,get_lang_str(LANG_LOG_MAIN_UNRECOGNIZED_SIGNAL),the_signal);
      break;
    }
}
  ...
  switch (sig)
    ...
    case SIGNEWNYM: {
      ...
      if (time_of_last_signewnym + MAX_SIGNEWNYM_RATE > now) {
        ...
        log(LOG_NOTICE, LD_CONTROL,get_lang_str(LANG_LOG_MAIN_SIGNEWNYM),(int)(MAX_SIGNEWNYM_RATE+time_of_last_signewnym-now));
      } else {
        signewnym_impl(now);
      }
      break;
...
void dumpstats(int severity)
{
  ...
  {
    int i = conn_sl_idx;
    log(severity, LD_GENERAL,get_lang_str(LANG_LOG_MAIN_STATS_CONN),i,conn->s,conn->type,conn_type_to_string(conn->type),conn->state,conn_state_to_string(conn->type,conn->state),(int)(now - conn->timestamp_created));
    if (!connection_is_listener(conn)) {
      log(severity,LD_GENERAL,get_lang_str(LANG_LOG_MAIN_STATS_CONN_2),i,safe_str(conn->address),conn->port);
...
void __stdcall tor_thread(LPARAM);
DWORD thread_id;
HANDLE hThread;
HANDLE hDialog=NULL;
...
{	if((!options->DirFlags&16)||(!options->DirFlags&32)) return get_time(NULL);
	time_t oldtime=time(NULL);
	time_t result=newtime;
	if(best_delta_t==delta_t)
	{	update_best_delta_t(newtime-oldtime);
		options->BestTimeDelta=best_delta_t;
...
{	update_best_delta_t(best_delta_t-seconds);
	delta_t-=seconds;
	if(options) options->BestTimeDelta=best_delta_t;
}
...
get_version(void)
{
	if(!options) _version=tor_strdup("0.2.1.30");
	else if(options->torver)	_version=tor_strdup(options->torver);
	else if (_version == NULL)
		_version = tor_strdup(options->torver);
	return _version;
}
...
int tor_main(int argc, char *argv[])
{
	...
	fullpath[i]=0;SetCurrentDirectory(fullpath);
...
//	log_set_application_name(buf);
	...
	tmpOptions=options = get_options();
	dlgAuthorities_initDirServers(&options->DirServers);
	if(options->Language) load_lng(options->Language);
	...
	if(options->DirFlags&16)
	{	update_best_delta_t(options->BestTimeDelta);
		delta_t=best_delta_t;
		if((options->BestTimeDelta)&&(options->DirFlags&32)) delta_t=options->BestTimeDelta;
		else delta_t=crypto_rand_int(options->MaxTimeDelta*2)-options->MaxTimeDelta;
...
void __stdcall tor_thread(LPARAM lParam)
{
	int	loop_result;
	time_t	now;
	char	*msg;
	...
	if((! identity_key_is_set())&&(init_keys() < 0))
	...
	if(trusted_dirs_reload_certs())    tor_end();
	...
	second_elapsed_callback(0,0,NULL);	/* set up once-a-second callback. */
