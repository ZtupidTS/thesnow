...
rep_hist_load_mtbf_data(time_t now)
{
    ...
    if (format == 1) {
      ...
      if (mtbf_idx >= 0) {
        ...
        if (n == 2 || n == 4) {
          ...
          log_warn(LD_HIST,get_lang_str(LANG_LOG_REPHIST_COULD_NOT_SCAN_LINE_2),escaped(mtbfline));
        }
      }
      if (wfu_idx >= 0) {
        ...
        if (n == 2 || n == 4) {
          ...
          log_warn(LD_HIST,get_lang_str(LANG_LOG_REPHIST_COULD_NOT_SCAN_LINE_3),escaped(wfuline));
        }
      }
      if (wfu_idx > i)
        i = wfu_idx;
    ...
    if (have_mtbf) {
      if (mtbf_timebuf[0]) {
        ...
        if (parse_possibly_bad_iso_time(mtbf_timebuf, &start_of_run)<0)
          log_warn(LD_HIST,get_lang_str(LANG_LOG_REPHIST_MTBF_FORMAT_ERROR_5),escaped(mtbf_timebuf));
      }
    ...
    if (have_wfu) {
      if (wfu_timebuf[0]) {
        ...
        if (parse_possibly_bad_iso_time(wfu_timebuf, &start_of_downtime)<0)
          log_warn(LD_HIST,get_lang_str(LANG_LOG_REPHIST_MTBF_FORMAT_ERROR_5),escaped(wfu_timebuf));
      }
    }
  ...
  if (strcmp(line, "."))
    log_warn(LD_HIST,get_lang_str(LANG_LOG_REPHIST_MTBF_TRUNCATED));
...
rep_hist_update_state(or_state_t *state)
{
  ...
  tor_free(buf);
  if (server_mode(get_options())) {
    or_state_mark_dirty(get_or_state(), get_time(NULL)+(2*3600));
  }
}
  ...
  for (r=0;r<2;++r) {
    ...
    if (s_values && s_begins >= now - NUM_SECS_BW_SUM_INTERVAL*NUM_TOTALS) {
      ...
      SMARTLIST_FOREACH(s_values, char *, cp, {
        ...
        if (!ok) {
          all_ok=0;
          log_notice(LD_HIST,get_lang_str(LANG_LOG_REPHIST_PARSE_INT_ERROR),cp);
        }
        if (start < now) {
          add_obs(b, start, v);
...
hs_usage_write_statistics_to_file(time_t now)
{
  ...
  fname = get_datadir_fname("hsusage");
