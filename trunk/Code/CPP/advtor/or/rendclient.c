                              ...
                              origin_circuit_t *rendcirc)
{
  ...
  if (rend_cache_lookup_entry(introcirc->rend_data->onion_address, -1,
                              &entry) < 1) {
    log_warn(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_QUERY_WITHOUT_VALID_REND_DESC),escaped_safe_str(introcirc->rend_data->onion_address));
    goto err;
  }
  ...
  if (entry->parsed->version == 0) { /* un-versioned descriptor */
    ...
    if (!intro_key) {
      ...
      if (rend_cache_lookup_entry(introcirc->rend_data->onion_address,
          0, &entry) > 0) {
        log_info(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_BOTH_V0_AND_V2));
        /* See flyspray task 1024. */
        intro_key = entry->parsed->pk;
      } else {
        log_info(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_INTERNAL_ERROR_2),num_intro_points);
        goto err;
      }
    }
  }
  ...
  r = crypto_pk_public_hybrid_encrypt(intro_key, payload+DIGEST_LEN,sizeof(payload)-DIGEST_LEN,
                                      tmp,
                               ...
                               const uint8_t *request, size_t request_len)
{
  ...
  if (request_len == 0) {
    ...
    if (rend_client_remove_intro_point(circ->build_state->chosen_exit,
      ...
      if (!extend_info) {
        log_warn(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_NO_INTRO_POINTS_LEFT),escaped_safe_str(circ->rend_data->onion_address));
        circuit_mark_for_close(TO_CIRCUIT(circ), END_CIRC_REASON_INTERNAL);
        return -1;
      }
      if (circ->remaining_relay_early_cells) {
        log_info(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_RE_EXTENDING_CIRC),escaped_safe_str(circ->rend_data->onion_address),circ->build_state->chosen_exit->nickname,circ->_base.n_circ_id, extend_info->nickname);
        result = circuit_extend_to_new_exit(circ, extend_info);
      } else {
        log_info(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_BUILDING_NEW_INTRO_CIRC),escaped_safe_str(circ->rend_data->onion_address),circ->build_state->chosen_exit->nickname,extend_info->nickname);
        circuit_mark_for_close(TO_CIRCUIT(circ), END_CIRC_REASON_FINISHED);
        if (!circuit_launch_by_extend_info(CIRCUIT_PURPOSE_C_INTRODUCING,
                                           extend_info,
                                           CIRCLAUNCH_IS_INTERNAL,1)) {
          log_warn(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_BUILDING_INTRO_CIRC_FAILED));
          result = -1;
        } else {
          result = 0;
        }
...
rend_client_refetch_renddesc(const char *query)
{
  if (!get_options()->FetchHidServDescriptors)
    return;
  log_info(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_FETCHING_REND_DESC),escaped_safe_str(query));
  if (connection_get_by_type_state_rendquery(CONN_TYPE_DIR, 0, query, 0)) {
    log_info(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_FETCHING_IN_PROGRESS),escaped_safe_str(query));
  } else {
                               ...
                               const rend_data_t *rend_query)
{
  ...
  log_info(LD_REND,get_lang_str(LANG_LOG_RENDCLIENT_OPTIONS),smartlist_len(ent->parsed->intro_nodes),escaped_safe_str(rend_query->onion_address));
  return 1;
}
...
rend_client_get_random_intro(const rend_data_t *rend_query)
{
 ...
 again:
  ...
  if (!intro->extend_info->onion_key) {
    ...
    if (!router || (options->_ExcludeExitNodesUnion && routerset_contains_router(options->_ExcludeExitNodesUnion,router))) {
