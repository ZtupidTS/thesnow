...
void CGameListCtrl::BrowseForDirectory()
{
	...
	// browse
	wxDirDialog dialog(this, _("浏览需要添加的目录"), dirHome, wxDD_DEFAULT_STYLE | wxDD_DIR_MUST_EXIST);
...
void CGameListCtrl::Update()
{
	...
	else
	{
		...
		{
			errorString = _("Dolphin 当前设置为隐藏所有游戏. 双击这里显示所有游戏...");
		}
...
void CGameListCtrl::CompressSelection(bool _compress)
{
	...
	wxDirDialog browseDialog(this, _("浏览输出目录"), dirHome, wxDD_DEFAULT_STYLE | wxDD_DIR_MUST_EXIST);
	if (browseDialog.ShowModal() != wxID_OK)
		return;

	wxProgressDialog progressDialog(_compress ? _("压缩 ISO 中") : _("解压缩 ISO 中"),
		_("工作中..."),
		1000, // range
		this, // parent
		wxPD_APP_MODAL |
...
void CGameListCtrl::OnCompressGCM(wxCommandEvent& WXUNUSED (event)) 
{
	...
	if (iso->IsCompressed())
	{
		 path = wxFileSelector(
			...
			wxString::Format
			(
			_T("所有 GC/Wii ISO 文件 (gcm)|*.gcm|所有文件 (%s)|%s"),
	...
	else
	{
		path = wxFileSelector(
			...
			wxString::Format
			(
			_T("所有压缩的 GC/Wii ISO 文件 (gcz)|*.gcz|所有文件 (%s)|%s"),
			wxFileSelectorDefaultWildcardStr,
			wxFileSelectorDefaultWildcardStr
			),
			wxFD_SAVE,
	...
	wxProgressDialog dialog(iso->IsCompressed() ? _T("解压缩 ISO 中") : _T("压缩 ISO 中"),
		_T("工作中..."),
		1000, // range
		this, // parent
		wxPD_APP_MODAL |
