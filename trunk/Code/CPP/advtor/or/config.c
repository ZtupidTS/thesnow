...
static config_var_t _option_vars[] = {
...
#endif
  VAR("DirServers",              LINELIST, DirServers, NULL),
  V(DNSPort,                     UINT,     "0"),
  ...
  V(FallbackNetworkstatusFile,   FILENAME,  "AdvTor-fallback-consensus"),
  ...
  VAR("__HashedControlSessionPassword", LINELIST, HashedControlSessionPassword,
      NULL),
  V(MaxUnusedOpenCircuits,              UINT,     "12"),
  V(MinUptimeHidServDirectoryV2, INTERVAL, "24 hours"),
  V(AutoStart,              UINT,     "0"),
  V(logging,              UINT,     "16390"),
  V(ForceFlags,              UINT,     "11"),
  V(DirFlags,              UINT,     "49"),		//1 | 8 | 16 | 32
  V(IdentityFlags,              UINT,     "7"),
  V(winver,                        STRING,   NULL),
  V(torver,                        STRING,   "0.2.1.29"),
...
static config_var_description_t options_description[] = {
  ...
  { "Plugins", "Plugins that can be used by Advanced TOR must be placed in %[exename]-plugins\\ directory. For more information about writing plugins, see plugins.txt." },
...
//    return -1;
  ...
  if(options->SelectedTorVer==NULL)	options->SelectedTorVer="<< Auto >>";
...
//    return -1;
  ...
  if (old_options) {
    ...
    if (! bool_eq(options->BridgeRelay, old_options->BridgeRelay)) {
      ...
      geoip_remove_old_clients(get_time(NULL)+(2*60*60));
    }
    ...
    if (options_transition_affects_workers(old_options, options)) {
      ...
      if (server_mode(options) && !server_mode(old_options)) {
        if (init_keys() < 0) {
          log_warn(LD_BUG,get_lang_str(LANG_LOG_CONFIG_ERROR_INITIALIZING_KEYS));
//          return -1;
        }
        ip_address_changed(0);
        if (has_completed_circuit || !any_predicted_circuits(get_time(NULL)))
...
//        return -1;
  ...
  if (options->Nickname == NULL) {
      if (!(options->Nickname = get_default_nickname())) {
        log_notice(LD_CONFIG,get_lang_str(LANG_LOG_CONFIG_DEFAULT_NICKNAME_ERROR), UNNAMED_ROUTER_NICKNAME);
                    ...
                    config_line_t *c, char **msg)
{
  ...
  case CONFIG_TYPE_UINT:
    ...
    else i = (int)tor_parse_long(c->value, 10, 0, INT_MAX, &ok, NULL);
    if (!ok) {
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_INVALID_INT),c->key, c->value);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
    *(int *)lvalue = i;
    break;
  ...
  case CONFIG_TYPE_INTERVAL: {
    ...
    if (!ok) {
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_INVALID_INTERVAL),c->key,c->value);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
    *(int *)lvalue = i;
  ...
  case CONFIG_TYPE_MEMUNIT: {
    ...
    if (!ok) {
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_INVALID_VALUE),c->key,c->value);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
    *(uint64_t *)lvalue = u64;
  ...
  case CONFIG_TYPE_BOOL:
    ...
    if (!ok) {
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_INVALID_BOOLEAN),c->key,c->value);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
    *(int *)lvalue = i;
  ...
  case CONFIG_TYPE_ISOTIME:
    if (parse_iso_time(c->value, (time_t *)lvalue)) {
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_INVALID_KEYWORD_TYPE),c->value,c->key);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
    break;
  ...
  case CONFIG_TYPE_ROUTERSET:
    ...
    *(routerset_t**)lvalue = routerset_new();
    if (routerset_parse(*(routerset_t**)lvalue, c->value, c->key)<0) {
      tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_INVALID_EXIT_LIST),c->value,c->key);
      *msg = tor_strdup(buf);
      return -1;
    }
    break;
  ...
  case CONFIG_TYPE_LINELIST_V:
    r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_VIRTUAL_OPTION_VALUE),c->key);
    *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
    return -1;
  default:
    tor_assert(0);
                   ...
                   int clear_first, char **msg)
{
  ...
  if (!var) {
    if (fmt->extra) {
      ...
      char buf[1024];
      int tmp = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_UNKNOWN_OPTION),c->key);
      *msg = tor_strdup(tmp >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return 0;
    }
  }
...
config_assign(config_format_t *fmt, void *options, config_line_t *list,
              int use_defaults, int clear_first, char **msg,char *section)
{
  config_line_t *p;
  int i;
  int r=0;
  ...
  while (list) {
    ...
    if(list->key[0]!='[') r=config_assign_line(fmt, options, list, use_defaults,clear_first, msg);
    list = list->next;
  }
  return r;
}
...
validate_ports_csv(smartlist_t *sl, const char *name, char **msg)
{
  ...
  SMARTLIST_FOREACH(sl, const char *, cp,
  {
    i = atoi(cp);
    if (i < 1 || i > 65535) {
      int r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_INVALID_PORT),cp,name);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
  });
...
#define COMPLAIN(arg) STMT_BEGIN log(LOG_WARN, LD_CONFIG, arg); STMT_END
  ...
  *msg = NULL;
  ...
  if (options->ORPort < 0 || options->ORPort > 65535)
    REJECT(get_lang_str(LANG_LOG_CONFIG_ORPORT_OUT_OF_BOUNDS));
...
/*  if (options->ORPort == 0 && options->ORListenAddress != NULL)
  ...
  if (options->Nickname == NULL) {
    ...
    if (!is_legal_nickname(options->Nickname)) {
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_INVALID_NICKNAME),options->Nickname);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
  }
  ...
  if (options->SocksPort < 0 || options->SocksPort > 65535)
    REJECT(get_lang_str(LANG_LOG_CONFIG_SOCKSPORT_OUT_OF_BOUNDS));

  if (options->DNSPort < 0 || options->DNSPort > 65535)
    REJECT(get_lang_str(LANG_LOG_CONFIG_DNSPORT_OUT_OF_BOUNDS));

  if (options->TransPort < 0 || options->TransPort > 65535)
    REJECT(get_lang_str(LANG_LOG_CONFIG_TRANSPORT_OUT_OF_BOUNDS));

  if (options->NatdPort < 0 || options->NatdPort > 65535)
    REJECT(get_lang_str(LANG_LOG_CONFIG_NATDPORT_OUT_OF_BOUNDS));
  ...
  if (options->ControlPort < 0 || options->ControlPort > 65535)
    REJECT(get_lang_str(LANG_LOG_CONFIG_CONTROLPORT_OUT_OF_BOUNDS));

  if (options->DirPort < 0 || options->DirPort > 65535)
    REJECT(get_lang_str(LANG_LOG_CONFIG_DIRPORT_OUT_OF_BOUNDS));
  ...
  if (options->HSAuthorityRecordStats && !options->HSAuthoritativeDir)
    REJECT(get_lang_str(LANG_LOG_CONFIG_HSAUTHORITYRECORDSTATS_WITHOUT_HS_AUTH));
  ...
  if (compute_publishserverdescriptor(options) < 0) {
    r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_PUBLISHSERVERDESCRIPTOR_INVALID));
    *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
    return -1;
  }
...
*/
  ...
  if (server_mode(options)) {
    if (options->BandwidthRate < ROUTER_REQUIRED_MIN_BANDWIDTH*2) {
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_BANDWIDTHRATE_TOO_LOW),(int)options->BandwidthRate,ROUTER_REQUIRED_MIN_BANDWIDTH*2);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    } else if (options->MaxAdvertisedBandwidth <
               ROUTER_REQUIRED_MIN_BANDWIDTH) {
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_MAXADVERTISEDBANDWIDTH_TOO_LOW),(int)options->MaxAdvertisedBandwidth,ROUTER_REQUIRED_MIN_BANDWIDTH);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
    if (options->RelayBandwidthRate &&
      options->RelayBandwidthRate < ROUTER_REQUIRED_MIN_BANDWIDTH) {
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_RELAYBANDWIDTHRATE_TOO_LOW),(int)options->RelayBandwidthRate,ROUTER_REQUIRED_MIN_BANDWIDTH);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
  }
...
//    REJECT(get_lang_str(LANG_LOG_CONFIG_USEBRIDGES_WITHOUT_TUNNELDIRCONS));
  ...
  if (options->ConstrainedSockets) {
    ...
    if (options->ConstrainedSockSize < MIN_CONSTRAINED_TCP_BUFFER ||
      ...
      r = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_CONSTRAINEDSOCKSIZE_INVALID),MIN_CONSTRAINED_TCP_BUFFER, MAX_CONSTRAINED_TCP_BUFFER);
      *msg = tor_strdup(r >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
      return -1;
    }
    if (options->DirPort) {
  ...
  if (options->PreferTunneledDirConns && !options->TunnelDirConns)
    REJECT(get_lang_str(LANG_LOG_CONFIG_PREFERTUNNELDIRCONNS_WITHOUT_TUNNELDIRCONNS));
  ...
  SMARTLIST_FOREACH(sl, const char *, s,
    {
      if (!is_legal_nickname_or_hexdigest(s)) {
        char buf[1024];
        int tmp = tor_snprintf(buf, sizeof(buf),get_lang_str(LANG_LOG_CONFIG_INVALID_NICKNAME_IN_LINE),s,name);
        *msg = tor_strdup(tmp >= 0 ? buf : get_lang_str(LANG_LOG_CONFIG_INTERNAL_ERROR));
        r = -1;
        break;
      }
                         ...
                         char **msg)
{
  ...
  if (!badstate && or_state_validate(NULL, new_state, 1, &msgerr) < 0)
    badstate = 1;
  if (msgerr) {
  ...
  retval = config_assign(&options_format, newoptions,
                         global_cmdline_options, 0, 0, msg,NULL);
  if (retval < 0) {
    err = SETOPT_ERR_PARSE;
    goto err;
  }
...
//  config_free(&options_format, newoptions);
  if (*msg) {
    ...
    tor_snprintf(newmsg,len,get_lang_str(LANG_LOG_CONFIG_CONFIG_PARSE_FAILED),*msg);
    tor_free(*msg);
    *msg = newmsg;
  }
  if (new_state)
...
int parse_dir_server_line(const char *line, authority_type_t required_type,
                      int validate_only)
{
  smartlist_t *items = NULL;
  int r;
  ...
  while (smartlist_len(items)) {
    ...
    if (!strcasecmp(flag, "v1")) {
      ...
      if (!ok)
        log_warn(LD_CONFIG,get_lang_str(LANG_LOG_CONFIG_DIRSERVER_INVALID_ORPORT),portstring);
      ...
      log_warn(LD_CONFIG,get_lang_str(LANG_LOG_CONFIG_DIRSERVER_UNRECOGNIZED_FLAG),flag);
    }
    tor_free(flag);
    smartlist_del_keeporder(items, 0);
  }
...
#define GENERATED_FILE_PREFIX "; This file was generated by AdvTor; " \
...
write_configuration_file(const char *fname, or_options_t *options)
{
  ...
  if (accounting_is_enabled(get_options()))
    accounting_run_housekeeping(get_time(NULL));
  ...
  len = strlen(new_conf)+strlen(state)+256;
  new_val = tor_malloc(len);
  tor_snprintf(new_val, len, "%s\n\n%s\n\n%s\n\n[state]\n\n%s",
               GENERATED_FILE_PREFIX, "[torrc]", new_conf,state);
...
init_libevent(void)
{
   ...
   */
  suppress_libevent_log_msg(get_lang_str(LANG_LOG_CONFIG_LIBEVENT_FN_NOT_IMPLEMENTED));
#ifdef __APPLE__
  ...
  if (strcmp(_EVENT_VERSION, event_get_version())) {
    ...
    log(verybad ? LOG_WARN : (prettybad ? LOG_NOTICE : LOG_INFO),LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_LIBEVENT_VERSION_MISMATCH),_EVENT_VERSION,event_get_version());
    if (verybad)
      log_warn(LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_CRASH_WARN_1));
    else if (prettybad)
      log_notice(LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_CRASH_WARN_2));
    else
      log_info(LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_CRASH_WARN_3));
  }
  ...
  */
   ...
   * 1.4.0-beta, we are incompatible. */
  {
    if (strcmp(event_get_version(), "1.4.0-beta")) {
      log_warn(LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_LIBEVENT_HEADERS_MISMATCH_1),event_get_version());
    }
  }
#else
     ...
     later, we're probably fine. */
  {
    ...
    if ((v[0] == '1' && v[2] == '.' && v[3] > '3') || v[0] > '1') {
      log_warn(LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_LIBEVENT_HEADERS_MISMATCH_2),event_get_version());
    }
  }
#endif
...
#warn "_EVENT_VERSION is defined but not get_event_version(): Libevent is odd."
   ...
   * or methods better. */
  log(LOG_NOTICE, LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_LIBEVENT_INIT_OK),event_get_version(), event_get_method());
  check_libevent_version(event_get_method(), get_options()->ORPort != 0);
#else
  log(LOG_NOTICE, LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_LIBEVENT_TOO_OLD));
#endif
}
...
check_libevent_version(const char *m, int server)
{
  ...
  if (thread_unsafe) {
    log(LOG_WARN, LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_LIBEVENT_UNSAFE),v,sad_os);
    badness = "BROKEN";
  } else if (buggy) {
    log(LOG_WARN, LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_LIBEVENT_BUGGY_1),m,v);
    badness = "BROKEN";
  } else if (iffy) {
    log(LOG_WARN, LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_LIBEVENT_BUGGY_2),m,v);
    badness = "BUGGY";
  } else if (slow && server) {
    log(LOG_WARN, LD_GENERAL,get_lang_str(LANG_LOG_CONFIG_LIBEVENT_TOO_SLOW),v,m);
    badness = "SLOW";
  }
  if (badness) {
                                  ...
                                  const char *suffix)
{
  ...
  if (sub1) {
    if (sub2) {
      tor_snprintf(fname, len, "%s-%s-%s",
                   fullpath,sub1, sub2);
    } else {
      tor_snprintf(fname, len, "%s-%s",fullpath,sub1);
    }
  } else {
    tor_snprintf(fname, len, "%s.",fullpath);
  }
  if (suffix)
...
//#define VERY_OLD_FILE_AGE (28*24*60*60)
  ...
  if (stat(fname, &st)==0 && update_time(st.st_mtime) < now-(options->MaxFileAge*24*60*60)) {
    char buf[ISO_TIME_LEN+1];
    format_local_iso_time(buf, update_time(st.st_mtime));
