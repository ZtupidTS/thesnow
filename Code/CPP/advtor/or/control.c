    ...
    if (smartlist_len(elts) == 2) {
      ...
      if (address_is_invalid_destination(to, 1)) {
        ...
        if (!address) {
          ...
          smartlist_add(reply, ans);
          log_warn(LD_CONTROL,get_lang_str(LANG_LOG_CONTROL_MAPADDRESS_ALLOCATION_ERROR),safe_str(line));
        } else {
      ...
      smartlist_add(reply, ans);
      log_info(LD_CONTROL,get_lang_str(LANG_LOG_CONTROL_MAPADDRESS_WRONG_NUMBER_OF_ITEMS),safe_str(line));
    }
  ...
  if (!strcmp(question, "circuit-status")) {
    ...
    *answer = smartlist_join_strings(status, "\r\n", 0, NULL);
    ...
    if (!strcmpstart(question, "addr-mappings/")) {
      ...
      log_warn(LD_CONTROL,get_lang_str(LANG_LOG_CONTROL_OBSOLETE_ADDR_MAPPING));
    }
    ...
    *answer = smartlist_join_strings(mappings, "\r\n", 0, NULL);
    ...
    if (!strcmp(question, "status/circuit-established")) {
      ...
      *answer = tor_strdup(last_sent_bootstrap_message);
      ...
      size_t answer_len;
      char *geoip_summary = extrainfo_get_client_geoip_summary(get_time(NULL));
