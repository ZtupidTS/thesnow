                                         ...
                                         const char *keyfile)
{
  ...
  if (!contents) {
    log_warn(LD_CRYPTO,get_lang_str(LANG_LOG_CRYPTO_ERROR_READING_PRIVATE_KEY),keyfile);
    return -1;
  }
...
tor_check_dh_key(BIGNUM *bn)
{
  ...
  BN_set_word(x, 1);
  if (BN_cmp(bn,x)<=0) {
    log_warn(LD_CRYPTO,get_lang_str(LANG_LOG_CRYPTO_INVALID_DH_KEY_3));
    goto err;
  }
  BN_copy(x,dh_param_p);
  BN_sub_word(x, 1);
  if (BN_cmp(bn,x)>=0) {
    log_warn(LD_CRYPTO,get_lang_str(LANG_LOG_CRYPTO_INVALID_DH_KEY_4));
    goto err;
  }
  BN_free(x);
  return 0;
 err:
  BN_free(x);
  s = BN_bn2hex(bn);
  log_warn(LD_CRYPTO,get_lang_str(LANG_LOG_CRYPTO_INSECURE_DH_KEY),s);
  OPENSSL_free(s);
  return -1;
}
                         ...
                         char *secret_out, size_t secret_bytes_out)
{
  ...
  if (tor_check_dh_key(pubkey_bn)<0) {
    ...
    log_warn(LD_CRYPTO,get_lang_str(LANG_LOG_CRYPTO_INVALID_KEY));
    goto error;
  }
