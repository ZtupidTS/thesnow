...
const char *circuit_purpose_to_string(uint8_t purpose)
{
  ...
  switch (purpose) {
    case CIRCUIT_PURPOSE_OR:
      return "Server: OR";
    case CIRCUIT_PURPOSE_INTRO_POINT:
      return "Server: intro_point";
    case CIRCUIT_PURPOSE_REND_POINT_WAITING:
      return "Server: intro_point_wait";
    case CIRCUIT_PURPOSE_REND_ESTABLISHED:
      return "Server: intro_point_established";

    case CIRCUIT_PURPOSE_C_GENERAL:
      return "General";
    case CIRCUIT_PURPOSE_C_INTRODUCING:
      return "General: introducing";
    case CIRCUIT_PURPOSE_C_INTRODUCE_ACK_WAIT:
      return "General: introduce_ack_wait";
    case CIRCUIT_PURPOSE_C_INTRODUCE_ACKED:
      return "General: HS_introduce_acked";

    case CIRCUIT_PURPOSE_C_ESTABLISH_REND:
      return "HS: establish_rend";
    case CIRCUIT_PURPOSE_C_REND_READY:
      return "HS: rend_ready";
    case CIRCUIT_PURPOSE_C_REND_READY_INTRO_ACKED:
      return "HS: rend_ready_intro_acked";
    case CIRCUIT_PURPOSE_C_REND_JOINED:
      return "HS: rend_joined";

    case CIRCUIT_PURPOSE_S_ESTABLISH_INTRO:
      return "HS_SERVICE: establish_intro";
    case CIRCUIT_PURPOSE_S_INTRO:
      return "HS_SERVICE: intro";

    case CIRCUIT_PURPOSE_S_CONNECT_REND:
      return "HS_SERVICE: connect_rend";
    case CIRCUIT_PURPOSE_S_REND_JOINED:
      return "HS_SERVICE: rend_joined";
    ...
    default:
      tor_snprintf(buf, sizeof(buf), "Unknown: %d", (int)purpose);
      return buf;
  }
}
...
{
  circ->timestamp_created = get_time(NULL);
...
{
  log(severity, LD_CIRC,get_lang_str(LANG_LOG_CIRCUITLIST_DUMP_DETAILS),conn_array_index, type, this_circid, other_circid, circ->state,circuit_state_to_string(circ->state), (int)circ->timestamp_created);
...
{	time_t most_recent1=0,most_recent2=0,most_recent3=0;
	...
	{	if (c->type != CONN_TYPE_AP) continue;
		...
		if(circ && circ->purpose==CIRCUIT_PURPOSE_C_GENERAL && (conn->cpath_layer))
...
{	char *s=tor_malloc_zero(8192);
	...
	circid_t n_circ_id = circ->n_circ_id, p_circ_id = 0;
	if(!circ->marked_for_close)
	{	format_rfc1123_time(s2,circ->timestamp_created);
		if(!CIRCUIT_IS_ORIGIN(circ))
		{	p_circ_id = TO_OR_CIRCUIT(circ)->p_circ_id;
