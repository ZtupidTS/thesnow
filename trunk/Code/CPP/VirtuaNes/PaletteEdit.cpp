...
//	int	nPos = (int) HIWORD(wParam);		// scroll box position 
	...
	if( nEdit ) {
		CHAR	szStr[16];
		::wsprintfA( szStr, "%d", nPos );
		::SetWindowTextA( ::GetDlgItem( m_hWnd, nEdit ), szStr );
	}
...
//	DEBUGOUT( "CPaletteEdit::OnRGBChange\n" );
	...
	::GetWindowTextA( hWnd, szColor, sizeof(szColor)-1 );
	if( ::lstrlenA(szColor) > 0 ) {
		val = ::atoi(szColor);
		if( val < 0 ) {
			val = 0;
...
//	DEBUGOUT( "CPaletteEdit::OnLoad\n" );
	...
	ofn.lpstrInitialDir =(LPTSTR) CApp::GetModulePath();
	szFile[0] = '\0';
...
//	ofn.lpstrDefExt = ".pal";
	...
	ofn.lpstrFilter = L"Palette File(*.pal)\0*.pal\0All File(*.*)\0*.*\0";
	ofn.nFilterIndex = 1;
...
//	DEBUGOUT( "CPaletteEdit::OnSave\n" );
	...
	ofn.lpstrDefExt = L".pal";

	WCHAR	szTitle[256];
	CApp::LoadString( IDS_UI_SAVEPALETTE, (LPTSTR)szTitle, sizeof(szTitle) );
	ofn.lpstrTitle = szTitle;
	...
	if( ::GetSaveFileName( &ofn ) ) {
		...
		if( (fp = ::_wfopen( szFile, L"wb" )) ) {
			for( int i = 0; i < 64; i++ ) {
				::fputwc( (int)m_Palette[i].rgbRed, fp );
				::fputwc( (int)m_Palette[i].rgbGreen, fp );
				::fputwc( (int)m_Palette[i].rgbBlue, fp );
			}
			::fclose( fp );
		}
	}
...
//	DEBUGOUT( "CPaletteEdit::OnChaneSelect\n" );
	...
	CHAR	szStr[16];
	::wsprintfA( szStr, "%02X", m_PaletteSelect );
	::SetDlgItemTextA( m_hWnd, IDC_PAL_NO, szStr );

	::wsprintfA( szStr, "%d", m_RGB.rgbRed );
	::SetDlgItemTextA( m_hWnd, IDC_PAL_R_EDIT, szStr );
	::wsprintfA( szStr, "%d", m_RGB.rgbGreen );
	::SetDlgItemTextA( m_hWnd, IDC_PAL_G_EDIT, szStr );
	::wsprintfA( szStr, "%d", m_RGB.rgbBlue );
	::SetDlgItemTextA( m_hWnd, IDC_PAL_B_EDIT, szStr );
