...
Dialogs::AboutBoxDialog::AboutBoxDialog( wxWindow* parent )
	: wxDialogWithHelpers( parent, AddAppName(_("关于 %s")), pxDialogFlags().Resize().MinWidth( 460 ) )
	...
	)
{
	...
	aboutUs		+= Heading(_("开发者")).Bold()	| StdExpand();
	aboutUs		+= label_auth						| StdExpand();
	contribs	+= Heading(_("赞助者")).Bold()	| StdExpand();
	...
	*this	+= new wxButton( this, wxID_OK, _("我知道了"))	| StdCenter();
