...
int GetPadString(wchar_t *string, unsigned int port, unsigned int slot) {
	if (!slot && !config.multitap[port]) {
		wsprintfW(string, L"手柄 %i", port+1);
	}
	else {
		wsprintfW(string, L"设备 %i%c", port+1, 'A'+slot);
...
void UpdatePadList(HWND hWnd) {
	...
	int port;
	int index = 0;
	wchar_t *padTypes[] = {L"未插入", L"双震 2", L"吉他"};
...
INT_PTR CALLBACK GeneralDialogProc(HWND hWnd, unsigned int msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
		{
			...
			if (SendMessage(hWndCombo, CB_GETCOUNT, 0, 0) == 0) {
				...
				SendMessage(hWndList, LVM_SETEXTENDEDLISTVIEWSTYLE, 0, LVS_EX_FULLROWSELECT);
				SendMessage(hWndCombo, CB_ADDSTRING, 0, (LPARAM) L"未插入");
				SendMessage(hWndCombo, CB_ADDSTRING, 0, (LPARAM) L"双震 2");
				SendMessage(hWndCombo, CB_ADDSTRING, 0, (LPARAM) L"吉他");
			}
		}
		UpdatePadPages();
		hWndGeneral = hWnd;
	...
	case WM_COMMAND:
		if (LOWORD(wParam) == IDC_PAD_TYPE) {
			...
			ofn.hwndOwner = hWnd;
			ofn.lpstrFilter = L"LilyPad 设置文件\0*.lily\0所有文件\0*.*\0\0";
			...
			if (LOWORD(wParam) == ID_LOAD) {
				ofn.lpstrTitle = L"载入 LilyPad 设置";
				...
				ofn.lpstrTitle = L"保存 LilyPad 设置";
				ofn.Flags |= OFN_OVERWRITEPROMPT;
				if (GetSaveFileNameW(&ofn)) {
					if (SaveSettings(ofn.lpstrFile) == -1) {
						MessageBox(hWnd, L"保存失败", L"不能保存文件.", MB_OK);
					}
				}
			}
			break;
