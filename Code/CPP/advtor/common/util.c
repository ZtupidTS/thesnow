...
check_private_dir(const char *dirname, cpd_check_t check)
{
  ...
  if (r) {
    if (errno != ENOENT) {
      log(LOG_WARN,LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_READING_DIR),dirname,strerror(errno));
      return -1;
    }
    if (check == CPD_NONE) {
      log(LOG_WARN,LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_READING_DIR_2),dirname);
      return -1;
    } else if (check == CPD_CREATE) {
      log_info(LD_GENERAL,get_lang_str(LANG_LOG_UTIL_CREATING_DIR),dirname);
      /*
#ifdef MS_WINDOWS
      r = mkdir(dirname);
#else
...
#endif
      if (r) {
        log(LOG_WARN,LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_CREATING_DIR),dirname,strerror(errno));
        return -1;
      }*/
    }
  ...
  if (!(st.st_mode & S_IFDIR)) {
    log(LOG_WARN,LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_READING_DIR_3),dirname);
    return -1;
  }
  return 0;
}
...
#ifdef MS_WINDOWS
  if (!bin && strchr(str, '\r')) {
    log_warn(LD_BUG,get_lang_str(LANG_LOG_UTIL_DUPLICATE_CR));
  }
#endif
                      ...
                      open_file_t **data_out)
{
  ...
  if (open_flags & O_APPEND) {
    ...
    if (tor_snprintf(new_file->tempname, tempname_len, "%s.tmp", fname)<0) {
      log(LOG_WARN,LD_GENERAL,get_lang_str(LANG_LOG_UTIL_ERROR_CREATING_FILE));
      goto err;
    }
  ...
  if ((new_file->fd = open(open_name, open_flags, mode)) < 0) {
    log(LOG_WARN,LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_OPENING_FILE),open_name,fname,strerror(errno));
    goto err;
  }
  if (append) {
    if (tor_fd_seekend(new_file->fd) < 0) {
      log_warn(LD_FS,get_lang_str(LANG_LOG_UTIL_SEEK_ERROR),open_name,strerror(errno));
      goto err;
    }
  }
...
fdopen_file(open_file_t *file_data)
{
  ...
  if (!(file_data->stdio_file = fdopen(file_data->fd, "a"))) {
    log_warn(LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_OPENING_FILE_2),file_data->filename,file_data->fd,strerror(errno));
  }
  return file_data->stdio_file;
}
...
finish_writing_to_file_impl(open_file_t *file_data, int abort_write)
{
  ...
  if (file_data->stdio_file) {
    if (fclose(file_data->stdio_file)) {
      log_warn(LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_CLOSING_FILE),file_data->filename,strerror(errno));
      abort_write = r = -1;
    }
  } else if (file_data->fd >= 0 && close(file_data->fd) < 0) {
    log_warn(LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_FLUSHING_FILE),file_data->filename,strerror(errno));
    abort_write = r = -1;
  }
                          ...
                          int open_flags)
{
  ...
  SMARTLIST_FOREACH(chunks, sized_chunk_t *, chunk,
  {
    ...
    if (result < 0) {
      log(LOG_WARN,LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_WRITING_FILE),fname,strerror(errno));
      goto err;
    }
...
read_file_to_str(const char *filename, int flags, struct stat *stat_out)
{
  ...
  if (fd<0) {
    ...
    if (errno == ENOENT && (flags & RFTS_IGNORE_MISSING))
      severity = LOG_INFO;
    log_fn(severity,LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_OPENING_FILE_3),filename,strerror(errno));
    errno = save_errno;
    return NULL;
  }
  ...
  if (fstat(fd, &statbuf)<0) {
    int save_errno = errno;
    close(fd);
    log_warn(LD_FS,get_lang_str(LANG_LOG_UTIL_FSTAT_ERROR),filename);
    errno = save_errno;
    return NULL;
  }
  ...
  if (r<0) {
    int save_errno = errno;
    log_warn(LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_READING_FILE),filename,strerror(errno));
    tor_free(string);
    close(fd);
    errno = save_errno;
    return NULL;
  ...
  if (!bin) {
    ...
    if (r != statbuf.st_size) {
       ...
       * match for size. */
      int save_errno = errno;
      log_warn(LD_FS,get_lang_str(LANG_LOG_UTIL_ERROR_READING_FILE_2),r,(long)statbuf.st_size,filename);
      tor_free(string);
      close(fd);
      errno = save_errno;
      return NULL;
...
expand_filename(const char *filename)
{
  ...
  if (*filename == '~') {
    ...
    if (filename[1] == '/' || filename[1] == '\0') {
      home = getenv("HOME");
      if (!home) {
        log_warn(LD_CONFIG,get_lang_str(LANG_LOG_UTIL_ERROR_FINDING_HOME),filename);
        return NULL;
      }
      ...
      if (!(home = get_user_homedir(username))) {
        log_warn(LD_CONFIG,get_lang_str(LANG_LOG_UTIL_ERROR_FINDING_HOME_2),username);
        tor_free(username);
        return NULL;
      }
      tor_free(username);
      rest = slash ? (slash+1) : "";
#else
      log_warn(LD_CONFIG,get_lang_str(LANG_LOG_UTIL_PWD_H));
...
start_daemon(void)
{
  ...
  if (pipe(daemon_filedes)) {
    log_err(LD_GENERAL,get_lang_str(LANG_LOG_UTIL_PIPE_FAILED),strerror(errno));
    exit(1);
  }
  pid = fork();
  if (pid < 0) {
    log_err(LD_GENERAL,get_lang_str(LANG_LOG_UTIL_FORK_FAILED));
    exit(1);
  }
  if (pid) {  /* Parent */
    int ok;
...
finish_daemon(const char *desired_cwd)
{
  ...
  if (chdir(desired_cwd) < 0) {
    log_err(LD_GENERAL,get_lang_str(LANG_LOG_UTIL_CHDIR_FAILED),desired_cwd);
    exit(1);
  }

  nullfd = open("/dev/null", O_RDWR);
  if (nullfd < 0) {
    log_err(LD_GENERAL,get_lang_str(LANG_LOG_UTIL_DEV_NULL));
    exit(1);
  }
  ...
  if (dup2(nullfd,0) < 0 ||
      dup2(nullfd,1) < 0 ||
      dup2(nullfd,2) < 0) {
    log_err(LD_GENERAL,get_lang_str(LANG_LOG_UTIL_DUP2_FAILED));
    exit(1);
  }
  if (nullfd > 2)
    close(nullfd);
  ...
  if (write(daemon_filedes[1], &c, sizeof(char)) != sizeof(char)) {
    log_err(LD_GENERAL,get_lang_str(LANG_LOG_UTIL_WRITE_FAILED));
  }
