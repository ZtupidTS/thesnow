                        ...
                        uint8_t replica)
{
  ...
  if (!service_id ||
    ...
    log_warn(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_ILLEGAL_SERVICE_ID),safe_str(service_id));
    return -1;
  }
                           ...
                           smartlist_t *client_cookies)
{
  ...
  log_info(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_V2_DESCRIPTOR_OK));
  goto done;
                               ...
                               char **str_out, size_t *len_out)
{
  ...
  r = crypto_pk_private_sign_digest(key, cp, buflen - (cp - *str_out),*str_out, cp-*str_out);
  if (r<0) {
    tor_free(*str_out);
    return -1;
  }
...
rend_cache_clean_v2_descs_as_dir(void)
{
  ...
  for (iter = digestmap_iter_init(rend_cache_v2_dir);
    ...
    if (ent->parsed->timestamp < cutoff ||
      ...
      log_info(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_REMOVING_DESCRIPTOR),safe_str(key_base32));
...
rend_cache_lookup_v2_desc_as_dir(const char *desc_id, const char **desc)
{
  ...
  if (hid_serv_responsible_for_desc_id(desc_id_digest) < 0) {
    log_info(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_COULD_NOT_ANSWER_DESCRIPTOR_FETCH));
    return -1;
  }
...
rend_cache_store_v2_desc_as_dir(const char *desc)
{
  ...
  while (rend_parse_v2_service_descriptor(&parsed, desc_id, &intro_content,
    ...
    if (!hid_serv_responsible_for_desc_id(desc_id)) {
      log_info(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_DESCRIPTOR_NOT_IN_OUR_INTERVAL),safe_str(desc_id_base32));
      goto skip;
    }
                                   ...
                                   const rend_data_t *rend_query)
{
  ...
  /* Is descriptor too old? */
  if (parsed->timestamp < now - REND_CACHE_MAX_AGE-REND_CACHE_MAX_SKEW) {
    log_warn(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_DESCRIPTOR_TOO_OLD),safe_str(service_id));
    retval = -2;
    goto err;
  }
  /* Is descriptor too far in the future? */
  if (parsed->timestamp > now + REND_CACHE_MAX_SKEW) {
    log_warn(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_DESCRIPTOR_TOO_NEW),safe_str(service_id));
    retval = -2;
    goto err;
  }
  ...
  if (strmap_get_lc(rend_cache, key)) {
    log_info(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_DESCRIPTOR_ALREADY_HAVE_4),safe_str(service_id));
    retval = -1;
    goto err;
  }
  /* Do we already have a newer descriptor? */
  ...
  e = (rend_cache_entry_t*) strmap_get_lc(rend_cache, key);
  if (e && e->parsed->timestamp > parsed->timestamp) {
    log_info(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_DESCRIPTOR_ALREADY_HAVE_2),safe_str(service_id));
    retval = 0;
    goto err;
  }
  /* Do we already have this descriptor? */
  if (e && !strcmp(desc, e->desc)) {
    log_info(LD_REND,get_lang_str(LANG_LOG_RENDCOMMON_DESCRIPTOR_ALREADY_HAVE_3),safe_str(service_id));
    e->received = get_time(NULL);
    retval = 0;
    goto err;
  }
