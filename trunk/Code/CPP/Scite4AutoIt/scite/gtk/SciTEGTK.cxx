...
void SciTEGTK::AddToolBar() {
	...
	if (props.GetInt("toolbar.usestockicons") == 1) {
		AddToolButton("新建", IDM_NEW, gtk_image_new_from_stock("gtk-new", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("打开", IDM_OPEN, gtk_image_new_from_stock("gtk-open", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("保存", IDM_SAVE, gtk_image_new_from_stock("gtk-save", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("关闭", IDM_CLOSE, gtk_image_new_from_stock("gtk-close", GTK_ICON_SIZE_LARGE_TOOLBAR));

		gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
		AddToolButton("撤销", IDM_UNDO, gtk_image_new_from_stock("gtk-undo", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("恢复", IDM_REDO, gtk_image_new_from_stock("gtk-redo", GTK_ICON_SIZE_LARGE_TOOLBAR));

		gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
		AddToolButton("剪切", IDM_CUT, gtk_image_new_from_stock("gtk-cut", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("复制", IDM_COPY, gtk_image_new_from_stock("gtk-copy", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("粘贴", IDM_PASTE, gtk_image_new_from_stock("gtk-paste", GTK_ICON_SIZE_LARGE_TOOLBAR));

		gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
		AddToolButton("文件中查找", IDM_FINDINFILES, gtk_image_new_from_stock("gtk-find", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("查找", IDM_FIND, gtk_image_new_from_stock("gtk-zoom-fit", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("查找下一个", IDM_FINDNEXT, gtk_image_new_from_stock("gtk-jump-to", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("替换", IDM_REPLACE, gtk_image_new_from_stock("gtk-find-and-replace", GTK_ICON_SIZE_LARGE_TOOLBAR));

		gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
		btnCompile = AddToolButton("编译", IDM_COMPILE, gtk_image_new_from_stock("gtk-execute", GTK_ICON_SIZE_LARGE_TOOLBAR));
		btnBuild = AddToolButton("构建", IDM_BUILD, gtk_image_new_from_stock("gtk-convert", GTK_ICON_SIZE_LARGE_TOOLBAR));
		btnStop = AddToolButton("停止", IDM_STOPEXECUTE, gtk_image_new_from_stock("gtk-stop", GTK_ICON_SIZE_LARGE_TOOLBAR));

		gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
		AddToolButton("上一个", IDM_PREVFILE, gtk_image_new_from_stock("gtk-go-back", GTK_ICON_SIZE_LARGE_TOOLBAR));
		AddToolButton("下一个", IDM_NEXTFILE, gtk_image_new_from_stock("gtk-go-forward", GTK_ICON_SIZE_LARGE_TOOLBAR));
		return;
	}
#endif
	AddToolButton("新建", IDM_NEW, pixmap_new(PWidget(wSciTE), (gchar**)filenew_xpm));
	AddToolButton("打开", IDM_OPEN, pixmap_new(PWidget(wSciTE), (gchar**)fileopen_xpm));
	AddToolButton("保存", IDM_SAVE, pixmap_new(PWidget(wSciTE), (gchar**)filesave_xpm));
	AddToolButton("关闭", IDM_CLOSE, pixmap_new(PWidget(wSciTE), (gchar**)close_xpm));

	gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
	AddToolButton("撤销", IDM_UNDO, pixmap_new(PWidget(wSciTE), (gchar**)undo_xpm));
	AddToolButton("恢复", IDM_REDO, pixmap_new(PWidget(wSciTE), (gchar**)redo_xpm));

	gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
	AddToolButton("剪切", IDM_CUT, pixmap_new(PWidget(wSciTE), (gchar**)editcut_xpm));
	AddToolButton("复制", IDM_COPY, pixmap_new(PWidget(wSciTE), (gchar**)editcopy_xpm));
	AddToolButton("粘贴", IDM_PASTE, pixmap_new(PWidget(wSciTE), (gchar**)editpaste_xpm));

	gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
	AddToolButton("文件中查找", IDM_FINDINFILES, pixmap_new(PWidget(wSciTE), (gchar**)findinfiles_xpm));
	AddToolButton("查找", IDM_FIND, pixmap_new(PWidget(wSciTE), (gchar**)search_xpm));
	AddToolButton("查找下一个", IDM_FINDNEXT, pixmap_new(PWidget(wSciTE), (gchar**)findnext_xpm));
	AddToolButton("替换", IDM_REPLACE, pixmap_new(PWidget(wSciTE), (gchar**)replace_xpm));

	gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
	btnCompile = AddToolButton("编译", IDM_COMPILE, pixmap_new(PWidget(wSciTE), (gchar**)compile_xpm));
	btnBuild = AddToolButton("构建", IDM_BUILD, pixmap_new(PWidget(wSciTE), (gchar**)build_xpm));
	btnStop = AddToolButton("停止", IDM_STOPEXECUTE, pixmap_new(PWidget(wSciTE), (gchar**)stop_xpm));

	gtk_toolbar_append_space(GTK_TOOLBAR(PWidget(wToolBar)));
	AddToolButton("上一个", IDM_PREVFILE, pixmap_new(PWidget(wSciTE), (gchar**)prev_xpm));
	AddToolButton("下一个", IDM_NEXTFILE, pixmap_new(PWidget(wSciTE), (gchar**)next_xpm));
}
