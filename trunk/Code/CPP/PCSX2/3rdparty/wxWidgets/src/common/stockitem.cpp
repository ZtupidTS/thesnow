...
wxString wxGetStockLabel(wxWindowID id, long flags)
{
    ...
    switch (id)
    {
        STOCKITEM(wxID_ABOUT,               _("关于(&A)"))
        STOCKITEM(wxID_ADD,                 _("添加"))
        STOCKITEM(wxID_APPLY,               _("应用(&A)"))
        STOCKITEM(wxID_BOLD,                _("&Bold"))
        STOCKITEM(wxID_CANCEL,              _("取消(&C)"))
        STOCKITEM(wxID_CLEAR,               _("清除(&C)"))
        STOCKITEM(wxID_CLOSE,               _("关闭(&C)"))
        STOCKITEM(wxID_COPY,                _("复制(&C)"))
        STOCKITEM(wxID_CUT,                 _("剪切(&T)"))
        STOCKITEM(wxID_DELETE,              _("删除(&D)"))
        STOCKITEM(wxID_EDIT,                _("编辑(&E)"))
        STOCKITEM(wxID_FIND,                _("查找(&F)"))
        STOCKITEM(wxID_FILE,                _("文件(&F)"))
        STOCKITEM(wxID_REPLACE,             _("替换(&L)"))
        STOCKITEM(wxID_BACKWARD,            _("上一步(&B)"))
        STOCKITEM(wxID_DOWN,                _("下(&D)"))
        STOCKITEM(wxID_FORWARD,             _("下一步(&F)"))
        STOCKITEM(wxID_UP,                  _("&Up"))
        STOCKITEM(wxID_HELP,                _("帮助(&H)"))
        STOCKITEM(wxID_HOME,                _("&Home"))
        STOCKITEM(wxID_INDENT,              _("Indent"))
        STOCKITEM(wxID_INDEX,               _("&Index"))
        STOCKITEM(wxID_ITALIC,              _("&Italic"))
        STOCKITEM(wxID_JUSTIFY_CENTER,      _("居中"))
        ...
        STOCKITEM(wxID_NEW,                 _("新建(&N)"))
        STOCKITEM(wxID_NO,                  _("否(&N)"))
        STOCKITEM(wxID_OK,                  _("确定(&O)"))
        STOCKITEM(wxID_OPEN,                _("打开(&O)"))
        STOCKITEM(wxID_PASTE,               _("粘贴(&P)"))
        STOCKITEM(wxID_PREFERENCES,         _("参数(&P)"))
        STOCKITEM(wxID_PRINT,               _("打印(&P)"))
        STOCKITEM(wxID_PREVIEW,             _("打印预览(&W)"))
        STOCKITEM(wxID_PROPERTIES,          _("属性(&P)"))
        STOCKITEM(wxID_EXIT,                _("退出(&Q)"))
        STOCKITEM(wxID_REDO,                _("撤销(&R)"))
        STOCKITEM(wxID_REFRESH,             _("刷新(&R)"))
        STOCKITEM(wxID_REMOVE,              _("移除"))
        STOCKITEM(wxID_REVERT_TO_SAVED,     _("Revert to Saved"))
        STOCKITEM(wxID_SAVE,                _("保存(&S)"))
        STOCKITEM(wxID_SAVEAS,              _("另存为(&A)..."))
        STOCKITEM(wxID_SELECTALL,           _("全选"))
        STOCKITEM(wxID_STOP,                _("停止(&S)"))
        STOCKITEM(wxID_UNDELETE,            _("Undelete"))
        STOCKITEM(wxID_UNDERLINE,           _("下划线(&U)"))
        STOCKITEM(wxID_UNDO,                _("撤销(&U)"))
        STOCKITEM(wxID_UNINDENT,            _("&Unindent"))
        STOCKITEM(wxID_YES,                 _("是(&Y)"))
        STOCKITEM(wxID_ZOOM_100,            _("实际大小(&A)"))
        STOCKITEM(wxID_ZOOM_FIT,            _("Zoom to &Fit"))
        STOCKITEM(wxID_ZOOM_IN,             _("放大(&I)"))
        STOCKITEM(wxID_ZOOM_OUT,            _("缩小(&O)"))
...
wxString wxGetStockHelpString(wxWindowID id, wxStockHelpStringClient client)
{
    ...
    switch (id)
    {
        ...
        STOCKITEM(wxID_COPY,     wxSTOCK_MENU, _("复制所选"))
        STOCKITEM(wxID_CUT,      wxSTOCK_MENU, _("剪切所选"))
        STOCKITEM(wxID_DELETE,   wxSTOCK_MENU, _("删除所选"))
        STOCKITEM(wxID_REPLACE,  wxSTOCK_MENU, _("替换所选"))
        STOCKITEM(wxID_PASTE,    wxSTOCK_MENU, _("粘贴所选"))
        STOCKITEM(wxID_EXIT,     wxSTOCK_MENU, _("退出此程序"))
        STOCKITEM(wxID_REDO,     wxSTOCK_MENU, _("Redo last action"))
        STOCKITEM(wxID_UNDO,     wxSTOCK_MENU, _("Undo last action"))
        STOCKITEM(wxID_CLOSE,    wxSTOCK_MENU, _("关闭当前文档"))
