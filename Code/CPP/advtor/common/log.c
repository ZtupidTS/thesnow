...
{	lseverity=severity;
	if(fname)
	{	hfile=CreateFile(fname,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ,0,OPEN_ALWAYS,0,0);
...
sev_to_string(int severity)
{
  switch (severity) {
    ...
    default:          /* Call assert, not tor_assert, since tor_assert
                      ...
                      assert(0); return get_lang_str(LANG_LOG_LOG__UNKNOWN);
  }
}
           ...
           const char *format, va_list ap)
{
  ...
  assert(buf_len >= 3); /* prevent integer underflow */
...
libevent_logging_callback(int severity, const char *msg)
{
  ...
  switch (severity) {
    case _EVENT_LOG_DEBUG:
      log(LOG_DEBUG,LD_NOCB|LD_NET,get_lang_str(LANG_LOG_LOG_LIBEVENT),buf);
      break;
    case _EVENT_LOG_MSG:
      log(LOG_INFO,LD_NOCB|LD_NET,get_lang_str(LANG_LOG_LOG_LIBEVENT),buf);
      break;
    case _EVENT_LOG_WARN:
      log(LOG_WARN,LD_NOCB|LD_GENERAL,get_lang_str(LANG_LOG_LOG_LIBEVENT_2),buf);
      break;
    case _EVENT_LOG_ERR:
      log(LOG_ERR,LD_NOCB|LD_GENERAL,get_lang_str(LANG_LOG_LOG_LIBEVENT_3),buf);
      break;
    default:
      log(LOG_WARN,LD_NOCB|LD_GENERAL,get_lang_str(LANG_LOG_LOG_LIBEVENT_4),severity,buf);
      break;
  }
}
