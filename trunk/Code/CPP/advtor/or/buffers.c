...
buf_shrink_freelists(int free_all)
{
  ...
  for (i = 0; freelists[i].alloc_size; ++i) {
    ...
    if (free_all || freelists[i].lowest_length > slack) {
      ...
      chunk_t *chunk;
      // "Cleaning freelist for %d-byte chunks: keeping %d, dropping %d."
      log_info(LD_MM, get_lang_str(LANG_LOG_BUFFERS_CLEAN_FREELIST),(int)freelists[i].alloc_size, n_to_skip, n_to_free);
      while (n_to_skip) {
        tor_assert((*chp)->next);
        chp = &(*chp)->next;
        --n_to_skip;
              ...
              int *reached_eof, int *socket_error)
{
  ...
  if (read_result < 0) {
    ...
    *reached_eof = 1;
    ...
    log_debug(LD_NET,get_lang_str(LANG_LOG_BUFFERS_READ_INBUF), (long)read_result,
              (int)buf->datalen);
  ...
  switch (socksver) { /* which version of socks? */
    ...
    case 5: /* socks5 */
      ...
      if (req->socks_version != 5) { /* we need to negotiate a method */
        ...
        if (!nummethods || (!memchr(buf->head->data+2, 0, nummethods)&&!memchr(buf->head->data+2, 0xE0, nummethods))) {
	...
	{	req->reply[1] = 0; /* tell client to use "none" auth method */
      ...
      {	if(buf->head->datalen < 18) return 0;
      	else
	{	tmpbuf[0]=*(buf->head->data+4);tmpbuf[1]=*(buf->head->data+5);tmpbuf[2]=0;
		...
		getRandomExitNode(*(buf->head->data+2),*(buf->head->data+3),tmpbuf,destip,destipH,bw,(char*)&req->address,(char*)&req->reply);
		req->replylen=14;
      ...
      switch (*(buf->head->data+3)) { /* address type */
        ...
        case 3: /* fqdn */
          ...
          if (len+1 > MAX_SOCKS_ADDR_LEN) {
            log_warn(LD_APP,get_lang_str(LANG_LOG_BUFFERS_SOCKS_HOSTNAME_TOO_BIG), len+1,MAX_SOCKS_ADDR_LEN);
            return -1;
          }
	...
	http_hdr += 4;
	if((int)buf->head->datalen < http_hdr)	buf_pullup(buf,http_hdr,0);
	...
	{	tmp = buf->head->data;i=0;j=0;
		...
		{	len=strchr(tmp+i,10)-tmp-i+1;
			...
			else if(!strcasecmpstart(tmp+i,"connection:") && len > 19)
