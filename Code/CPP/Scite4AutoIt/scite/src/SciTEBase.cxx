...
SciTEBase::SciTEBase(Extension *ext) : apis(true), extender(ext) {
	...
	tabHideOne = false;						//一个标签时隐藏
	tabMultiLine = false;					//多行标签
	sbNum = 1;
	visHeightTools = 0;
	visHeightStatus = 0;
	visHeightEditor = 1;
	...
	firstPropertiesRead = true;				//首先读取属性文件
	...
	indentationWSVisible = true;			//缩进
	indentExamine = SC_IV_LOOKBOTH;			//缩进元素

	autoCompleteIgnoreCase = false;
	callTipIgnoreCase = false;
	autoCCausedByOnlyOne = false;
	startCalltipWord = 0;
	currentCallTip = 0;							//当前调用提示
	maxCallTips = 1;							//最大调用提示
	currentCallTipWord = "";					//当前调用提示单词
	lastPosCallTip = 0;							//最后调用提示坐标

	margin = false;								//页边空白
	marginWidth = marginWidthDefault;			//页边空白宽度
	foldMargin = true;
	foldMarginWidth = foldMarginWidthDefault;
	lineNumbers = true;							//行号
	lineNumbersWidth = lineNumbersWidthDefault;	//行号宽度
	lineNumbersExpand = false;					//行号展开
	usePalette = false;							//使用调色板

	replacing = false;							//替换
	havefound = false;							//已经找到
	matchCase = false;							//区分大小写
	wholeWord = false;
	reverseFind = false;						//反向查找
	regExp = false;								//使用正则表达式
	wrapFind = true;
	unSlash = false;
	findInStyle = false;
	findStyle = 0;
	...
	needReadProperties = false;					//需要读取属性
	allowAlpha = true;
}
...
#ifdef AUTOIT
		AddStyledText(wsci, "    http://www.autoit.net.cn\n", 4);
#else
...
bool SciTEBase::StartAutoCompleteWord(bool onlyOneWord) {
	...
	while (posFind >= 0 && posFind < doclen) {				// 搜索所有文档
...
//		break;
	...
	case IDM_DONATE:								//捐助
		...
		SString msg = LocaliseMessage("最新Autoit汉化版本所有文件更新于(非独立安装文件):\n"
			 ...
			 "对于狂热FANS.请自行使用SVN客户端使用检出.(永远比现在发布的汉化版本新)\n"
...
void SciTEBase::ContextMenu(Window wSource, Point pt, Window wCmd) {
	...
	AddToPopUp("撤销", IDM_UNDO, writable && SendWindow(wSource, SCI_CANUNDO));
	AddToPopUp("恢复", IDM_REDO, writable && SendWindow(wSource, SCI_CANREDO));
	AddToPopUp("");
	AddToPopUp("剪切", IDM_CUT, writable && currentPos != anchor);
	AddToPopUp("复制", IDM_COPY, currentPos != anchor);
	AddToPopUp("粘贴", IDM_PASTE, writable && SendWindow(wSource, SCI_CANPASTE));
