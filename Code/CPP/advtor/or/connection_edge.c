...
connection_edge_end(edge_connection_t *conn, uint8_t reason)
{
  ...
  if (circ && !circ->marked_for_close) {
    ...
    log_debug(LD_EDGE,get_lang_str(LANG_LOG_EDGE_NO_CIRC_TO_SEND_END),conn->_base.s);
  }
...
connection_ap_expire_beginning(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, c) {
    ...
    if (AP_CONN_STATE_IS_UNATTACHED(conn->_base.state)) {
        ...
        log_fn(severity, LD_APP,get_lang_str(LANG_LOG_EDGE_CONNECTION_TIMEOUT),seconds_since_born, safe_str(conn->socks_request->address),conn->socks_request->port,conn_state_to_string(CONN_TYPE_AP, conn->_base.state));
        connection_mark_unattached_ap(conn, END_STREAM_REASON_TIMEOUT);
      }
      continue;
    }
    ...
    if (!circ) { /* it's vanished? */
      log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_CONN_LOST_CIRC),safe_str(conn->socks_request->address));
      connection_mark_unattached_ap(conn, END_STREAM_REASON_TIMEOUT);
      continue;
    }
    if (circ->purpose == CIRCUIT_PURPOSE_C_REND_JOINED) {
        if (seconds_idle >= options->SocksTimeout) {
        log_fn(severity, LD_REND,get_lang_str(LANG_LOG_EDGE_REND_TIMEOUT),seconds_idle,safe_str(conn->socks_request->address));
    ...
    log_fn(cutoff < 15 ? LOG_INFO : severity, LD_APP,get_lang_str(LANG_LOG_EDGE_CONNECTION_TIMEOUT_2),seconds_idle, safe_str(conn->socks_request->address),conn->cpath_layer ?conn->cpath_layer->extend_info->nickname : "*unnamed*");
...
circuit_discard_optional_exit_enclaves(extend_info_t *info)
{
  ...
  SMARTLIST_FOREACH_BEGIN(conns, connection_t *, conn) {
    ...
    if (edge_conn->chosen_exit_optional) {
      log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_GIVING_UP_ON_ENCLAVE_EXIT),safe_str(edge_conn->chosen_exit_name),escaped_safe_str(edge_conn->socks_request->address));
...
addressmap_rewrite(char *address, size_t maxlen, time_t *expires_out)
{
  ...
  for (rewrites = 0; rewrites < 16; rewrites++) {
    ...
    log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_ADDRESSMAP_REWRITE_ADDRESS),escaped_safe_str(address), cp);
    if (ent->expires > 1 && ent->expires < expires)
      expires = ent->expires;
    tor_free(cp);
    strlcpy(address, ent->new_address, maxlen);
  }
  log_warn(LD_CONFIG,get_lang_str(LANG_LOG_EDGE_ADDRESSMAP_REWRITTEN_TOO_MUCH),escaped_safe_str(address));
...
addressmap_rewrite_reverse(char *address, size_t maxlen, time_t *expires_out)
{
  ...
  if (ent) {
    cp = tor_strdup(escaped_safe_str(ent->new_address));
    log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_ADDRESSMAP_REWRITE_REVDNS),escaped_safe_str(s),cp);
    tor_free(cp);
    strlcpy(address, ent->new_address, maxlen);
    r = 1;
  }
 ...
 */
void
addressmap_register(char *address, char *new_address, time_t expires,
                    addressmap_entry_source_t source)
{
  ...
  if (!ent) { /* make a new one and register it */
    ...
    if (expires > 1) {
      log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_ADDRESSMAP_ADDR_ALREADY_MAPPED),safe_str(address), safe_str(new_address), safe_str(ent->new_address));
      tor_free(new_address);
      return;
    }
  ...
  log_info(LD_CONFIG,get_lang_str(LANG_LOG_EDGE_ADDRESSMAP_REMAPPED_ADDR),safe_str(address), safe_str(ent->new_address));
...
client_dns_incr_failures(const char *address)
{
  ...
  log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_ADDRESS_HAS_RESOLVE_FAILURES),safe_str(address),ent->num_resolve_failures);
                               ...
                               int ttl)
{
  ...
  addressmap_register(extendedaddress, tor_strdup(extendedval),
                      get_time(NULL) + ttl, ADDRMAPSRC_DNS);
}
...
addressmap_register_virtual_address(int type, char *new_address)
{
  ...
  if (*addrp) {
    ...
    if (ent && ent->new_address &&
      ...
      log_warn(LD_BUG,get_lang_str(LANG_LOG_EDGE_INTERNAL_CONFUSION),safe_str(new_address), safe_str(*addrp), safe_str(*addrp),ent?safe_str(ent->new_address):"(nothing)");
  }
  ...
  *addrp = addressmap_get_virtual_address(type);
...
#if 0
  {
    ...
    tor_assert(!strcasecmp(*addrp,
                           ...
                           vent->ipv4_address : vent->hostname_address));
    log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_ADDRESSMAP_MAP_OK),safe_str(*addrp),safe_str(new_address));
  }
#endif
...
/*  log_debug(LD_APP,get_lang_str(LANG_LOG_EDGE_CLIENT_ASKED_FOR),safe_str(socks->address),socks->port);*/

  if(addressmap_rewrite(socks->address,sizeof(socks->address),&map_expires)) control_event_stream_status(conn, STREAM_EVENT_REMAP,REMAP_STREAM_SOURCE_CACHE);
  if(!plugins_remap(conn,&socks->address[0],&socks->original_address[0],0))
  ...
  if (socks->command == SOCKS_COMMAND_RESOLVE &&
    ...
    if (automap) {
      ...
      log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_AUTOMAPPING),escaped_safe_str(socks->address), safe_str(new_addr));
  ...
  if (socks->command == SOCKS_COMMAND_RESOLVE_PTR) {
    if (addressmap_rewrite_reverse(socks->address, sizeof(socks->address),
      ...
      connection_ap_handshake_socks_resolved(conn, RESOLVED_TYPE_HOSTNAME,
                                             strlen(result), (uint8_t*)result, -1,
                                             map_expires);
  ...
  if (addresstype == BAD_HOSTNAME) {
    log_warn(LD_APP,get_lang_str(LANG_LOG_EDGE_INVALID_HOSTNAME), socks->address);
  ...
  if (addresstype == EXIT_HOSTNAME) {
	...
	{	log_warn(LD_APP,get_lang_str(LANG_LOG_CONNECTION_BANNED_EXIT),safe_str(socks->address),socks->port,safe_str(conn->chosen_exit_name));
		connection_mark_unattached_ap(conn, END_STREAM_REASON_TORPROTOCOL);
		return -1;
	}
    }
  ...
  if (addresstype != ONION_HOSTNAME) {
    ...
    if (socks->command == SOCKS_COMMAND_RESOLVE) {
      ...
      if (!conn->use_begindir && !conn->chosen_exit_name && !circ) {
        ...
        if (r) {
          log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_REDIRECTING_TO_EXIT),safe_str(socks->address), r->nickname);
    ...
    log_info(LD_REND,get_lang_str(LANG_LOG_EDGE_HS_REQUEST),safe_str(conn->rend_data->onion_address));
    /* see if we already have it cached */
    r = rend_cache_lookup_entry(conn->rend_data->onion_address, -1, &entry);
    if (r<0) {
      log_warn(LD_BUG,get_lang_str(LANG_LOG_EDGE_INVALID_HS_NAME),safe_str(conn->rend_data->onion_address));
      connection_mark_unattached_ap(conn, END_STREAM_REASON_TORPROTOCOL);
      return -1;
    }
    ...
    if (r==0) {
      ...
      log_info(LD_REND,get_lang_str(LANG_LOG_EDGE_FETCHING_UNKNOWN_DESCRIPTOR),safe_str(conn->rend_data->onion_address));
...
#define NUM_SECONDS_BEFORE_HS_REFETCH (60*15)
      if (now - entry->received < NUM_SECONDS_BEFORE_HS_REFETCH) {
        ...
        conn->_base.state = AP_CONN_STATE_RENDDESC_WAIT;
        log_info(LD_REND,get_lang_str(LANG_LOG_EDGE_FETCHING_STALE_DESCRIPTOR),safe_str(conn->rend_data->onion_address));
        /* Fetch both, v0 and v2 rend descriptors in parallel. Use whichever
         * arrives first. Exception: When using client authorization, only
         * fetch v2 descriptors.*/
        rend_client_refetch_v2_renddesc(conn->rend_data);
...
connection_ap_handshake_process_socks(edge_connection_t *conn)
{
  ...
  if(socks->address[0] && !socks->original_address[0])	strlcpy(socks->original_address,socks->address,MAX_SOCKS_ADDR_LEN);
  if (sockshere == 0) {
    ...
    if (socks->replylen) {
      ...
      log_debug(LD_APP,get_lang_str(LANG_LOG_EDGE_HANDSHAKE_INCOMPLETE));
    }
    return 0;
  } else if (sockshere == -1) {
  ...
  else if(is_banned(socks->address))
  {     log(LOG_ADDR,LD_APP,get_lang_str(LANG_LOG_CONNECTION_BANNED_ADDRESS),safe_str(socks->address),socks->port);
	connection_ap_handshake_socks_reply(conn, NULL, 0,END_STREAM_REASON_SOCKSPROTOCOL);
	connection_mark_unattached_ap(conn,END_STREAM_REASON_SOCKSPROTOCOL |END_STREAM_REASON_FLAG_ALREADY_SOCKS_REPLIED);
	return -1;
  ...
  {     log(LOG_ADDR,LD_APP,get_lang_str(LANG_LOG_CONNECTION_CONNECTION_REQUEST),socks->address,socks->port);
...
connection_ap_handshake_send_resolve(edge_connection_t *ap_conn)
{
  ...
  if (command == SOCKS_COMMAND_RESOLVE) {
    ...
    if (r <= 0) {
      log_warn(LD_APP,get_lang_str(LANG_LOG_EDGE_INVALID_REVDNS),safe_str(a));
      connection_mark_unattached_ap(ap_conn, END_STREAM_REASON_INTERNAL);
      return -1;
    }
    ...
    if (r < 0) {
      log_warn(LD_BUG,get_lang_str(LANG_LOG_EDGE_REVDNS_ERROR),safe_str(a));
      connection_mark_unattached_ap(ap_conn, END_STREAM_REASON_INTERNAL);
      return -1;
    }
                        ...
                        const char *digest, int use_begindir, int want_onehop)
{
  ...
  log_info(LD_APP,get_lang_str(LANG_LOG_EDGE_MAKE_INTERNAL_TUNNEL),want_onehop ? get_lang_str(LANG_LOG_EDGE_DIRECT) : get_lang_str(LANG_LOG_EDGE_ANONYMIZED) , safe_str(address),port);
