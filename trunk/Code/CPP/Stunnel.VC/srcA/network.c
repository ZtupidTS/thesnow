...
int connect_blocking(CLI *c, SOCKADDR_UNION *addr, socklen_t addrlen) {
    ...
    case -1:
        error=get_last_socket_error();
        s_log(LOG_ERR, "连接区块: s_poll_wait %s: %s (%d)",
            dst, s_strerror(error), error);
        str_free(dst);
        return -1;
    case 0:
        s_log(LOG_ERR, "连接区块: s_poll_wait %s:"
    ...
    default:
        if(s_poll_canread(c->fds, c->fd) || s_poll_error(c->fds, c->fd)) {
            ...
            if(error) { /* really an error? */
                s_log(LOG_ERR, "连接区块: getsockopt %s: %s (%d)",
                    dst, s_strerror(error), error);
                str_free(dst);
                return -1;
            }
        ...
        s_log(LOG_ERR, "连接区块: s_poll_wait %s: 内部错误",
            dst);
        str_free(dst);
        return -1;
    }
...
void write_blocking(CLI *c, int fd, void *ptr, int len) {
    ...
    while(len>0) {
        ...
        case -1:
            sockerror("写入区块: s_poll_wait");
            longjmp(c->err, 1); /* error */
        case 0:
            s_log(LOG_INFO, "写入区块: s_poll_wait:"
                " TIMEOUTbusy exceeded: sending reset");
            longjmp(c->err, 1); /* timeout */
        case 1:
            break; /* OK */
        default:
            s_log(LOG_ERR, "写入区块: s_poll_wait: 未知结果");
            longjmp(c->err, 1); /* error */
        }
        num=writesocket(fd, ptr, len);
        switch(num) {
        case -1: /* error */
            sockerror("写入套接字 (写入区块)");
            longjmp(c->err, 1);
        }
        ptr=(u8 *)ptr+num;
        len-=num;
...
void read_blocking(CLI *c, int fd, void *ptr, int len) {
    ...
    while(len>0) {
        ...
        switch(s_poll_wait(c->fds, c->opt->timeout_busy, 0)) {
        case -1:
            sockerror("读取区块: s_poll_wait");
            longjmp(c->err, 1); /* error */
        case 0:
            s_log(LOG_INFO, "读取区块: s_poll_wait:"
                " TIMEOUTbusy exceeded: sending reset");
            longjmp(c->err, 1); /* timeout */
        case 1:
            break; /* OK */
        default:
            s_log(LOG_ERR, "读取区块: s_poll_wait: 未知结果");
            longjmp(c->err, 1); /* error */
        }
        num=readsocket(fd, ptr, len);
        switch(num) {
        case -1: /* error */
            sockerror("读取套接字 (读取区块)");
            longjmp(c->err, 1);
        case 0: /* EOF */
            s_log(LOG_ERR, "套接字意外关闭 (读取区块)");
            longjmp(c->err, 1);
        }
        ptr=(u8 *)ptr+num;
        len-=num;
