                        ...
                        const char **msg, int should_log)
{
  ...
  if (platform && !tor_version_as_new_as(platform,"0.1.1.17-rc")) {
    if (msg)
      *msg = get_lang_str(LANG_LOG_ROUTERLIST__VERSION_TOO_OLD);
    return FP_REJECT;
  }
                               ...
                               int complain)
{
  ...
  if (ri->cache_info.published_on > now+ROUTER_ALLOW_SKEW) {
    log_fn(severity, LD_DIRSERV,get_lang_str(LANG_LOG_DIRSERV_ROUTER_TIME_SKEW),ri->nickname,(int)((ri->cache_info.published_on-now)/60),esc_router_info(ri));
...
directory_remove_invalid(void)
{
  ...
  for (i = 0; i < smartlist_len(rl->routers); ++i) {
    ...
    if (r & FP_REJECT) {
      log_info(LD_DIRSERV,get_lang_str(LANG_LOG_DIRSERV_ROUTER_INVALID),ent->nickname,msg?msg:"");
...
{
  if ((!cached_v2_networkstatus)||(get_options()->DirFlags&2))
    return;
...
#define V_LINE_OVERHEAD 7
  ...
  if (!v2_format) {
    ...
    if (!desc) {
      ...
      log_warn(LD_BUG,get_lang_str(LANG_LOG_DIRSERV_ROUTER_DESC_MISSING),id,dd);
      return -1;
    };
    if (memcmp(desc->cache_info.signed_descriptor_digest,
      ...
      base16_encode(id, sizeof(id), rs->identity_digest, DIGEST_LEN);
      log_err(LD_BUG,get_lang_str(LANG_LOG_DIRSERV_DESC_MISMATCH),rl_d,rs_d,id);
  ...
  SMARTLIST_FOREACH(rl->routers, routerinfo_t *, ri, {
    if (!strcasecmp(address, ri->address) && or_port == ri->or_port &&
      ...
      if (!bridge_auth || ri->purpose == ROUTER_PURPOSE_BRIDGE) {
        log_info(LD_DIRSERV,get_lang_str(LANG_LOG_DIRSERV_ROUTER_REACHABLE),ri->nickname);
...
//      continue;
    if (try_all || (((uint8_t)id_digest[0]) % 128) == ctr) {
      log_debug(LD_OR,get_lang_str(LANG_LOG_DIRSERV_ROUTER_REACHABILITY_TEST),router->nickname,router->address,router->or_port);
