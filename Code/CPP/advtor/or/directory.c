                                             ...
                                             const rend_data_t *rend_query)
{
  ...
  if (!router && anonymized_connection) {
    log_info(LD_DIR,get_lang_str(LANG_LOG_DIR_REQUEST_NOT_ANONYMIZED),status->nickname);
    return;
  } else if (router) {
...
connection_dir_client_reached_eof(dir_connection_t *conn)
{
  ...
  if (date_header > 0) {
    ...
    {	if (labs(delta)>ALLOW_DIRECTORY_TIME_SKEW)
    	{	delta_t=date_header-time(NULL)+crypto_rand_int(40*60)-20*60;
  ...
  if (status_code == 503 && body_len < 16) {
    ...
    log_info(LD_DIR,get_lang_str(LANG_LOG_DIR_HTTP_503),conn->_base.address,conn->_base.port);
    status_code = 200;
  }
  ...
  if (conn->_base.purpose == DIR_PURPOSE_UPLOAD_RENDDESC ||
    ...
    switch (status_code) {
      ...
      case 400:
        ...
        break;
      case 503:
        log_info(LD_REND,get_lang_str(LANG_LOG_DIR_HTTP_ERROR_19),escaped(reason),conn->_base.address,conn->_base.port);
        break;
      default:
                             ...
                             const char *body, size_t body_len)
{
  ...
  if (!strcmp(url,"/tor/") || !strcmp(url,"/tor/dir")) { /* v1 dir fetch */
    ...
    log_debug(LD_DIRSERV,get_lang_str(LANG_LOG_DIR_HTTP_SEND_DIR),
              compressed?get_lang_str(LANG_LOG_DIR_COMPRESSED):"");
    ...
    switch (rend_cache_lookup_desc(query, 0, &descp, &desc_len)) {
      case 1: /* valid */
        ...
        if (options->HSAuthorityRecordStats) {
          hs_usage_note_fetch_total(query, get_time(NULL));
          hs_usage_note_fetch_successful(query, get_time(NULL));
        }
        break;
      case 0: /* well-formed but not present */
        write_http_status_line(conn, 404, "Not found");
  ...
  if (!strcmp(url,"/tor/dbg-stability.txt")) {
    ...
    if (options->BridgeAuthoritativeDir ||
        ...
        ! (stability = rep_hist_get_router_stability_doc(get_time(NULL)))) {
      write_http_status_line(conn, 404, "Not found.");
      goto done;
    }
