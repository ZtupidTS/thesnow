...
{	int i,j,k,l;
	...
	{	for(l=0;last_max>>l!=0;l++)	;
		...
		for(j=63;j>32;j--)
		{
			for(i=0;i<128;i++)
			{	*memPtr++=10;
				*memPtr++=draw_bw_read[i]>j?255:0;
				*memPtr++=draw_bw_written[i]>j?255:0;
				*memPtr++=0;
			}
		}
		for(i=0;i<128;i++)
...
void show_connection_info(connection_t *conn,char *s1)
{
	...
	{	edge_connection_t *conn2=TO_EDGE_CONN(conn);
		...
		{	tor_snprintf(s1,100,get_lang_str(LANG_NETINFO_SOCKS_ADDRESS),conn2->socks_request->original_address,conn2->socks_request->port);s1 += strlen(s1);
			tor_snprintf(s1,100,get_lang_str(LANG_NETINFO_SOCKS_FINAL_ADDRESS),conn2->socks_request->address,conn2->socks_request->port);s1 += strlen(s1);
...
{	tvit.hItem=hItem;tvit.mask=TVIF_PARAM;tvit.lParam=0;
	...
	{	crypt_path_t *hop=(crypt_path_t*)lParam;
		...
		{	char *s=tor_malloc(32767),*s1;
			...
			tor_snprintf(s1,100,get_lang_str(LANG_NETINFO_HOP_ADDRESS),geoip_get_country_name(geoip_get_country_by_ip(raddr)),raddr&0xff,(raddr>>8)&0xff,(raddr>>16)&0xff,(raddr>>24)&0xff,port);
			s1+=strlen(s1);
			while(1)
		...
		{	if(hop->hItem) tvins1.hParent=hop->hItem;
			...
			{	ri = router_get_by_digest(hop->extend_info->identity_digest);
				...
				if(hop->extend_info->nickname)
					tor_snprintf(nodename,1024,"[%s] %d.%d.%d.%d (%s)",geoip_get_country_name(geoip_get_country_by_ip(raddr)),raddr&0xff,(raddr>>8)&0xff,(raddr>>16)&0xff,(raddr>>24)&0xff,hop->extend_info->nickname);
				else if(ri && ri->nickname)
					tor_snprintf(nodename,1024,"[%s] %d.%d.%d.%d (%s)",geoip_get_country_name(geoip_get_country_by_ip(raddr)),raddr&0xff,(raddr>>8)&0xff,(raddr>>16)&0xff,(raddr>>24)&0xff,ri->nickname);
				else
				{	tor_snprintf(nodename,1024,"[%s] %d.%d.%d.%d - $",geoip_get_country_name(geoip_get_country_by_ip(raddr)),raddr&0xff,(raddr>>8)&0xff,(raddr>>16)&0xff,(raddr>>24)&0xff);
				...
				hop->circ=circ;
				hop->hItem=tvins1.hParent=(HTREEITEM)SendDlgItemMessage(hDlgNetInfo,25500,TVM_INSERTITEM,0,(LPARAM)&tvins1);
				if(expandtree) SendDlgItemMessage(hDlgNetInfo,25500,TVM_EXPAND,TVE_EXPAND,(LPARAM)hParent);
			}
			hop=hop->next;
...
{	if(!hItem) return;
	char *nodename=tor_malloc(1024),*s1;
	LangEnterCriticalSection();
	while(streams)
	{	if(!streams->_base.marked_for_close)
		{	uint32_t raddr=tor_addr_to_ipv4n(&streams->_base.addr);
			...
			{	if(streams->socks_request->original_address[0]){	tor_snprintf(s1,MAX_SOCKS_ADDR_LEN," -> %s",streams->socks_request->original_address);s1 += strlen(s1);}
				else if(streams->socks_request->address[0]){	tor_snprintf(s1,MAX_SOCKS_ADDR_LEN," -> %s",streams->socks_request->address);s1 += strlen(s1);}
			...
			tvins.item.lParam=(LPARAM)streams;
			...
			streams->_base.hItem=tvins.hInsertAfter=(HTREEITEM)SendDlgItemMessage(hDlgNetInfo,25500,TVM_INSERTITEM,0,(LPARAM)&tvins);
		}
		else break;
...
{	LangEnterCriticalSection();
	...
	circ->hItem=tvins.hInsertAfter=(HTREEITEM)SendDlgItemMessage(hDlgNetInfo,25500,TVM_INSERTITEM,0,(LPARAM)&tvins);
	LangLeaveCriticalSection();
	add_all_conns(circ);
	if(CIRCUIT_IS_ORIGIN(circ))
	{	origin_circuit_t *c=TO_ORIGIN_CIRCUIT(circ);
...
void tree_destroy_circuit_menu()
{	LPARAM lParam;
	...
	{	HTREEITEM hParent=hItem;
		while(hItem)
		{	hItem=(HTREEITEM)SendDlgItemMessage(hDlgNetInfo,25500,TVM_GETNEXTITEM,TVGN_PARENT,(LPARAM)hItem);
...
{	if(r)
	{	uint32_t raddr=geoip_reverse(r->addr);
		tor_snprintf(s1,100,"; [%s] %d.%d.%d.%d (",geoip_get_country_name(geoip_get_country_by_ip(raddr)),raddr&0xff,(raddr>>8)&0xff,(raddr>>16)&0xff,(raddr>>24)&0xff);
		s1 += strlen(s1);
	...
	{	crypt_path_t *hop=(crypt_path_t*)lParam;
		...
		{	connection_t *conn=(connection_t*)lParam;
			...
			{	LangAppendMenu(hMenu,MF_STRING|MF_UNCHECKED|MF_ENABLED,25003,LANG_NETINFO_DESTROY_CIRCUIT);
				...
				if(conn2->socks_request && conn2->socks_request->original_address[0])
...
{	if(uMsg==WM_INITDIALOG)
	...
	else if(uMsg==WM_COMMAND)
	{
		if(LOWORD(wParam)==25001)
			...
			dlgCircuitBuild_trackedHostExitAdd(hDlg,lastSocksOriginalAddress);
		else if(LOWORD(wParam)==25013)
			dlgCircuitBuild_trackedDomainExitAdd(hDlg,lastSocksOriginalAddress);
		else if(LOWORD(wParam)==25014)
			dlgCircuitBuild_addressMapAdd(hDlg,lastSocksOriginalAddress);
		else if(LOWORD(wParam)==25015)
			dlgCircuitBuild_addressMapRemove(hDlg,lastSocksOriginalAddress);
	...
	{	if(wParam==25500)
		{	pnmtvw=(NM_TREEVIEWW*)lParam;
			...
			else if(pnmtvw->hdr.code==NM_RCLICK)
			{	HTREEITEM hItem=(HTREEITEM)SendDlgItemMessage(hDlg,25500,TVM_GETNEXTITEM,TVGN_DROPHILITE,0);
				if(!hItem)	hItem=(HTREEITEM)SendDlgItemMessage(hDlg,25500,TVM_GETNEXTITEM,TVGN_CARET,0);
				if(hItem)	tree_show_menu(hItem);
			}
		}
	}
	return 0;
