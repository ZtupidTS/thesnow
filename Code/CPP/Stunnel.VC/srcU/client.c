...
static void run_client(CLI *c) {
    ...
    s_log(LOG_NOTICE,
        "连接 %s: %d 字节发送给了SSL加密, %d 字节发送给了套接字",
         error==1 ? "已重置" : "已关闭", c->ssl_bytes, c->sock_bytes);
...
static void print_bound_address(CLI *c) {
    ...
    s_log(LOG_NOTICE,"%s 服务已经使用 %s 成功连接上了远程服务器.",
        c->opt->servname, txt);
    str_free(txt);
}
