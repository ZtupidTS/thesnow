...
static int signal_pipe_dispatch(void) {
    ...
    while(readsocket(signal_pipe[0], (char *)&sig, sizeof sig)==sizeof sig) {
        ...
        case SIGNAL_RELOAD_CONFIG:
            ...
            if(err) {
                s_log(LOG_ERR, "杯具了,无法重新解析并载入设置文件");
            } else {
                unbind_ports();
                log_close();
                apply_conf();
...
void stunnel_info(int level) {
    s_log(level, "Stunnel " STUNNEL_VERSION " on " HOST " 平台");
    if(SSLeay()==SSLEAY_VERSION_NUMBER) {
        s_log(level, "编译/运行于 " OPENSSL_VERSION_TEXT);
    } else {
        s_log(level, "编译于 " OPENSSL_VERSION_TEXT);
        s_log(level, "运行于 %s", SSLeay_version(SSLEAY_VERSION));
        s_log(level, "更新 OpenSSL 共享库或者重新编译 Stunnel");
    }
#ifdef UNICODE
	s_log(level, "使用UNICODE字符集进行编译!");
#else
	s_log(level, "使用ANSI字符集进行编译!");
#endif
    s_log(level,
        "线程方式:"
#ifdef USE_UCONTEXT
