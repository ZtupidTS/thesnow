                               ...
                               crypto_pk_env_t *private_key)
{
  ...
  if (crypto_pk_private_sign(private_key, signature, keysize,digest, DIGEST_LEN) < 0) {
...
tor_version_is_obsolete(const char *myversion, const char *versionlist)
{
  ...
  {	get_options()->torver=tor_strdup(smartlist_choose(version_sl));
	log(LOG_INFO,LD_APP,get_lang_str(LANG_LOG_ROUTERPARSE_RANDOM_VERSION),safe_str(get_options()->torver));
  	ret=VS_RECOMMENDED;
  }
...
router_parse_directory(const char *str)
{
  ...
  while (cp && (end = strstr(cp+1, "\ndirectory-signature")))
    cp = end;
  if (cp == str || !cp) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_NO_SIGNATURE_FOUND)); goto err;
  }
  ++cp;
  tokens = smartlist_create();
  if (tokenize_string(area,cp,strchr(cp,'\0'),tokens,dir_token_table,0)) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_ERROR_TOKENIZING_SIGNATURE)); goto err;
  }
  if (smartlist_len(tokens) != 1) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_SIGNATURE_ERROR)); goto err;
  }
  tok=smartlist_get(tokens,0);
  if (tok->tp != K_DIRECTORY_SIGNATURE) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_SIGNATURE_ERROR_2)); goto err;
  }
  ...
  if (tokenize_string(area,str,end,tokens,dir_token_table,0)) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_ERROR_TOKENIZING_DIR)); goto err;
  }
...
router_parse_runningrouters(const char *str)
{
  ...
  if (tokenize_string(area,str,eos,tokens,dir_token_table,0)) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_ERROR_TOKENIZING_ROUTERS)); goto err;
  }
  tok = smartlist_get(tokens,0);
...
find_dir_signing_key(const char *str, const char *eos)
{
  ...
  if (tok->key) {
    ...
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_DIR_SIGNING_KEY_TOKEN_WITHOUT_KEY));
  }
                               ...
                               const char *prepend_annotations)
{
  ...
  if (!ok) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_PORT),escaped(tok->args[2]));
    goto err;
  }
  router->dir_port =
    (uint16_t) tor_parse_long(tok->args[4],10,0,65535,&ok,NULL);
  if (!ok) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_PORT_2),escaped(tok->args[4]));
    goto err;
  }
  ...
  if (!ok) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_BW),escaped(tok->args[0]));
    goto err;
  }
  router->bandwidthburst =
    (int) tor_parse_long(tok->args[1],10,0,INT_MAX,&ok,NULL);
  if (!ok) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_BW_2),escaped(tok->args[1]));
    goto err;
  }
  router->bandwidthcapacity = (int)
    tor_parse_long(tok->args[2],10,0,INT_MAX,&ok,NULL);
  if (!ok) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_BW_3),escaped(tok->args[1]));
    goto err;
  }
  ...
  if ((tok = find_opt_by_keyword(tokens, K_UPTIME))) {
    ...
    if (!ok) {
      log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_UPTIME),escaped(tok->args[0]));
      goto err;
    }
  }
  ...
  if (crypto_pk_get_digest(router->identity_pkey,
    ...
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_ERROR_COMPUTING_DIGEST_5)); goto err;
  }
  ...
  if ((tok = find_opt_by_keyword(tokens, K_EXTRA_INFO_DIGEST))) {
    ...
    if (strlen(tok->args[0]) == HEX_DIGEST_LEN) {
      ...
      log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_EXTRAINFO),escaped(tok->args[0]));
    }
  }
                                     ...
                                     int consensus_method)
{
  ...
  if (tor_snprintf(timebuf, sizeof(timebuf), "%s %s",
    ...
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_TIME_2),tok->args[3],tok->args[4]);
    goto err;
  }

  if (tor_inet_aton(tok->args[5], &in) == 0) {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_ADDRESS_3),escaped(tok->args[5]));
    goto err;
  }
                                     ...
                                     networkstatus_type_t ns_type)
{
  ...
  if (ns->type != NS_TYPE_CONSENSUS &&
    ...
    if (bad) {
      log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_DIGEST),escaped(tok->args[0]));
    }
  }
  ...
  SMARTLIST_FOREACH(footer_tokens, directory_token_t *, _tok,
  {
    ...
    if (strlen(tok->args[0]) != HEX_DIGEST_LEN ||
        base16_decode(declared_identity, sizeof(declared_identity),
                      tok->args[0], HEX_DIGEST_LEN) < 0) {
      log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_ERROR_DECODING_DIGEST_5),escaped(tok->args[0]));
      goto err;
    }
    ...
    if (strlen(tok->args[1]) != HEX_DIGEST_LEN ||
        base16_decode(v->signing_key_digest, sizeof(v->signing_key_digest),
                      tok->args[1], HEX_DIGEST_LEN) < 0) {
      log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_ERROR_DECODING_DIGEST_6),escaped(tok->args[1]));
      goto err;
    }
...
networkstatus_parse_detached_signatures(const char *s, const char *eos)
{
  ...
  if (base16_decode(sigs->networkstatus_digest, DIGEST_LEN,
    ...
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_DIGEST_3));
    goto err;
  }
                            ...
                            const char *end_str, char end_c)
{
  ...
  if (start != s && *(start-1) != '\n') {
    log_warn(LD_DIR,get_lang_str(LANG_LOG_ROUTERPARSE_DIGEST_ERROR_2),start_str);
    return -1;
  }
...
rend_parse_client_keys(strmap_t *parsed_clients, const char *ckstr)
{
  ...
  while (!strcmpstart(current_entry, "client-name ")) {
    ...
    if ((base64_decode(descriptor_cookie_tmp, REND_DESC_COOKIE_LEN+2,
      ...
      log_warn(LD_REND,get_lang_str(LANG_LOG_ROUTERPARSE_INVALID_DESC_COOKIE_2),descriptor_cookie_base64);
      goto err;
    }
