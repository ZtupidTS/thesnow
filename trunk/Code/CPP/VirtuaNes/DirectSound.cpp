...
//
// WaveFileÇÃÉçÅ[ÉhÇ∆ÉÅÉÇÉäÇ÷ÇÃï€éù
//
CWaveData::CWaveData()
{
	m_pBuffer = NULL;
...
//				      | DSBCAPS_PRIMARYBUFFER;
		...
		ReleaseDSound();
		::MessageBoxA( hWnd, str, "¥ÌŒÛ", MB_ICONERROR|MB_OK );
...
WAVEFORMATEX	pcmwf;
	...
	try {
		...
		// ÉvÉâÉCÉ}ÉäÉoÉbÉtÉ@ÇÃWaveÉtÉHÅ[É}ÉbÉgÇê›íË(èÌÇ…ÉÇÉmÉâÉã)
		...
		// ÉXÉgÉäÅ[ÉÄÉZÉJÉìÉ_ÉäÉoÉbÉtÉ@çÏê¨
		if( m_BufferSize < 2 )
			m_BufferSize = 2;

		// ÉoÉbÉtÉ@ÉTÉCÉYìôÇÃåvéZ
		...
		::MessageBoxA( m_hWnd, str, "¥ÌŒÛ", MB_ICONERROR|MB_OK );
...
DWORD	dwBytes0, dwBytes1;
	...
	// ä˘Ç…égÇ¡ÇƒÇ¢ÇΩÇÁäJï˙ÇµÇƒÇ®Ç≠
	if( m_pEsfDSBuffer[no] ) {
		RELEASE( m_pEsfDSBuffer[no] );
	}

	// DirectSound ÉZÉJÉìÉ_ÉäÉoÉbÉtÉ@çÏê¨
	ZEROMEMORY( &dsbdesc, sizeof(DSBUFFERDESC) );
	dsbdesc.dwSize        = sizeof(DSBUFFERDESC);
	dsbdesc.dwFlags       = DSBCAPS_LOCSOFTWARE;
	...
	// çÏê¨ÇµÇΩÉZÉJÉìÉ_ÉäÉoÉbÉtÉ@Ç…ÉEÉFÅ[ÉuÉfÅ[É^ÇÉRÉsÅ[
