...
rend_add_service(rend_service_t *service)
{
  ...
  if (service->descriptor_version == -1) {
    if (service->auth_type == REND_NO_AUTH) {
      ...
      strlcpy(v0_service->plugin,service->plugin,MAX_PATH);
  ...
  if (service->auth_type != REND_NO_AUTH && !service->descriptor_version) {
    log_warn(LD_CONFIG,get_lang_str(LANG_LOG_RENDSERVICE_HS_WITH_AUTH_AND_V0));
...
parse_port_config(const char *string,int is_dll)
{
  smartlist_t *sl;
  int virtport;
  int realport;
    ...
    if (!strcasecmp(line->key, "HiddenServicePort")) {
      ...
      for (i = 0; i < smartlist_len(versions); i++) {
        ...
        if (strlen(version_str) != 1 || strspn(version_str, "02") != 1) {
          log_warn(LD_CONFIG,get_lang_str(LANG_LOG_RENDSERVICE_INVALID_HS_VERSION));
  ...
  if (old_service_list && !validate_only) {
    ...
    for (circ = _circuit_get_global_list(); circ; circ = circ->next) {
      if (!circ->marked_for_close &&
        ...
        if (keep_it)
          continue;
        log_info(LD_REND,get_lang_str(LANG_LOG_RENDSERVICE_CLOSING_INTRO_POINT),safe_str(oc->build_state->chosen_exit->nickname),oc->rend_data->onion_address,oc->rend_data->rend_desc_version);
...
rend_service_load_keys(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(rend_service_list, rend_service_t *, s) {
    ...
    /* Load key */
    if (strlcpy(fname,fullpath,sizeof(fname))>=sizeof(fname) || strlcat(fname,"-",sizeof(fname)) >= sizeof(fname) ||
    	strlcat(fname,s->directory,sizeof(fname)) >= sizeof(fname) ||
        strlcat(fname,"-private_key",sizeof(fname))
                                                  >= sizeof(fname)) {
    ...
    if (strlcpy(fname,fullpath,sizeof(fname))>=sizeof(fname) || strlcat(fname,"-",sizeof(fname)) >= sizeof(fname) ||
    	strlcat(fname,s->directory,sizeof(fname)) >= sizeof(fname) ||
        strlcat(fname,"-hostname",sizeof(fname))
                                                  >= sizeof(fname)) {
    ...
    if (s->auth_type != REND_NO_AUTH) {
      ...
      if (tor_snprintf(cfname, sizeof(cfname), "%s-%s-client_keys",fullpath,
                       s->directory)<0) {
                       ...
                       size_t request_len)
{
  ...
  r = crypto_pk_private_hybrid_decrypt(
       intro_key,buf,sizeof(buf),(char*)(request+DIGEST_LEN),request_len-DIGEST_LEN,
  ...
  if (*buf == 3) {
    ...
    if (((now - ts) < -1 * REND_REPLAY_TIME_INTERVAL / 2 ||
        (now - ts) > REND_REPLAY_TIME_INTERVAL / 2)&&!get_options()->DirFlags&16) {
  ...
  *access_time = now;
  ...
  rep_hist_note_used_internal(get_time(NULL), circ_needs_uptime, 1);
  ...
  for (i=0;i<get_options()->MaxRendFailures;i++) {
    ...
    launched = circuit_launch_by_extend_info(
                        CIRCUIT_PURPOSE_S_CONNECT_REND, extend_info, flags,1);
  ...
  cpath->hItem=NULL;
  launched->build_state->expiry_time = get_time(NULL) + get_options()->MaxRendTimeout;
...
rend_service_relaunch_rendezvous(origin_circuit_t *oldcirc)
{
  ...
  if (!oldcirc->build_state ||
    ...
    log_info(LD_REND,get_lang_str(LANG_LOG_RENDSERVICE_CIRC_BUILD_FAILED),oldcirc->build_state ?oldcirc->build_state->chosen_exit->nickname : "*unknown*");
    return;
  }
  ...
  log_info(LD_REND,get_lang_str(LANG_LOG_RENDSERVICE_REATTEMPTING_REND),oldstate->chosen_exit->nickname);
  ...
  if (!newcirc) {
    log_warn(LD_REND,get_lang_str(LANG_LOG_RENDSERVICE_ERROR_RELAUNCHING_CIRC),oldstate->chosen_exit->nickname);
    return;
  }
                                    ...
                                    rend_intro_point_t *intro)
{
  ...
  log_info(LD_REND,get_lang_str(LANG_LOG_RENDSERVICE_LAUNCHING_CIRC_TO_INTRO_POINT),escaped_safe_str(intro->extend_info->nickname),service->service_id);
  ...
  if (!launched) {
    log_info(LD_REND,get_lang_str(LANG_LOG_RENDSERVICE_ERROR_RELAUNCHING_CIRC_2),escaped_safe_str(intro->extend_info->nickname));
    return -1;
  }
...
rend_service_intro_has_opened(origin_circuit_t *circuit)
{
  ...
  if (count_established_intro_points(serviceid,
          circuit->rend_data->rend_desc_version) > get_options()->NumIntroPoints) {
                         ...
                         int seconds_valid)
{
  ...
  for (i = 0; i < smartlist_len(descs); i++) {
    ...
    for (j = 0; j < smartlist_len(responsible_dirs); j++) {
      ...
      log_info(LD_REND,get_lang_str(LANG_LOG_RENDSERVICE_PUBLISH_REQUEST),safe_str(service_id),safe_str(desc_id_base32),seconds_valid,hs_dir->nickname,hs_dir->dir_port);
...
upload_service_descriptor(rend_service_t *service)
{
  ...
  if (service->descriptor_version == 0 &&
    ...
    if (rend_encode_service_descriptor(service->desc,
                                       service->private_key,
                                       &desc, &desc_len)<0) {
      log_warn(LD_BUG,get_lang_str(LANG_LOG_RENDSERVICE_INTERNAL_ERROR_7));
      return;
    }
  ...
  for (i=0; i < smartlist_len(rend_service_list); ++i) {
    ...
    for (j=0; j < smartlist_len(service->intro_nodes); ++j) {
      ...
      if (!router || !find_intro_circuit(intro, service->pk_digest,
        ...
        log_info(LD_REND,get_lang_str(LANG_LOG_RENDSERVICE_GIVING_UP_ON_INTRO),intro->extend_info->nickname,service->service_id);
        if (service->desc) {
...
long rend_add_new_service(char *realPorts,char *virtualPorts,char *hostname,char *onionAddress)
{
	...
	{	char *s1=strrchr(onionAddress,'.');
		...
		char buf[1500];
		SMARTLIST_FOREACH_BEGIN(rend_service_list, rend_service_t *, s)
		{	if(!strcasecmp(onionAddress,s->service_id))
			{	if(s->private_key)
				{	if(strlcpy(fname1,fullpath,sizeof(fname1))>=sizeof(fname1) || strlcat(fname1,"-",sizeof(fname1)) >= sizeof(fname1) ||
						...
						log_warn(LD_CONFIG,get_lang_str(LANG_LOG_RENDSERVICE_DIR_NAME_TOO_LONG_1),s->directory);
					tor_snprintf(buf, sizeof(buf),"%s.onion\n", s->service_id);
					if(write_str_to_file(fname1,buf,0)<0)
						log_warn(LD_CONFIG,get_lang_str(LANG_LOG_RENDSERVICE_ERROR_WRITING_ONION_ADDRESS));
					if(strlcpy(fname1,fullpath,sizeof(fname1))>=sizeof(fname1) || strlcat(fname1,"-",sizeof(fname1)) >= sizeof(fname1) ||
						strlcat(fname1,service->directory,sizeof(fname1)) >= sizeof(fname1) ||
						strlcat(fname1,"-private_key",sizeof(fname1))
						>= sizeof(fname1)) ;
					crypto_pk_write_private_key_to_filename(s->private_key,fname1);
		...
		if(!service->private_key)
		{	tor_snprintf(service->service_id,REND_SERVICE_ID_LEN_BASE32+1,onionAddress);
			getHSKey(service);
			if(!service->private_key)
			{	rend_service_free(service);
				return 0;
	...
	service=find_service_by_key(lastKey|(2<<16));
	if(service) insertService(service);
	return lastKey;
}
...
{	char fname1[512];
	...
	if(strlcpy(fname1,fullpath,sizeof(fname1))>=sizeof(fname1) || strlcat(fname1,"-",sizeof(fname1)) >= sizeof(fname1) ||
						...
						strlcat(fname1,"-private_key",sizeof(fname1))
						>= sizeof(fname1)) ;
	keyfile=fopen(fname1,"wb");
	if(keyfile)
	{	fwrite(keystr,1,strlen(keystr),keyfile);
		fclose(keyfile);
		service->private_key = init_key_from_file(fname1,0,LOG_DEBUG);
		if(service->private_key)
		{	if(rend_get_service_id(service->private_key, service->service_id)<0)
...
{	char dirStr[20];
	unsigned int tmpKey=key&0xffff;
	unsigned int tmpVer=(key>>16)&0xffff;
	tor_snprintf(dirStr,20,"%i",tmpKey);
	SMARTLIST_FOREACH(rend_service_list, rend_service_t*, ptr,
		if(!strcmp(ptr->directory,dirStr)&&(((unsigned int)ptr->descriptor_version==tmpVer)||(tmpVer==0xffff))) return ptr;
	);
	return NULL;
}
...
{	config_line_t *line;
	...
	char fname[512];
	for(line = options->RendConfigLines; line&&line->next; line = line->next)
	{	if(!strcasecmp(line->key,"HiddenServiceKey") && !strcasecmp(line->value,key))	return;
	}
...
{	config_line_t *line,*newLine,*oldLine;
	char dirStr[20];
	unsigned int tmpKey=key&0xffff;
	unsigned int tmpVer=(key>>16)&0xffff;
	tor_snprintf(dirStr,20,"%i",tmpKey);
	...
	{	if(!strcasecmp(line->key,"HiddenServiceKey") && !strcasecmp(line->value,dirStr))
		{	tor_snprintf(dirStr,20,"%i",tmpVer);
			for(newLine=line->next;newLine;newLine=newLine->next)
			{	if(!strcasecmp(newLine->key,"HiddenServiceVersion"))
				{ 	if(!strcasecmp(newLine->value,dirStr))
					{	for(newLine=options->RendConfigLines;newLine&&newLine!=line&&newLine->next!=line;newLine=newLine->next);
						if(line==newLine)
						{	while(line)
							{	oldLine=line->next;
							...
							remove_service(key);tor_snprintf(dirStr,20,"%i",tmpKey);delete_serv_files(dirStr);
							return 1;
						}
						else if(newLine)
						{	while(line)
					...
					else if(!strcasecmp(newLine->value,"-1"))
					{	char *oldVal=newLine->value;
						if(tmpVer==0) newLine->value=tor_strdup("2");
						else newLine->value=tor_strdup("0");
						tor_free(oldVal);
						remove_service(key);tor_snprintf(dirStr,20,"%i",tmpKey);delete_serv_files(dirStr);
						return 1;
					}
				}
				else if(!strcasecmp(newLine->key,"HiddenServiceKey")) break;
			}
			newLine=tor_malloc_zero(sizeof(config_line_t));
			newLine->key=tor_strdup("HiddenServiceVersion");
			newLine->value=(tmpVer==0)?tor_strdup("2"):tor_strdup("0");
			newLine->next=line->next;line->next=newLine;
			remove_service(key);tor_snprintf(dirStr,20,"%i",tmpKey);delete_serv_files(dirStr);
			return 1;
		}
	}
