		...
		if( ret < 0 ) {
			...
			switch( event ) {
				...
				case	0x88:
					...
					break;
				case	0xF0:
					if( g_nes->LoadState( (LPWSTR)strNetStateName.c_str() ) ) {
						::wsprintf( (LPWSTR)szMes, L"Net State Load." );
						DirectDraw.SetMessageString( szMes );
					}
					break;
				case	0xF1:
					if( g_nes->SaveState( (LPWSTR)strNetStateName.c_str() ) ) {
						::wsprintfA( szMes, "Net State Save." );
						DirectDraw.SetMessageString( szMes );
					}
					break;
				default:
			...
			switch( Ev ) {
				...
				case	EV_NETPLAY_START:
					if( g_nes ) {
						...
						DEBUGOUT( "Path: %s\n", pathstr.c_str() );
							...
							pathstr =(LPWSTR)g_nes->rom->GetRomPath();
						}
						strNetStateName = CPathlib::MakePathExt( (LPSTR)pathstr.c_str(), (LPSTR)g_nes->rom->GetRomName(), "stn" );
				...
				case	EV_FRAMESKIP_UP:
					if( !NetPlay.IsConnect() ) {
						if( nFrameSkip < 20 )
							nFrameSkip++;
						::wsprintfA( szStr, "FrameSkip %d", nFrameSkip );
						...
						if( !NetPlay.IsConnect() ) {
							if( g_nes->LoadState( (const WCHAR*)Param ) ) {
								if( Param2 < 0 )
									...
									::wsprintfA( szStr, "State Load #%d", Param2 );
								DirectDraw.SetMessageString( szStr );
							}
						} else {
						...
						if( !NetPlay.IsConnect() ) {
							if( g_nes->SaveState( (const WCHAR*)Param ) ) {
								if( Param2 < 0 )
									...
									::wsprintfA( szStr, "State Save #%d", Param2 );
								DirectDraw.SetMessageString( szStr );
							}
						} else {
				...
				case	EV_SOUND_MUTE:
					...
					if( g_nes )
						if( g_nes->CommandParam( NES::NESCMD_SOUND_MUTE, Param ) ) {
							::wsprintfA( szStr, "%s Enable.", g_lpSoundMuteStringTable[Param] );
						} else {
							::wsprintfA( szStr, "%s Mute.", g_lpSoundMuteStringTable[Param] );
						}
						DirectDraw.SetMessageString( szStr );
					}
					::SetEvent( g_hEventAccept );
