...
//			}
			...
			CHAR	szStr[64];
			CApp::LoadString( m_HeaderID[Config.launcher.nHeaderOrder[i]], (LPTSTR)szStr, sizeof(szStr) );
...
void	CLauncherDlg::SetListView( INT index, FILELIST& fl )
{
	...
	if( Config.launcher.bHeaderView[0] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[0], (LPTSTR)fl.fname.c_str() );
	}
	// Path
	if( Config.launcher.bHeaderView[1] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[1], (LPTSTR)fl.path.c_str() );
	}
	// Mapper
	if( Config.launcher.bHeaderView[2] ) {
		...
		if( Mapper == 20 ) {
			...
			::wsprintfA( szStr, "%d", fl.mapper&0xFF );
			if( fl.mapper&0xF000 )
				image = 2;
			else
				image = 1;
		}
		ListView_SetItemText( hWndCtrl, index, RealOrder[2], (LPTSTR)szStr );
	...
	if( Config.launcher.bHeaderView[3] ) {
		...
		// FDS
			::wsprintfA( szStr, "%d", fl.prg_size );
		} else if( (fl.mapper&0x0FFF) == 0x100 ) {
		// NSF
			::wsprintfA( szStr, "%d", fl.prg_size );
		} else if( (fl.mapper&0x0FFF) < 0x100 ) {
		// NES
			::wsprintfA( szStr, "%d", 16*fl.prg_size );
		} else {
		// Bad file
			::strcpy( szStr, "0" );
		}
		ListView_SetItemText( hWndCtrl, index, RealOrder[3], (LPTSTR)szStr );
	}
	// CHR size
	if( Config.launcher.bHeaderView[4] ) {
		if( (fl.mapper&0x0FFF) == 0x100 ) {
		// NSF
			::wsprintfA( szStr, "%d", fl.chr_size );
		} else if( (fl.mapper&0x0FFF) < 0x100 ) {
		// NES
			::wsprintfA( szStr, "%d", 8*fl.chr_size );
		}
//		::wsprintf( szStr, "%d", 8*fl.chr_size );
		ListView_SetItemText( hWndCtrl, index, RealOrder[4], (LPTSTR)szStr );
	}
	// CRC ALL
	if( Config.launcher.bHeaderView[5] ) {
		::wsprintfA( szStr, "%08X", fl.crcall );
		ListView_SetItemText( hWndCtrl, index, RealOrder[5], (LPTSTR)szStr );
	}
	// CRC
	if( Config.launcher.bHeaderView[6] ) {
		::wsprintfA( szStr, "%08X", fl.crc );
		ListView_SetItemText( hWndCtrl, index, RealOrder[6], (LPTSTR)szStr );
	}
	// Info
	if( Config.launcher.bHeaderView[7] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[7], (LPTSTR)fl.info.c_str() );
	}
	// DB
	if( Config.launcher.bHeaderView[8] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[8], (LPTSTR)fl.db.c_str() );
	}
	// TITLE
	if( Config.launcher.bHeaderView[9] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[9], (LPTSTR)fl.title.c_str() );
	}
	// Country
	if( Config.launcher.bHeaderView[10] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[10], (LPTSTR)fl.country.c_str() );
	}
	// Manufacturer
	if( Config.launcher.bHeaderView[11] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[11], (LPTSTR)fl.manufacturer.c_str() );
	}
	// Sale date
	if( Config.launcher.bHeaderView[12] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[12], (LPTSTR)fl.saledate.c_str() );
	}
	// Price
	if( Config.launcher.bHeaderView[13] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[13], (LPTSTR)fl.price.c_str() );
	}
	// Genre
	if( Config.launcher.bHeaderView[14] ) {
		ListView_SetItemText( hWndCtrl, index, RealOrder[14], (LPTSTR)fl.genre.c_str() );
	}
}
...
DEBUGOUT( "Path:%s\n", Path.c_str() );
		...
		if( dlg.DoModal( m_hWnd ) == IDOK ) {
			...
			if( header.control1 != control1 || header.control2 != control2 ) {
				CHAR	szStr[256];
				CApp::LoadString( IDS_LCH_HEADERREWRITE, (LPTSTR)szStr, sizeof(szStr) );

				if( ::MessageBoxA( m_hWnd, szStr, "", MB_YESNO ) == IDYES ) {
...
//	string	Path = CPathlib::MakePathExt( CApp::GetModulePath(), "launcher", "lst" );
	string	Path = CPathlib::MakePath( (LPCSTR)CApp::GetModulePath(), "launcher" );
	if( m_nListSelect ) {
...
//	DEBUGOUT( "CLchFolderConfigDlg::OnOK\n" );
	...
	INT	i;
	for( i = 0; i < nCount; i++ ) {
		ListView_GetItemText( hWndCtrl, i, 0,(LPTSTR) Config.launcher.szFolder[i], sizeof(Config.launcher.szFolder[i]) );
...
DLGCMD	CLchFolderConfigDlg::OnAdd( DLGCMDPARAM )
{
	...
	if( CPathlib::SelectFolder( m_hWnd, (LPSTR)szTitle, szFolder ) ) {
		LV_ITEMA	lvitem;
		lvitem.mask = LVIF_TEXT;
...
//	DEBUGOUT( "CLchHeaderEditDlg::OnOK\n" );
	...
	if( m_nMapperNo < 0 || m_nMapperNo > 255 ) {
		...
		::MessageBox( m_hWnd, szStr, L"ERROR", MB_ICONERROR|MB_OK );
		return;
	}
