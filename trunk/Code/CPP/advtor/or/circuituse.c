...
circuit_expire_building(time_t now)
{
  ...
  while (circ) {
    ...
    if (victim->purpose == CIRCUIT_PURPOSE_C_INTRODUCING &&
      ...
      log_info(LD_REND|LD_CIRC,get_lang_str(LANG_LOG_CIRCUITUSE_INTRO_TIMEOUT));

    if (victim->purpose >= CIRCUIT_PURPOSE_C_INTRODUCING &&
        victim->purpose <= CIRCUIT_PURPOSE_C_REND_READY_INTRO_ACKED) {
      if (!victim->timestamp_dirty)
        log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITUSE_CONSIDERING_CIRCUIT),victim->state == CIRCUIT_STATE_OPEN ? "" : "non",victim->purpose, victim->build_state->chosen_exit_name,victim->n_circ_id);
      else
        log_fn(LOG_DEBUG,get_lang_str(LANG_LOG_CIRCUITUSE_CONSIDERING_DIRTY_CIRCUIT),victim->state == CIRCUIT_STATE_OPEN ? "" : "non",victim->purpose, victim->build_state->chosen_exit_name,victim->n_circ_id,(int)(now - victim->timestamp_dirty));
    }
#endif
...
//      log_debug(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITUSE_PORT_ALREADY_HANDLED), port);
      smartlist_del(needed_ports, i--);
      tor_free(port);
    } else {
      log_debug(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITUSE_PORT_NOT_HANDLED), *port);
    }
  }
}
...
#ifdef int3
         ((get_options()->MaxCircuitDirtiness)&&(circ->timestamp_dirty + get_options()->MaxCircuitDirtiness > now))))
#else
         circ->timestamp_dirty + get_options()->MaxCircuitDirtiness > now))
#endif
...
circuit_build_needed_circs(time_t now)
{
  ...
  if (time_to_new_circuit < now) {
    ...
    if (get_options()->RunTesting &&
      ...
      log_fn(LOG_INFO,get_lang_str(LANG_LOG_CIRCUITUSE_CREATING_NEW_TESTING_CIRCUIT));
...
circuit_expire_old_circuits(time_t now)
{
  ...
  for (circ = global_circuitlist; circ; circ = circ->next) {
	...
	get_options()->MaxCircuitDirtiness &&
      ...
      log_debug(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITUSE_CLOSING_N_CIRC_ID),circ->n_circ_id, (int)(now - circ->timestamp_dirty),circ->purpose);
      circuit_mark_for_close(circ, END_CIRC_REASON_FINISHED);
    } else if (!circ->timestamp_dirty &&
               circ->state == CIRCUIT_STATE_OPEN &&
               circ->purpose == CIRCUIT_PURPOSE_C_GENERAL) {
      if (circ->timestamp_created < cutoff) {
        log_debug(LD_CIRC,get_lang_str(LANG_LOG_CIRCUITUSE_CLOSING_UNUSED_CIRCUIT),(int)(now - circ->timestamp_created));
        circuit_mark_for_close(circ, END_CIRC_REASON_FINISHED);
      }
    }
  }
...
//  if ((extend_info || purpose != CIRCUIT_PURPOSE_C_GENERAL) &&
  if ((purpose != CIRCUIT_PURPOSE_C_GENERAL) &&
    ...
    if (circ) {
       ...
       * began. */
      circ->_base.timestamp_created = get_time(NULL);
      switch (purpose) {
        case CIRCUIT_PURPOSE_C_ESTABLISH_REND:
        case CIRCUIT_PURPOSE_S_ESTABLISH_INTRO:
                                ...
                                origin_circuit_t **circp)
{
  ...
  if (!circ)
    log_info(LD_APP,get_lang_str(LANG_LOG_CIRCUITUSE_NO_SAFE_CIRCUIT),desired_circuit_purpose);
  *circp = circ;
  return 0;
}
