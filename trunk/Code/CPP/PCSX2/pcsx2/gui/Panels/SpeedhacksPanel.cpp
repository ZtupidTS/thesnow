	...
	: BaseApplicableConfigPanel( parent )
{
	...
	m_check_Enable->SetToolTip(_("为了更好的兼容性,可能需要关闭游戏加速."));
	
	m_button_Defaults = new wxButton( this, wxID_DEFAULT, _("还原默认") );
	pxSetToolTip( m_button_Defaults, _("还原所有游戏加速选项到默认,相当于关闭所有项目.") );
	...
	wxPanelWithHelpers* eeSliderPanel = new wxPanelWithHelpers( left, wxVERTICAL, _("EE 循环速率") );
	...
	wxPanelWithHelpers* vuSliderPanel = new wxPanelWithHelpers( right, wxVERTICAL, _("VU 循环速率欺骗") );
	...
	wxPanelWithHelpers* vuHacksPanel = new wxPanelWithHelpers( right, wxVERTICAL, _("microVU 破解") );

	m_check_vuFlagHack = new pxCheckBox( vuHacksPanel, _("mVU Flag 破解"),
	...
	wxPanelWithHelpers* miscHacksPanel = new wxPanelWithHelpers( left, wxVERTICAL, _("其它破解") );

	m_check_intc = new pxCheckBox( miscHacksPanel, _("启用 INTC Spin 检测"),
		_("Huge speedup for some games, with almost no compatibility side effects. [推荐]") );

	m_check_b1fc0 = new pxCheckBox( miscHacksPanel, _("启用 BIFC0 Spin 检测"),
		_("Moderate speedup for some games, with no known side effects. [推荐]" ) );
	
	m_check_IOPx2 = new pxCheckBox( miscHacksPanel, _("IOP x2 cycle rate 破解"),
