...
protected:
	wchar_t *s;				///< The C string
	lenpos_t sSize;			///< The size of the buffer, less 1: ie. the maximum size of the string

	SContainerW() : s(0), sSize(0) {}
	~SContainerW() {
		delete []s;	// Suppose it was allocated using StringAllocate
		s = 0;
		sSize = 0;
...
public:
	 ...
	 * @return the pointer to the new string
	 */
	static wchar_t *StringAllocate(lenpos_t len);
	/**
	 * Duplicate a buffer/C string.
	 * Allocate memory of the given size, or big enough to fit the string if length isn't given;
	 * then copy the given string in the allocated memory.
	 * @return the pointer to the new string
	 */
	static wchar_t *StringAllocate(
		const wchar_t *s,			///< The string to duplicate
		lenpos_t len=measure_length);	///< The length of memory to allocate. Optional.
};
 ...
 * we should set sLen to strlen(sb.ptr()) in related SString constructor and assignment.
 */
class SBufferW : protected SContainerW {
public:
	SBufferW(lenpos_t len) {
		s = StringAllocate(len);
		if (s) {
			*s = L'\0';
		sSize = len;
...
private:
	/// Copy constructor
	// Here only to be on the safe size, user should avoid returning SBufferW values.
	SBufferW(const SBufferW &source) : SContainerW() {
		s = StringAllocate(source.s, source.sSize);
		sSize = (s) ? source.sSize : 0;
	}
	/// Default assignment operator
	// Same here, shouldn't be used
	SBufferW &operator=(const SBufferW &source) {
		if (this != &source) {
			delete []s;
			s = StringAllocate(source.s, source.sSize);
			sSize = (s) ? source.sSize : 0;
 ...
 * functions to allow reliable manipulations of these strings, other than simple appends, etc.
 */
class SStringW : protected SContainerW {
	lenpos_t sLen;			///< The size of the string in s
	lenpos_t sizeGrowth;	///< Minimum growth size when appending strings
	enum { sizeGrowthDefault = 64 };

	bool grow(lenpos_t lenNew);
	SStringW &assign(const wchar_t *sOther, lenpos_t sSize_=measure_length);

public:

	SStringW() : sLen(0), sizeGrowth(sizeGrowthDefault) {}
	SStringW(const SStringW &source) : SContainerW(), sizeGrowth(sizeGrowthDefault) {
		s = StringAllocate(source.s, source.sLen);
		sSize = sLen = (s) ? source.sLen : 0;
	}
	SStringW(const wchar_t *s_) : sizeGrowth(sizeGrowthDefault) {
		s = StringAllocate(s_);
		sSize = sLen = (s) ? wcslen(s) : 0;
	}
	SStringW(SBufferW &buf) : sizeGrowth(sizeGrowthDefault) {
		s = buf.ptr();
		sSize = sLen = buf.size();
		// Consumes the given buffer!
		buf.reset();
	}
	SStringW(const wchar_t *s_, lenpos_t first, lenpos_t last) : sizeGrowth(sizeGrowthDefault) {
		// note: expects the "last" argument to point one beyond the range end (a la STL iterators)
		s = StringAllocate(s_ + first, last - first);
		sSize = sLen = (s) ? last - first : 0;
	}
	SStringW(int i);
	SStringW(double d, int precision);
	~SStringW() {
		sLen = 0;
	}
	void clear() {
	...
	/** Read access to a character of the string. */
	wchar_t operator[](lenpos_t i) const {
		return (s && i < sSize) ? s[i] : L'\0';
	}
	SStringW &operator=(const wchar_t *source) {
		return assign(source);
	}
	SStringW &operator=(const SStringW &source) {
		if (this != &source) {
			assign(source.s, source.sLen);
		}
		return *this;
	}
	bool operator==(const SStringW &sOther) const;
	bool operator!=(const SStringW &sOther) const {
		return !operator==(sOther);
	}
	bool operator==(const wchar_t *sOther) const;
	bool operator!=(const wchar_t *sOther) const {
		return !operator==(sOther);
	}
	bool contains(wchar_t ch) const {
		return (s && *s) ? wcschr(s, ch) != 0 : false;
	}
	void setsizegrowth(lenpos_t sizeGrowth_) {
		sizeGrowth = sizeGrowth_;
	}
	...
	char* SStringWtoA(SStringW *origs);
	SStringW substr(lenpos_t subPos, lenpos_t subLen=measure_length) const;
	SStringW &lowercase(lenpos_t subPos = 0, lenpos_t subLen=measure_length);
	SStringW &uppercase(lenpos_t subPos = 0, lenpos_t subLen=measure_length);
	SStringW &append(const wchar_t *sOther, lenpos_t sLenOther=measure_length, wchar_t sep = L'\0');
	SStringW &operator+=(const wchar_t *sOther) {
		return append(sOther, static_cast<lenpos_t>(measure_length));
	}
	SStringW &operator+=(const SStringW &sOther) {
		return append(sOther.s, sOther.sLen);
	}
	SStringW &operator+=(wchar_t ch) {
		return append(&ch, 1);
	}
	SStringW &appendwithseparator(const wchar_t *sOther, wchar_t sep) {
		return append(sOther, wcslen(sOther), sep);
	}
	SStringW &insert(lenpos_t pos, const wchar_t *sOther, lenpos_t sLenOther=measure_length);
	...
	SStringW &change(lenpos_t pos, wchar_t ch) {
		if (pos < sLen) {					// character changed must be in string bounds
			*(s + pos) = ch;
		}
		return *this;
	...
	bool endswith(const wchar_t *suffix);
	int search(const wchar_t *sFind, lenpos_t start=0) const;
	bool contains(const wchar_t *sFind) const {
		return search(sFind) >= 0;
	}
	int substitute(wchar_t chFind, wchar_t chReplace);
	int substitute(const wchar_t *sFind, const wchar_t *sReplace);
	int remove(const wchar_t *sFind) {
		return substitute(sFind, L"");
	}
};
 ...
 * @return the pointer to the new string
 */
inline wchar_t *StringDup(
	const wchar_t *s,			///< The string to duplicate
	SContainerW::lenpos_t len=SContainerW::measure_length)	///< The length of memory to allocate. Optional.
{
	return SContainerW::StringAllocate(s, len);
}

bool isprefix(const wchar_t *target, const wchar_t *prefix);
int CompareNoCase(const wchar_t *a, const wchar_t *b);
bool EqualCaseInsensitive(const wchar_t *a, const wchar_t *b);
