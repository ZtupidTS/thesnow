	...
	: BaseAdvancedCpuOptions( parent )
{
	...
	AddFrame(_("EE/FPU 高级重编译选项"));

	m_ClampModePanel->Append(_("附加 + 保留签名"));
	m_ClampModePanel->Append(_("完全"));
	...
	: BaseAdvancedCpuOptions( parent )
{
	...
	AddFrame(_("VU0 / VU1 高级重编译选项"));

	m_ClampModePanel->Append(_("附加"));
	m_ClampModePanel->Append(_("附加 + 保留签名"));
	...
	: BaseApplicableConfigPanel_SpecificConfig( parent )
{
	*this	+= Text( pxE( "!Panel:EE/IOP:Heading",
	...
	wxStaticBoxSizer& s_ee	( *new wxStaticBoxSizer( wxVERTICAL, this, L"执行引擎" ) );
