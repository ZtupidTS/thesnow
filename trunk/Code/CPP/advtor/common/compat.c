...
tor_mmap_file(const char *filename)
{
  ...
  if (fd<0) {
    ...
    log_fn(severity,LD_FS,get_lang_str(LANG_LOG_COMPAT_ERROR_OPENING_FILE),filename,strerror(errno));
    errno = save_errno;
    return NULL;
  }
  ...
  if (string == MAP_FAILED) {
    int save_errno = errno;
    log_warn(LD_FS,get_lang_str(LANG_LOG_COMPAT_COULD_NOT_MMAP_FILE),filename,strerror(errno));
    errno = save_errno;
    return NULL;
  }
...
#define UNITS_PER_USEC U64_LITERAL(10)
  ...
  ft.ft_64 -= EPOCH_BIAS;
  ft.ft_64 += get_time(NULL)-time(NULL);
