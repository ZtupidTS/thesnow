...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	SetStatusbarText(m_wndStatusBar.CommandToIndex(ID_INDICATOR_SENDCOUNT), _T("0 字节发送"));
	SetStatusbarText(m_wndStatusBar.CommandToIndex(ID_INDICATOR_RECVCOUNT), _T("0 字节接收"));
	...
	case 7:
		...
		else
		{
			...
			if (!nType)
			{
				...
				m_RecvLed.Ping(100);
				CString str;
				str.Format(_T("已接收 %s 字节"), makeUserFriendlyString(m_nRecvCount).GetString());
			...
			else
			{
				...
				m_SendLed.Ping(100);
				CString str;
				str.Format(_T("已发送 %s 字节"), makeUserFriendlyString(m_nSendCount).GetString());
...
void CMainFrame::OnFileConnect() 
{
	...
	if (m_pAdminSocket)
		if (AfxMessageBox(_T("管理控制台只能同时工作于一个连接,是否关闭当前连接?"), MB_ICONQUESTION|MB_YESNO) != IDYES)
			return;
	CConnectDialog dlg(m_pOptions);
	if (dlg.DoModal() == IDOK)
	{
