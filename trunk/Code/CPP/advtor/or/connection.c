...
_connection_free(connection_t *conn)
{
  ...
  if (conn->s >= 0) {
    log_debug(LD_NET,get_lang_str(LANG_LOG_CONNECTION_CLOSING_FD),conn->s);
...
connection_close_immediate(connection_t *conn)
{
  ...
  if (conn->outbuf_flushlen) {
    log_info(LD_NET,get_lang_str(LANG_LOG_CONNECTION_CONN_STATS),conn->s, conn_type_to_string(conn->type),conn_state_to_string(conn->type, conn->state),(int)conn->outbuf_flushlen);
  }
    ...
    if (conn->hold_open_until_flushed) {
      ...
      if (now - conn->timestamp_lastwritten >= 15) {
        ...
        if (conn->type == CONN_TYPE_EXIT ||
          ...
          severity = LOG_NOTICE;
        log_fn(severity, LD_NET,get_lang_str(LANG_LOG_CONNECTION_WRITE_TIMEOUT_BEFORE_CLOSE),conn->s, conn_type_to_string(conn->type),conn_state_to_string(conn->type, conn->state));
...
#define WARN_TOO_MANY_CONNS_INTERVAL (6*60*60)
  ...
  if (last_warned + WARN_TOO_MANY_CONNS_INTERVAL < now) {
    log_warn(LD_NET,get_lang_str(LANG_LOG_CONNECTION_TOO_MANY_CONNS), n_conns);
    last_warned = now;
  }
  ...
  if (listensockaddr->sa_family == AF_INET) {
    ...
    if (bind(s,listensockaddr,socklen) < 0) {
      ...
      if (ERRNO_IS_EADDRINUSE(e))
        helpfulhint = get_lang_str(LANG_LOG_ROUTER_TOR_RUNNING);
    ...
    log_notice(LD_NET,get_lang_str(LANG_LOG_CONNECTION_OPENING_LISTENER_2),conn_type_to_string(type), address);

    if (unlink(address) < 0 && errno != ENOENT) {
      log_warn(LD_NET,get_lang_str(LANG_LOG_CONNECTION_COULD_NOT_UNLINK),address,strerror(errno));
      goto err;
    }
    s = tor_open_socket(AF_UNIX, SOCK_STREAM, 0);
    if (s < 0) {
      log_warn(LD_NET,get_lang_str(LANG_LOG_CONNECTION_SOCKET_CREATION_FAILED_2),strerror(errno));
      goto err;
    }

    if (bind(s, listensockaddr, (socklen_t)sizeof(struct sockaddr_un)) == -1) {
      log_warn(LD_NET,get_lang_str(LANG_LOG_CONNECTION_COULD_NOT_BIND_TO_2),address,tor_socket_strerror(tor_socket_errno(s)));
      goto err;
    }

    if (listen(s,SOMAXCONN) < 0) {
      log_warn(LD_NET,get_lang_str(LANG_LOG_CONNECTION_COULD_NOT_LISTEN_ON_2),address,tor_socket_strerror(tor_socket_errno(s)));
      tor_close_socket(s);
      goto err;
    }
#endif /* HAVE_SYS_UN_H */
...
connection_init_accepted_conn(connection_t *conn, uint8_t listener_type)
{
  ...
  switch (conn->type) {
	...
	{	e_conn->socks_request->command=SOCKS_COMMAND_CONNECT;
		if(e_conn->socks_request->address[0] && !e_conn->socks_request->original_address[0])	strlcpy(e_conn->socks_request->original_address,e_conn->socks_request->address,MAX_SOCKS_ADDR_LEN);
...
{	int result;
	...
	{	if ((size_t)at_most > slack_in_buf && slack_in_buf >= 1024)
		...
		{	int pending;
			...
			log_debug(LD_NET,get_lang_str(LANG_LOG_CONNECTION_READ_STATS),conn->s,(long)buf_datalen(conn->inbuf),tor_tls_get_pending_bytes(or_conn->tls), (long)at_most);
		...
		if (n_read > 0)	/* change *max_to_read */ /*XXXX021 check for overflow*/
			*max_to_read = (int)(at_most - n_read);
		if (conn->type == CONN_TYPE_AP)
		{	edge_connection_t *edge_conn = TO_EDGE_CONN(conn);	/*XXXX021 check for overflow*/
...
{	int e;
	...
	if (conn->marked_for_close || conn->s < 0)	return 0; /* do nothing */
	...
	if (conn->type == CONN_TYPE_AP)
	{	edge_connection_t *edge_conn = TO_EDGE_CONN(conn);
		edge_conn->n_written += (int)n_written;		/*XXXX021 check for overflow.*/
	}
	connection_buckets_decrement(conn, approx_time(), n_read, n_written);
	...
	{	if (zlib)
		...
		{	if (CONN_IS_EDGE(conn))
			{	/* if it failed, it means we have our package/delivery windows set wrong compared to our max outbuf size. close the whole circuit. */
				log_warn(LD_NET,get_lang_str(LANG_LOG_CONNECTION_WRITE_TO_BUF_FAILED),conn->s);
				circuit_mark_for_close(circuit_get_by_edge_conn(TO_EDGE_CONN(conn)),END_CIRC_REASON_INTERNAL);
			}
			else
			{	log_warn(LD_NET,get_lang_str(LANG_LOG_CONNECTION_WRITE_TO_BUF_FAILED_2),conn->s);
				connection_mark_for_close(conn);
			}
			return;
		}
		...
		{	ssize_t extra = 0;
			...
			{	if (!conn->marked_for_close)
				{	/* this connection is broken. remove it. */
					log_warn(LD_BUG,get_lang_str(LANG_LOG_CONNECTION_UNHANDLED_ERROR),conn->type,conn->s);
					tor_fragile_assert();
