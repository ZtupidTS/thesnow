...
//	DEBUGOUT( "CMovieInfoDlg::OnInitDialog\n" );
	...
	::wsprintfA( szTemp, "%01d.%01d%01d",
		(m_wRecVersion&0xF00)>>8,
		(m_wRecVersion&0x0F0)>>4,
		(m_wRecVersion&0x00F) );
	::SetDlgItemTextA( m_hWnd, IDC_MIF_RERECORDVERSION, szTemp );

	::wsprintfA( szTemp, "%04X", m_wVersion );
	::SetDlgItemTextA( m_hWnd, IDC_MIF_VERSION, szTemp );

	// フレーム数
	::wsprintfA( szTemp, "%d frames", m_dwFrames );
	::SetDlgItemTextA( m_hWnd, IDC_MIF_FRAMES, szTemp );
	...
	::wsprintfA( szTemp, "%02d:%02d:%02d", h, m, s );
	::SetDlgItemTextA( m_hWnd, IDC_MIF_TIMES, szTemp );

	// 撮り直し回数
	if( m_dwRerecordTimes == 0xFFFFFFFF ) {
		::SetDlgItemTextA( m_hWnd, IDC_MIF_RERECORDTIMES, "----" );
	} else {
		::wsprintfA( szTemp, "%d times", m_dwRerecordTimes );
		::SetDlgItemTextA( m_hWnd, IDC_MIF_RERECORDTIMES, szTemp );
	}
