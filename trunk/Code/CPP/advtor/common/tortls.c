...
tls_log_errors(tor_tls_t *tls, int severity, const char *doing)
{
  ...
  while ((err = ERR_get_error()) != 0) {
    ...
    if (!func) func = "(null)";
    if (doing) {
      log(severity,LD_NET,get_lang_str(LANG_LOG_TLS_ERROR),doing,addr?get_lang_str(LANG_LOG_TLS__WITH):"", addr?addr:"",msg,lib,func);
    } else {
      log(severity, LD_NET, get_lang_str(LANG_LOG_TLS_ERROR_2),addr?get_lang_str(LANG_LOG_TLS__WITH):"", addr?addr:"",msg,lib,func);
    }
  }
}
                  ...
                  const char *doing, int severity)
{
  ...
  switch (err) {
    ...
    case SSL_ERROR_SYSCALL:
      ...
      if (r == 0) {
        log(severity,LD_NET,get_lang_str(LANG_LOG_TLS_UNEXPECTED_CLOSE),doing);
        tor_error = TOR_TLS_ERROR_IO;
      } else {
        int e = tor_socket_errno(tls->socket);
        log(severity,LD_NET,get_lang_str(LANG_LOG_TLS_SYSCALL_ERROR),doing,e,tor_socket_strerror(e));
    ...
    case SSL_ERROR_ZERO_RETURN:
      ...
      log(severity,LD_NET,get_lang_str(LANG_LOG_TLS_CONNECTION_CLOSED),doing);
      tls_log_errors(tls, severity, doing);
                           ...
                           unsigned int cert_lifetime)
{
 ...
 error:
  ...
  tls_log_errors(NULL,LOG_WARN,get_lang_str(LANG_LOG_TLS_GENERATING_CERT));
  if (sign_pkey)
...
tor_tls_context_new(crypto_pk_env_t *identity, unsigned int key_lifetime)
{
  ...
  if (!cert || !idcert) {
    log(LOG_WARN,LD_CRYPTO,get_lang_str(LANG_LOG_TLS_ERROR_CREATING_CERT));
    goto error;
  }
 ...
 error:
  tls_log_errors(NULL,LOG_WARN,get_lang_str(LANG_LOG_TLS_CREATING_TLS_CONTEXT));
  tor_free(nickname);
  tor_free(nn2);
  if (pkey)
    EVP_PKEY_free(pkey);
...
tor_tls_client_is_using_v2_ciphers(const SSL *ssl, const char *address)
{
  ...
  if (!(session = SSL_get_session((SSL *)ssl))) {
    log_warn(LD_NET,get_lang_str(LANG_LOG_TLS_NO_SESSION));
    return 0;
  }
  if (!session->ciphers) {
    log_warn(LD_NET,get_lang_str(LANG_LOG_TLS_NO_CIPHERS_ON_SESSION));
    return 0;
  }
  ...
  for (i = 0; i < sk_SSL_CIPHER_num(session->ciphers); ++i) {
    ...
    if (strcmp(ciphername, TLS1_TXT_DHE_RSA_WITH_AES_128_SHA) &&
      ...
      log_info(LD_NET,get_lang_str(LANG_LOG_TLS_NON_V1_CIPHER),ciphername);
      // return 1;
      goto dump_list;
    }
  }
 ...
 dump_list:
  {
    ...
    log_info(LD_NET,get_lang_str(LANG_LOG_TLS_NON_V1_CIPHER_2),address,s);
    tor_free(s);
    smartlist_free(elts);
  }
  return 1;
...
tor_tls_new(int sock, int isServer)
{
  ...
  if (!(result->ssl = SSL_new(global_tls_context->ctx))) {
    tls_log_errors(NULL,LOG_WARN,get_lang_str(LANG_LOG_TLS_GENERATING_TLS_CONTEXT));
    tor_free(result);
    return NULL;
  }
  ...
  if (!SSL_set_cipher_list(result->ssl,
    ...
    tls_log_errors(NULL,LOG_WARN,get_lang_str(LANG_LOG_TLS_SETTING_CIPHERS));
#ifdef SSL_set_tlsext_host_name
  ...
  if (! bio) {
    tls_log_errors(NULL,LOG_WARN,get_lang_str(LANG_LOG_TLS_OPENING_BIO));
#ifdef SSL_set_tlsext_host_name
    SSL_set_tlsext_host_name(result->ssl, NULL);
#endif
    SSL_free(result->ssl);
  ...
  if (r == TOR_TLS_DONE) {
    ...
    if (tls->isServer) {
      ...
      if (SSL_set_cipher_list(tls->ssl, SERVER_CIPHER_LIST) == 0) {
        tls_log_errors(NULL,LOG_WARN,get_lang_str(LANG_LOG_TLS_RESETTING_CIPHERS));
...
tor_tls_shutdown(tor_tls_t *tls)
{
  ...
  while (1) {
    if (tls->state == TOR_TLS_ST_SENTCLOSE) {
      ...
      err = tor_tls_get_error(tls,r,CATCH_ZERO,get_lang_str(LANG_LOG_TLS_READING_TO_SHUT_DOWN),LOG_INFO);
...
tor_tls_peer_has_cert(tor_tls_t *tls)
{
  ...
  tls_log_errors(tls,LOG_WARN,get_lang_str(LANG_LOG_TLS_GETTING_PEER_CERT));
  if (!cert)
    return 0;
  X509_free(cert);
  return 1;
...
log_cert_lifetime(X509 *cert, const char *problem)
{
  ...
  if (!(bio = BIO_new(BIO_s_mem()))) {
    log_warn(LD_GENERAL,get_lang_str(LANG_LOG_TLS_BIO_ALLOCATION_ERROR)); goto end;
  }
  if (!(ASN1_TIME_print(bio, X509_get_notBefore(cert)))) {
    tls_log_errors(NULL,LOG_WARN,get_lang_str(LANG_LOG_TLS_CERT_LIFETIME));
    goto end;
  }
  ...
  tls_log_errors(NULL,LOG_WARN,get_lang_str(LANG_LOG_TLS_GETTING_CERT_LIFETIME));
  if (bio)
    BIO_free(bio);
  if (s1)
    tor_free(s1);
...
tor_tls_verify(int severity, tor_tls_t *tls, crypto_pk_env_t **identity_key)
{
  ...
  *identity_key = _crypto_new_pk_env_rsa(rsa);
   ...
   * acts unexpectedly. */
  tls_log_errors(tls,LOG_WARN,get_lang_str(LANG_LOG_TLS_FINISHING_TOR_TLS_VERIFY));
...
tor_tls_check_lifetime(tor_tls_t *tls, int tolerance)
{
  ...
  if (cert)
    X509_free(cert);
  /* Not expected to get invoked */
  tls_log_errors(tls,LOG_WARN,get_lang_str(LANG_LOG_TLS_CHECKING_CERT_LIFETIME));
