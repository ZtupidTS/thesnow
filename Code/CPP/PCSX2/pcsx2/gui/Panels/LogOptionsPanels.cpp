	...
	: CheckedStaticBox( parent, wxVERTICAL, L"EE 日志" )
{
	...
	s_disasm.Add( m_VU0micro	= new pxCheckBox( m_disasmPanel, L"VU0 宏" ));
	s_disasm.Add( m_VU1micro	= new pxCheckBox( m_disasmPanel, L"VU1 宏" ));

	s_hw.Add( m_KnownHw		= new pxCheckBox( m_hwPanel, L"寄存器" ));
	s_hw.Add( m_UnknownHw	= new pxCheckBox( m_hwPanel, L"未知寄存器" ));
...
{
	m_masterEnabler = new pxCheckBox( this, _("启用追踪日志"),
