...
{	if(!hDlgPlugins) return;
	...
	{	if(plugin_list->rights&PLUGIN_RIGHT__CAN_BE_SHOWN_IN_PLUGIN_LIST)
		{	lvit.iSubItem=0;lvit.mask=LVIF_TEXT|LVIF_PARAM;lvit.state=0;lvit.stateMask=0;lvit.iImage=0;
			...
			lvit.pszText=plugin_list->dll_name;
			lvit.cchTextMax=256;
			i=SendDlgItemMessage(hDlgPlugins,26400,LVM_INSERTITEM,0,(LPARAM)&lvit);
			if(i!=-1)
			{	lvit.iItem=i;
...
{	if(!hDlgPlugins) return;
	...
	{	lvit.iItem=i;lvit.mask=LVIF_PARAM;lvit.iSubItem=0;
		lvit.lParam=0;
		if(!SendDlgItemMessage(hDlgPlugins,26400,LVM_GETITEM,0,(LPARAM)&lvit)) break;
...
{	int i=0;
	...
	{	lvit.iItem=i;lvit.mask=LVIF_PARAM;lvit.iSubItem=0;
		...
		{	lvit.stateMask=LVIS_SELECTED;
			...
			SendDlgItemMessage(hDlgPlugins,26400,LVM_SETITEM,lvit.iItem,(LPARAM)&lvit);
			return;
		}
		i++;
	}
...
{	if(uMsg==WM_INITDIALOG)
	...
	else if(uMsg==WM_COMMAND)
	{
		...
		{	lvit.iItem=SendDlgItemMessage(hDlg,26400,LVM_GETNEXTITEM,-1,MAKELPARAM(LVNI_SELECTED,0));
			if(lvit.iItem!=-1)
			{	lvit.lParam=0;lvit.mask=LVIF_PARAM;lvit.iSubItem=0;
				if(SendDlgItemMessage(hDlg,26400,LVM_GETITEM,0,(LPARAM)&lvit) && lvit.lParam)
				{	if(remove_plugin(hDlg,lvit.lParam))
					{	SendDlgItemMessage(hDlg,26400,LVM_DELETEITEM,lvit.iItem,0);
						lvit.stateMask=LVIS_SELECTED;
						lvit.mask=LVIF_STATE;
						lvit.state=LVIS_SELECTED;
						if(!SendDlgItemMessage(hDlg,26400,LVM_SETITEM,lvit.iItem,(LPARAM)&lvit) && lvit.iItem)
						{	lvit.iItem--;
							SendDlgItemMessage(hDlg,26400,LVM_SETITEM,lvit.iItem,(LPARAM)&lvit);
						}
					}
				}
			}
		...
		{	lvit.iItem=SendDlgItemMessage(hDlg,26400,LVM_GETNEXTITEM,-1,MAKELPARAM(LVNI_SELECTED,0));
			...
			{	lvit.lParam=0;lvit.mask=LVIF_PARAM;lvit.iSubItem=0;
				...
				{	tmp_plugin=find_plugin(lvit.lParam);
					if(tmp_plugin)	DialogBoxParamW(hInstance,(LPWSTR)MAKEINTRESOURCE(1120),hDlg,&dlgPluginRights,0);
				}
			}
		}
