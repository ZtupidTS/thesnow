...
void GCPadConfigDialog::OnButtonClick(wxCommandEvent& event)
{
	...
	ClickedButton->SetLabel(wxT("<Çë°´¼ü>"));
...
void GCPadConfigDialog::CreateGUIControls()
{
	...
	{
		StrJoyname.Add(wxT("<Î´¼ì²âµ½ÊÖ±ú>"));
	}
	...
	StrSource.Add(wxT("¼üÅÌ"));
	StrSource.Add(wxT("Ò¡¸Ë 1"));
	StrSource.Add(wxT("Ò¡¸Ë 2"));
	...
	for (int i = 0; i < 4; i++)
	{
		...
		m_Notebook->AddPage(m_Controller[i], wxString::Format(wxT("Gamecube ÊÖ±ú %d"), i+1));
		...
		// Dead zone
		m_ComboDeadZoneLabel[i] = new wxStaticText(m_Controller[i], wxID_ANY, wxT("ËÀÇø"));
		...
		// Stick Status Panels
		m_tStatusLeftIn[i] = new wxStaticText(m_Controller[i], wxID_ANY, wxT("Î´Á¬½Ó"));
		m_tStatusLeftOut[i] = new wxStaticText(m_Controller[i], wxID_ANY, wxT("Î´Á¬½Ó"));
		m_tStatusRightIn[i] = new wxStaticText(m_Controller[i], wxID_ANY, wxT("Î´Á¬½Ó"));
		m_tStatusRightOut[i] = new wxStaticText(m_Controller[i], wxID_ANY, wxT("Î´Á¬½Ó"));
		...
		m_gStickLeft[i] = new wxStaticBoxSizer (wxHORIZONTAL, m_Controller[i], wxT("Ò¡¸Ë 1 Status (In) (Out)"));
		m_gStickLeft[i]->Add(m_sGridStickLeft[i], 0, (wxLEFT | wxRIGHT), 5);

		m_gStickRight[i] = new wxStaticBoxSizer (wxHORIZONTAL, m_Controller[i], wxT("Ò¡¸Ë 2 Status (In) (Out)"));
