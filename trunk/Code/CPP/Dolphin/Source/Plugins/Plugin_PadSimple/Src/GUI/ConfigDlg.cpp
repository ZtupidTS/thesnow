...
inline void AddControl(wxPanel *pan, wxButton **button,
		...
		wxStaticBoxSizer *sizer, const WCHAR *name, int ctl, int controller)
{
...
//	hButton->Add(new wxStaticText(pan, 0, wxString::FromAscii(name), 
	hButton->Add(new wxStaticText(pan, 0, name, 
		...
		wxStaticBoxSizer *sizer, const WCHAR *name, int ctl, int controller)
{
	...
	// Add the label
	hSlider->Add(new wxStaticText(pan, 0, name, 
	             wxDefaultPosition, wxDefaultSize), 0,
	             wxALIGN_CENTER_VERTICAL|wxALL);
...
void PADConfigDialogSimple::CreateGUIControls()
{
	...
	{	
		...
		sbDevice[i] = new wxStaticBoxSizer(wxVERTICAL, m_Controller[i], wxT("控制器设置"));
		m_Disable[i] = new wxCheckBox(m_Controller[i], ID_DISABLE, wxT("Dolphin 失去焦点则禁用"), wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator);
		...
		m_X360Pad[i] = new wxCheckBox(m_Controller[i], ID_X360PAD, wxT("启用 X360Pad"), wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator);
		...
		m_Rumble[i] = new wxCheckBox(m_Controller[i], ID_RUMBLE, wxT("启用 rumble"), wxDefaultPosition, wxDefaultSize, 0, wxDefaultValidator);
		...
		{
			m_X360Pad[i]->SetLabel(wxT("启用 X360Pad - 没有手柄连接"));
		...
		// Buttons
		// -----------------------------
		sButtons[i] = new wxStaticBoxSizer(wxVERTICAL, m_Controller[i], wxT("按钮"));

		AddControl(m_Controller[i], &(m_ButtonA[i]), sButtons[i], wxT("A: "), CTL_A, i);
		AddControl(m_Controller[i], &(m_ButtonB[i]), sButtons[i], wxT("B: "), CTL_B, i);
		AddControl(m_Controller[i], &(m_ButtonX[i]), sButtons[i], wxT("X: "), CTL_X, i);
		AddControl(m_Controller[i], &(m_ButtonY[i]), sButtons[i], wxT("Y: "), CTL_Y, i);
		AddControl(m_Controller[i], &(m_ButtonZ[i]), sButtons[i], wxT("Z: "), CTL_Z, i);
		AddControl(m_Controller[i], &(m_ButtonStart[i]), sButtons[i], wxT("开始: "), CTL_START, i);

		sTriggers[i] = new wxStaticBoxSizer(wxVERTICAL, m_Controller[i], wxT("Triggers"));

		AddControl(m_Controller[i], &(m_ButtonL[i]), sTriggers[i], wxT("L: "), CTL_L, i);
		AddControl(m_Controller[i], &(m_ButtonR[i]), sTriggers[i], wxT("R: "), CTL_R, i);
		AddControl(m_Controller[i], &(m_ButtonL_Semi[i]), sTriggers[i], wxT("Semi-L: "), CTL_L_SEMI, i);
		AddControl(m_Controller[i], &(m_ButtonR_Semi[i]), sTriggers[i], wxT("Semi-R: "), CTL_R_SEMI, i);
		AddSlider(m_Controller[i], &(m_Trigger_SemiValue[i]), sTriggers[i], wxT("Semi: "), ID_TRIGGER_SEMIVALUE, i);

		sStick[i] = new wxStaticBoxSizer(wxVERTICAL, m_Controller[i], wxT("Main Stick"));

		AddControl(m_Controller[i], &(m_StickUp[i]), sStick[i], wxT("上: "), CTL_MAINUP, i);
		AddControl(m_Controller[i], &(m_StickDown[i]), sStick[i], wxT("下: "), CTL_MAINDOWN, i);
		AddControl(m_Controller[i], &(m_StickLeft[i]), sStick[i], wxT("左: "), CTL_MAINLEFT, i);
		AddControl(m_Controller[i], &(m_StickRight[i]), sStick[i], wxT("右: "), CTL_MAINRIGHT, i);
		AddControl(m_Controller[i], &(m_Stick_Semi[i]), sStick[i], wxT("Semi-press: "), CTL_MAIN_SEMI, i);
		AddSlider(m_Controller[i], &(m_Stick_SemiValue[i]), sStick[i], wxT("Semi: "), ID_MAIN_SEMIVALUE, i);

		sDPad[i] = new wxStaticBoxSizer(wxVERTICAL, m_Controller[i], wxT("D-Pad"));

		AddControl(m_Controller[i], &(m_DPadUp[i]), sDPad[i], wxT("上: "), CTL_DPADUP, i);
		AddControl(m_Controller[i], &(m_DPadDown[i]), sDPad[i], wxT("下: "), CTL_DPADDOWN, i);
		AddControl(m_Controller[i], &(m_DPadLeft[i]), sDPad[i], wxT("左: "), CTL_DPADLEFT, i);
		AddControl(m_Controller[i], &(m_DPadRight[i]), sDPad[i], wxT("右: "), CTL_DPADRIGHT, i);

		sCStick[i] = new wxStaticBoxSizer(wxVERTICAL, m_Controller[i], wxT("C-Stick"));

		AddControl(m_Controller[i], &(m_CStickUp[i]), sCStick[i], wxT("上: "), CTL_SUBUP, i);
		AddControl(m_Controller[i], &(m_CStickDown[i]), sCStick[i], wxT("下: "), CTL_SUBDOWN, i);
		AddControl(m_Controller[i], &(m_CStickLeft[i]), sCStick[i], wxT("左: "), CTL_SUBLEFT, i);
		AddControl(m_Controller[i], &(m_CStickRight[i]), sCStick[i], wxT("右: "), CTL_SUBRIGHT, i);
		AddControl(m_Controller[i], &(m_CStick_Semi[i]), sCStick[i], wxT("Semi-press: "), CTL_SUB_SEMI, i);
		AddSlider(m_Controller[i], &(m_CStick_SemiValue[i]), sCStick[i], wxT("Semi: "), ID_SUB_SEMIVALUE, i);
...
void PADConfigDialogSimple::DllAbout(wxCommandEvent& event)
{
	wxString message;
#ifdef _WIN32
	message = _("dolphin中的一个简单的键盘与 XInput 插件.");
#else
	message = _("A simple keyboard plugin for dolphin.");
#endif

	wxMessageBox(_T("Dolphin PadSimple 插件\nBy ector and F|RES\n\n" + message),
