...
connection_or_read_proxy_response(or_connection_t *or_conn)
{
  ...
  switch (fetch_from_buf_http(conn->inbuf,
                              ...
                              NULL, NULL, 10000, 0)) {
    case -1: /* overflow */
      log_warn(LD_PROTOCOL,get_lang_str(LANG_LOG_CONN_OR_HTTPS_OVERSIZED_RESPONSE));
      return -1;
    case 0:
      log_info(LD_OR,get_lang_str(LANG_LOG_CONN_OR_HTTPS_INCOMPLETE_RESPONSE));
      return 0;
    /* case 1, fall through */
  }
  ...
  if (parse_http_response(headers, &status_code, &date_header,
                          NULL, &reason) < 0) {
    log_warn(LD_OR,get_lang_str(LANG_LOG_CONN_OR_PROXY_RESPONSE_PARSE_FAILED),conn->address);
    tor_free(headers);
    return -1;
  }
  if (!reason) reason = tor_strdup(get_lang_str(LANG_LOG_CONN_OR_NO_REASON));

  if (status_code == 200) {
    log_info(LD_OR,get_lang_str(LANG_LOG_CONN_OR_HTTPS_OK),conn->address,escaped(reason));
    tor_free(reason);
  ...
  log_warn(LD_OR,get_lang_str(LANG_LOG_CONN_OR_HTTPS_STATUS_ERROR),status_code,escaped(reason));
  tor_free(reason);
  connection_mark_for_close(conn);
  return -1;
}
...
connection_or_finished_flushing(or_connection_t *conn)
{
  ...
  switch (conn->_base.state) {
    case OR_CONN_STATE_PROXY_FLUSHING:
      log_debug(LD_OR,get_lang_str(LANG_LOG_CONN_OR_CONNECT_SENT));
...
connection_or_finished_connecting(or_connection_t *or_conn)
{
  ...
  log_debug(LD_OR,get_lang_str(LANG_LOG_CONN_OR_FINISHED_CONNECTING),conn->address,conn->port);
  ...
  if (get_options()->HttpsProxy) {
    ...
    if (authenticator) {
      ...
      if (!base64_authenticator)
        log_warn(LD_OR,get_lang_str(LANG_LOG_CONN_OR_HTTPS_AUTH_ENCODING_FAILED));
    }
    if (base64_authenticator) {
                                        ...
                                        char *digest_rcvd_out)
{
  ...
  if (has_cert) {
    ...
    if (started_here && v<0) {
      log_fn(severity,LD_OR,get_lang_str(LANG_LOG_CONN_OR_INVALID_CERT_FROM_ROUTER),safe_address,conn->_base.port);
        return -1;
    } else if (v<0) {
      log_info(LD_PROTOCOL,get_lang_str(LANG_LOG_CONN_OR_INVALID_CERT_FROM_CONN));
    } else {
      log_debug(LD_OR,get_lang_str(LANG_LOG_CONN_OR_CERT_OK),conn_type,safe_address,conn->_base.port);
    }
    check_no_tls_errors();
  }
  ...
  if (started_here && tor_digest_is_zero(conn->identity_digest)) {
    ...
    base16_encode(conn->nickname+1, HEX_DIGEST_LEN+1,
                  conn->identity_digest, DIGEST_LEN);
    log_info(LD_OR,get_lang_str(LANG_LOG_CONN_OR_CONNECTED_WITHOUT_KEY),conn->nickname,conn->_base.address,conn->_base.port);
  }
  ...
  if (started_here) {
    ...
    if (memcmp(digest_rcvd_out, conn->identity_digest, DIGEST_LEN)) {
      ...
      base16_encode(expected, sizeof(expected), conn->identity_digest,
                    DIGEST_LEN);
      log_fn(severity, LD_OR,get_lang_str(LANG_LOG_CONN_OR_UNEXPECTED_KEY_FROM_ROUTER),conn->_base.address,conn->_base.port,expected,seen);
...
connection_tls_finish_handshake(or_connection_t *conn)
{
  ...
  log_debug(LD_OR,get_lang_str(LANG_LOG_CONN_OR_TLS_HANDSHAKE_DONE),safe_str(conn->_base.address));
