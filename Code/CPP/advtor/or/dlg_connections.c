...
void initmemunits(int combo)
{	SendDlgItemMessage(hDlgConnections,combo,CB_SETITEMDATA,SendDlgItemMessage(hDlgConnections,combo,CB_ADDSTRING,0,(LPARAM)"Bytes"),(LPARAM)0);
	SendDlgItemMessage(hDlgConnections,combo,CB_SETITEMDATA,SendDlgItemMessage(hDlgConnections,combo,CB_ADDSTRING,0,(LPARAM)"KB"),(LPARAM)10);
	SendDlgItemMessage(hDlgConnections,combo,CB_SETITEMDATA,SendDlgItemMessage(hDlgConnections,combo,CB_ADDSTRING,0,(LPARAM)"MB"),(LPARAM)20);
	SendDlgItemMessage(hDlgConnections,combo,CB_SETITEMDATA,SendDlgItemMessage(hDlgConnections,combo,CB_ADDSTRING,0,(LPARAM)"GB"),(LPARAM)30);
	SendDlgItemMessage(hDlgConnections,combo,CB_SETITEMDATA,SendDlgItemMessage(hDlgConnections,combo,CB_ADDSTRING,0,(LPARAM)"TB"),(LPARAM)40);
}

void setbandwidthval(uint64_t bwrate,int checkbox,int editbox,int combo,int defval)
{	int i=0,j=0;
	if(!bwrate)
	{	CheckDlgButton(hDlgConnections,checkbox,BST_UNCHECKED);
		EnableWindow(GetDlgItem(hDlgConnections,editbox),0);
		EnableWindow(GetDlgItem(hDlgConnections,combo),0);
	}
	else	CheckDlgButton(hDlgConnections,checkbox,BST_CHECKED);
	for(j=0;j<5 && bwrate;j++)
	{	if((bwrate&0x3ff)==0){i++;bwrate/=1024;}
		else	break;}
	SendDlgItemMessage(hDlgConnections,combo,CB_SETCURSEL,i,0);
	i=bwrate&0xffffffff;
	SetDlgItemInt(hDlgConnections,editbox,i,0);
}
...
{	if(uMsg==WM_INITDIALOG)
	{	hDlgConnections=hDlg;
		...
		initmemunits(10300);initmemunits(10301);initmemunits(10302);initmemunits(10303);initmemunits(10304);initmemunits(10305);
		setbandwidthval(tmpOptions->BandwidthRate,10400,10100,10300,5*1024*1024);
		setbandwidthval(tmpOptions->BandwidthBurst,10401,10101,10301,10*1024*1024);
		setbandwidthval(tmpOptions->MaxAdvertisedBandwidth,10402,10102,10302,1024*1024*1024);
		setbandwidthval(tmpOptions->RelayBandwidthRate,10403,10103,10303,0);
		setbandwidthval(tmpOptions->RelayBandwidthBurst,10404,10104,10304,0);
		setbandwidthval(tmpOptions->AccountingMax,10405,10105,10305,0);
