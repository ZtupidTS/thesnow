...
CHAR	buf[512];
	...
	string	tempstr;
	tempstr = CPathlib::MakePathExt( (LPSTR)nes->rom->GetRomPath(), (LPSTR)nes->rom->GetRomName(), "vtd" );
	DEBUGOUT( "Path: %s\n", tempstr.c_str() );
	if( !(fp = ::fopen( tempstr.c_str(), "r" )) ) {
		// デフォルトファイル名で読んで見る
		tempstr = CPathlib::MakePathExt( (LPSTR)nes->rom->GetRomPath(), "Default", "vtd" );
		DEBUGOUT( "Path: %s\n", tempstr.c_str() );
		if( !(fp = ::fopen( tempstr.c_str(), "r" )) ) {
