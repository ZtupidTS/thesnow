...
router_reload_v2_networkstatus(void)
{
  ...
  SMARTLIST_FOREACH(entries, const char *, fn, {
      ...
      if (strlen(fn) != HEX_DIGEST_LEN ||
        ...
        log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_UNEXPECTED_FILE_NAME),fn);
        continue;
      }
      filename = get_datadir_fname2("cached-status", fn);
      s = read_file_to_str(filename, 0, &st);
      if (s) {
        if (router_set_networkstatus_v2(s, update_time(st.st_mtime), NS_FROM_CACHE,
                                        NULL)<0) {
...
router_reload_consensus_networkstatus(void)
{
  ...
  s = read_file_to_str(filename, RFTS_IGNORE_MISSING, NULL);
  if (s) {
    if (networkstatus_set_current_consensus(s,flags|NSSET_WAS_WAITING_FOR_CERTS)) {
  ...
  if (!current_consensus ||
      (stat(options->FallbackNetworkstatusFile, &st)==0 &&
       update_time(st.st_mtime) > current_consensus->valid_after)) {
    s = read_file_to_str(options->FallbackNetworkstatusFile,
                         RFTS_IGNORE_MISSING, NULL);
    if (s) {
      if (networkstatus_set_current_consensus(s,flags|NSSET_ACCEPT_OBSOLETE)) {
  ...
  SMARTLIST_FOREACH(consensus->voters, networkstatus_voter_info_t *, voter,
  {
    if (!voter->good_signature && !voter->bad_signature && voter->signature) {
      ...
      if (!is_v3_auth) {
        ...
        ++n_unknown;
        continue;
      } else if (!cert || (cert->expires < now && !(options->DirFlags&2))) {
...
networkstatus_v2_list_clean(time_t now)
{
  int i;
  or_options_t *options = get_options();
  if ((!networkstatus_v2_list)||(options->DirFlags&2))
    return;

  for (i = 0; i < smartlist_len(networkstatus_v2_list); ++i) {
    networkstatus_v2_t *ns = smartlist_get(networkstatus_v2_list, i);
    char *fname = NULL;
    if ((ns->published_on + MAX_NETWORKSTATUS_AGE > now)||(options->DirFlags&2))
      continue;
    ...
    if (file_status(fname) == FN_FILE) {
      log_info(LD_DIR,get_lang_str(LANG_LOG_NETWORKSTATUS_TOO_OLD),fname);
      unlink(fname);
    }
    tor_free(fname);
    if (directory_caches_v2_dir_info(get_options())) {
...
//  else if(get_options()->DirFlags&2) return;
...
update_consensus_networkstatus_fetch_time(time_t now)
{
  ...
  if (c) {
    ...
    if(options->DirFlags&2) time_to_download_next_consensus = -1;
    else
    {
...
{
  or_options_t *options = get_options();
  if (should_delay_dir_fetches(options)||((options->DirFlags&2)&&current_consensus))
    return;
  if (directory_fetches_dir_info_early(options))
...
{
  if (current_consensus && (((current_consensus->valid_after <= now) && (now <= current_consensus->valid_until))||(get_options()->DirFlags&2)))
    return current_consensus;
  else
    return NULL;
}
...
#define REASONABLY_LIVE_TIME (24*60*60)
  if (current_consensus && (((current_consensus->valid_after <= now) && (now <= current_consensus->valid_until+REASONABLY_LIVE_TIME))||(get_options()->DirFlags&2)))
    return current_consensus;
  else
    return NULL;
}
...
networkstatus_set_current_consensus(const char *consensus, unsigned flags)
{
  ...
  {	if((now - current_consensus->valid_after)<0) update_best_delta_t(best_delta_t + now - current_consensus->valid_after + crypto_rand_int(20*60));
  }
  else if (ftime_definitely_before(now, current_consensus->valid_after))
...
networkstatus_dump_bridge_status_to_file(time_t now)
{
  ...
  char *fname = get_datadir_fname("networkstatus-bridges");
